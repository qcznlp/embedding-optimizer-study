...
from scripts.dense_trainer_normalization_candidate import verify_stack

EXTRA_SOURCES = (
    "scripts/audit_dense_v3_dimension_chain.py",
    "tests/test_primary_v3_dimension_vectors.py",
    "tests/test_primary_v3_dimensions.py",
    "scripts/dense_trainer_normalization_candidate.py",
)
...
def verify_cpu_loading(primary, reference):
    ...
    try:
        ...
        with TensorStore(reference) as store:
            if not all(name.startswith("0.") for name in store.keys()) or set(actual) != {
                "0.model." + name[2:] for name in store.keys()
            }:
            ...
            for name in store.keys():
                ...
                loaded = actual["0.model." + name[2:]].detach().cpu()
                if (
...
def run(args):
    ...
    stack_bindings = verify_stack()
    actual_runtime = runtime_snapshot(
        ["torch", "numpy", "scipy", "sentence-transformers", "transformers", "accelerate"]
    )
    ...
    return {
        ...
        "pinned_stack_bindings": stack_bindings,
        "inputs": inputs,
        "actual_probe": probe,
