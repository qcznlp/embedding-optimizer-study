...
def verify_token_columns(model, rows, features):
    if len(features) != len(TEXT_COLUMNS):
        raise ValueError("Unexpected feature column count")
    for column, feature in zip(TEXT_COLUMNS, features, strict=True):
        ...
        left, right = feature["input_ids"], manual["input_ids"]
        if isinstance(left, torch.Tensor) and isinstance(right, torch.Tensor):
            matches = left.dtype == right.dtype and torch.equal(left, right)
...
def main():
    ...
    for run_id in args.run_ids:
        ...
        verify_token_columns(model, rows, features)
