...
def main():
    ...
    for state in ("adamw_state", "muon_state"):
        ...
        def reject_attribute(loaded_model, target, name, wrong):
            old = getattr(target, name)
            try:
                setattr(target, name, wrong)
                try:
                    factory.model_configuration(loaded_model, source, diagnostic_cpu=True)
                except ValueError:
        ...
        reject_attribute(model, model[0].model.config, "local_attention", 64)
        reject_attribute(model, model[0].model.config, "attention_dropout", 0.1)
        reject_attribute(model, model[0].model.config, "layer_norm_eps", 0.001)
        reject_attribute(model, model[1], "pooling_mode", "mean")
        reject_attribute(
            model, model, "prompts", {"query": "different: ", "document": "document: "}
        )
        reject_attribute(model, model, "truncate_dim", 128)
        reject_attribute(model, model, "training", False)
        reject_attribute(model, model.tokenizer, "padding_side", "left")
        reject_attribute(model, model.tokenizer, "truncation_side", "left")
        reject_attribute(model, model.tokenizer, "model_max_length", 512)
    ...
    if (args.output.parent / "not-created-training-output").exists():
        raise ValueError("Loading-only factory audit unexpectedly created training output")
    receipt = {
