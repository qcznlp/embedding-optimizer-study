...
CLAIM_WORDING = "configs/dense_no_packing_state_operator_claim_wording_amendment.json"
CLAIM_ARCHIVE = "reports/paper-review/factorial-claims-v2/before"
CLAIM_SOURCE_PATHS = {
    "publication_renderer": "src/embed_optim/state_operator_factorial_publication.py",
    "manuscript_topology": "paper/main.tex",
    "conceptual_figure_generator": "scripts/plot_weight_space_dimension_map.py",
    "development_fixture_generator": "scripts/render_state_operator_pending.py",
}
CLAIM_RENDERING_CONTRACT = {
    "abstract": "Report all three final 50K-continuation endpoint contrasts and marginal intervals within the existing 200-word abstract limit; do not describe them as a decomposition of the primary-training gain.",
    "main_mechanism": "Report averaged source-state and continuation contrasts and their interaction with every original estimate, marginal 95% interval and decision; calibration matches the fixed probe, not the first realized shuffled-batch update.",
    "main_conclusion": "Propagate the same source-bound endpoint contrasts without implying within-level benefit, feature mediation, equivalence or effect dominance.",
    "appendix": "Report all three co-primary contrasts with their marginal, not simultaneous, intervals and retain every exact seed-task contrast and probe trajectory.",
    "result_blind_branching": {
        "positive_interaction": "The relative Muon continuation effect is more favorable from the Muon-reached state; this need not imply benefit from either state or full-trajectory co-adaptation.",
        "weight_state_only": "The source-state contrast is positive after averaging continuation operators; this does not establish dominance or equivalence of the other effects to zero.",
        "operator_only": "The continuation contrast is positive after averaging the fixed source states; this need not hold within both states or dominate other contrasts.",
        "weight_and_operator_without_interaction": "Both averaged contrasts favor Muon, but their positive signs do not establish additivity; interpret the interaction separately.",
        "no_positive_effect": "Report supported negative contrasts or inconclusive intervals without treating nonsignificance as equivalence or absence of an effect.",
    },
}
CLAIM_BOUNDARY = (
    "These are final post-continuation contrasts for one fixed source pair and 50K horizon, "
    "not baseline-subtracted gains or a decomposition of primary training. The endpoint diagonal "
    "difference equals state plus operator, not state plus operator plus interaction. Neither "
    "averaged effects nor interaction alone establish within-level benefit, effect dominance, "
    "feature mediation or full-trajectory co-adaptation."
)
...
def _validate_prior_amendment_chain(
    repository: Path, protocol: dict, *, publication_snapshot: Path | None = None
) -> tuple[dict, dict]:
    """Reconstruct the original layers, optionally against their one exact archived publisher."""
    publication_source = repository / FACTORIAL_PUBLICATION
    if publication_snapshot is not None:
        expected_path = repository / CLAIM_ARCHIVE / FACTORIAL_PUBLICATION
        if publication_snapshot != expected_path:
            raise ValueError("Undeclared historical publication snapshot")
        publication_source = publication_snapshot
    ...
    for item in [*before.values(), *after.values(), *combined["evidence_bindings"]]:
        checked = item
        if publication_snapshot is not None and item is after[FACTORIAL_PUBLICATION]:
            # Preserve its expected original bytes/hash, changing only the lookup
            # to the one declared predecessor archive. No general source override.
            checked = {**item, "path": str(publication_snapshot.relative_to(repository))}
        if not controller._binding_valid(checked, repository):
    ...
    if json.loads(publication_source.read_text()) != expected_factorial_publication:
...
def validate_amendment_chain(repository: Path, protocol: dict) -> tuple[dict, dict]:
    """Verify a publication-only fourth layer after fully reconstructing its predecessor."""
    binding = protocol.get("parent_bindings", {}).get("claim_wording")
    if binding is None:
        return _validate_prior_amendment_chain(repository, protocol)
    if binding.get("path") != CLAIM_WORDING or not controller._binding_valid(binding, repository):
        raise ValueError("Claim wording amendment identity changed")
    amendment = json.loads((repository / CLAIM_WORDING).read_text())
    prior_completion = amendment.get("prior_prepared_completion_protocol", {})
    prior_publication = amendment.get("prior_prepared_publication_protocol", {})
    sources = amendment.get("new_source_bindings", {})
    evidence = amendment.get("evidence_bindings", [])
    evidence_paths = {
        "reports/paper-review/factorial-claims-v1/counterexamples.json",
        "reports/paper-review/factorial-claims-v1/exact-algebra.json",
        "reports/paper-review/factorial-claims-v1/candidate-tests-v4.xml",
        "reports/paper-review/factorial-claims-v1/validation.json",
    }
    if (
        amendment.get("status") != "prospective_factorial_claim_wording_amendment"
        ...
        or amendment.get("interpretation_narrowed") is not True
        or prior_completion.get("path") != f"{CLAIM_ARCHIVE}/{COMPLETION}"
        or prior_publication.get("path") != f"{CLAIM_ARCHIVE}/{FACTORIAL_PUBLICATION}"
        or set(sources) != set(CLAIM_SOURCE_PATHS)
        or any(sources[key].get("path") != path for key, path in CLAIM_SOURCE_PATHS.items())
        or len(evidence) != len(evidence_paths)
        or {item.get("path") for item in evidence} != evidence_paths
        or amendment.get("rendering_contract") != CLAIM_RENDERING_CONTRACT
        or amendment.get("claim_boundary") != CLAIM_BOUNDARY
        or amendment.get("allowed_completion_protocol_changes")
        != [
            "parent_bindings.publication_protocol",
            "parent_bindings.claim_wording",
            "amended_at_utc",
        ]
    ):
        raise ValueError("Unexpected claim wording scope")
    for item in [prior_completion, prior_publication, *sources.values(), *evidence]:
        if not controller._binding_valid(item, repository):
            raise ValueError("Claim wording evidence/source changed")
    previous = json.loads((repository / prior_completion["path"]).read_text())
    previous_publication = json.loads((repository / prior_publication["path"]).read_text())
    first, _ = _validate_prior_amendment_chain(
        repository,
        previous,
        publication_snapshot=repository / prior_publication["path"],
    )
    original_parents = previous_publication["parent_bindings"]
    implementation = json.loads(
        (repository / original_parents["implementation_protocol"]["path"]).read_text()
    )
    unchanged = {
        **original_parents,
        "factorial_summarizer": implementation["source_bindings"]["factorial_summarizer"],
    }
    if amendment.get("unchanged_scientific_bindings") != unchanged or any(
        not controller._binding_valid(item, repository) for item in unchanged.values()
    ):
        raise ValueError("Claim wording changed a numerical/scientific dependency")
    expected_publication = deepcopy(previous_publication)
    expected_publication["source_bindings"].update(sources)
    expected_publication["amendments"].append(binding)
    expected_publication["rendering_contract"] = CLAIM_RENDERING_CONTRACT
    expected_publication["claim_boundary"] = CLAIM_BOUNDARY
    expected_publication["amended_at_utc"] = amendment["frozen_at_utc"]
    if json.loads((repository / FACTORIAL_PUBLICATION).read_text()) != expected_publication:
        raise ValueError("Undeclared factorial publication change")
    expected = deepcopy(previous)
    expected["parent_bindings"]["publication_protocol"] = controller._identity(
        repository / FACTORIAL_PUBLICATION, repository
    )
    expected["parent_bindings"]["claim_wording"] = binding
...
def build_plan(repository: Path) -> dict:
    ...
    return {
        ...
        "claim_wording_dependency_verified": "claim_wording" in protocol["parent_bindings"],
