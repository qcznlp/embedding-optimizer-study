...
def validate(args):
    ...
    tests = {}
    for filename, count in (("focused-final.xml", 38), ("full-final.xml", 2419)):
    ...
    failed = next(ET.parse(archive / "attempt-1/full-failed.xml").iter("testsuite")).attrib
    assert int(failed["tests"]) == 2418 and int(failed["failures"]) == 17
    assert int(failed["errors"]) == 0 and int(failed["skipped"]) == 0
    ...
    return {
        ...
        "tests": tests,
        "preserved_first_full_suite_failure": failed,
