...
@torch.no_grad()
def direct_step(config, entry, names, layout, gradients, lr_lambdas=None):
    ...
    if "scheduler" in entry:
        if lr_lambdas is None:
            raise ValueError("Scheduled replay requires the observed actual scheduler functions")
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, copy.deepcopy(lr_lambdas))
        scheduler.load_state_dict(copy.deepcopy(entry["scheduler"]))
        replay.assert_exact_tree(scheduler.state_dict(), entry["scheduler"], "scheduler_entry")
    # LambdaLR construction changes group rates; restore the exact entry afterward.
    ...
    if scheduler is not None:
        result["scheduler"] = copy.deepcopy(scheduler.state_dict())
    return result
...
def exact_update(actual, expected):
    keys = (
        ...
        if "scheduler" in expected
        else ("weights", "optimizer")
    )
    for key in keys:
...
def analyze(config, reference, restored, names, layout, lr_lambdas, output, rank):
    ...
    left_update = direct_step(config.optimizer, entry, names, layout, left, lr_lambdas)
    exact_update(left_update, reference["entry_states"][2])
    right_update = direct_step(config.optimizer, entry, names, layout, right, lr_lambdas)
    exact_update(right_update, restored["entry_states"][2])
    repeat = direct_step(config.optimizer, entry, names, layout, left, lr_lambdas)
...
def main():
    ...
    original_scheduler = train.OptimizerTrainer.create_scheduler
    observed, references = {}, {}
    ...
    def create_scheduler(trainer, *a, **kw):
        scheduler = original_scheduler(trainer, *a, **kw)
        if type(scheduler) is not torch.optim.lr_scheduler.LambdaLR:
            raise ValueError("Require the actual declared LambdaLR scheduler")
        observed["lr_lambdas"] = copy.deepcopy(scheduler.lr_lambdas)
        return scheduler
    ...
    def segment(config, fixture, segment_output, *a, **kw):
        ...
        if segment_output.name == "uninterrupted":
            ...
            measured = analyze(
                ...
                snapshot,
                **topology,
                lr_lambdas=observed["lr_lambdas"],
                output=output,
                rank=rank,
            )
            gathered = [None] * 4
    ...
    try:
        with (
            ...
            patch.object(train.OptimizerTrainer, "create_scheduler", create_scheduler),
