...
The prepared combined handoff amendment now requires repaired main contract `af646ecf...`.
It preserves all completion conditions and the live interpreter setting `/usr/bin/python3`.
The read-only projection in `reports/experiment-integrity/combined-successor-contract-projection.json`
reconstructs both preparation layers, verifies all five actual consumer protocol loaders and
...
deployment contract is `37a4485c...`, distinct from isolated dry-run contract `a64b2658...`.
The earlier header-only amendment and its projected contracts remain archived historical proposals.
