...
state differs from the one AdamW creates, and where any full-corpus retrieval value resides.
...
5. Is retrieval value carried by the reached weights, by the next optimizer transform, or by a
   state-by-operator interaction?
...
- saved-checkpoint segment magnitude relative to weight norm;
...
- rank-16 left and right subspace overlap at matched optimizer dose and stage.
...
- remove shared random coordinate sets at 10%, 25%, 50%, and 75%;
- delete each coordinate in turn and recompute shortlist nDCG and margin;
- separate helpful mass, degrading mass, and participation of helpful attribution;
...
reported, including failures. Predictive support is not causal mediation.

## Decisive mechanism experiment
...
The initial global update scale is calibrated before branch training. All cells use the same
50,000-query horizon under three fixed data-order seeds. The factorial estimates:

- reached-state effect: average difference between Muon- and AdamW-reached states;
- continuation-operator effect: average difference between reset Muon and reset AdamW;
- interaction: whether the continuation effect depends on which state was reached.

This distinguishes three stories:

- **state finder:** Muon creates a better state that remains better under either continuation;
- **locally better transform:** reset Muon helps from both states;
- **closed-loop co-adaptation:** Muon continuation is especially useful from a Muon-reached state.

If none is supported, the empirical optimizer result remains valid but mechanistically unresolved.
...
6. **How the optimizers reshape the retriever:** weight trajectories, dimension utility, and
   out-of-dose bridges.
7. **Where retrieval value resides:** crossed state-by-operator intervention.
...
- **Muon retrieval gain + reached-state effect:** Muon is a state finder; the new state carries
  value even after optimizer reset.
- **Muon retrieval gain + operator effect:** Muon's continuation transform is useful across states.
- **Muon retrieval gain + interaction:** optimizer and state co-adapt in a closed loop.
...
- **Muon retrieval gain + no supported bridge/factorial effect:** report the gain and the falsified
  mechanisms; do not manufacture an explanation.
- **No Muon retrieval gain:** the paper becomes a bounded negative result about optimizer switching;
  weight-space differences remain descriptive unless independently linked to retrieval.
...
| reached-state factorial effect | value carried by tested source weights | universal Muon superiority |
| state-by-operator interaction | co-adaptation at the tested state/horizon | full-trajectory mediation |
