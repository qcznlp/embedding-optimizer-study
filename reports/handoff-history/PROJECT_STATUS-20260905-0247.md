...
Last updated: 2026-09-05 02:47 UTC
...
**Training milestone:** the artifact-only snapshot at 02:41 UTC records **9/12 complete** and
49/60 locally resumable checkpoints. W&B independently confirms NorMuon 1e-4 finished all 3907
steps at 02:46 UTC; 3e-4 is running at 3840 and the exact 1e-3 successor handle is running at step
40. Only 3e-3 remains unstarted. By 02:47 the existing finalizer reports all nine completed-run
backups complete and the sealed supervisor reports **49/60 remote coverage**, zero cycle failures.
These automatic receipts do not extend the earlier independent 808-file audit to the ninth run.
No manual controller or training job was launched for this handoff.

**Real weight-space progress:** the unchanged frozen CPU geometry entry point has now processed
Muon 1e-3 and 3e-3 at all five stages, increasing raw primary coverage from 30 to **40 checkpoints**
across all four AdamW and four Muon rates. All 3,520 hidden-matrix records, 48 output files and 82
unique model/metadata inputs pass content checks. The additions-only HF backup adds the 24 missing
Muon files (4,053,859 bytes), preserves the existing 24 AdamW files, and verifies all 48 files /
8,115,284 bytes at immutable commit `5aba281fd92fa9e6f8336423646cdb17acbfff17`.
See [the archival receipt](reports/experiment-integrity/primary-geometry-eight-run-archive.json).
These are raw weight/displacement features, not the complete 12-run subspace, retrieval-bridge or
functional-dimension analysis. The new NorMuon completion is not part of this eight-run archive.

The initial incremental-geometry upload was rejected before execution over potential non-public
checkpoint disclosure. A read-only anonymous check then verified all 20 underlying Muon weights at
their public original commits, and both existing project repositories as public. Re-review of the
same bounded, additions-only operation allowed the upload. No existing path/card/history was
replaced or deleted; this is separate from the still-unapproved withdrawal of old erroneous results.

The latest main-recovery work is described in
[the exact transition runbook](reports/engineering-archive/main-resume-v1/README.md). The real old
post-training loop repeats completed steps and overwrites logs on resume; its isolated candidate
preserves step/attempt evidence. A separate exact zero-step migration now passes 25 temporary-copy
checks, including both simulated archive/write interruption points and rejection under a held
controller lease. It preserves the original eight-backup ledger byte-for-byte and every prior
record. This is a prepared repair, not a runtime deployment or distributed training-resume result.

The seven-file main target is `4380a363...`, superseding the incomplete six-file proposal
`af646ecf...`. A third successor amendment now requires that exact target; it changes only the main
hash, its amendment binding and timestamp. All 47 commands/arguments and every scientific field
remain unchanged. All five actual consumer loaders and the three-layer reconstruction pass. The
isolated/projected successor identities are `ca0c7f5b...` / `6749baa1...`, neither live `6605090d...`.
The latest full suite passes all **1,124 tests**, with 81 focused checks and no failures, errors,
skips or exclusions. Its actual dry run still reports main incomplete; strict paper release still
rejects the missing real primary/dimension publications and three pending scientific includes.

At the preceding 02:27 UTC observation, training was 8/12 complete, with NorMuon steps 3797/3690, two later runs pending,
48/60 resumable stages and zero fatal markers. The two exact W&B handles were running at 02:13 UTC.
No live source, controller or ledger was changed. The latest migration and successor identities,
not the earlier projections retained below, are the current prepared deployment plan. Runtime
deployment, WIP source publication and the separately rejected HF cleanup remain unperformed.

The owner-requested integrity audit is documented in
[reports/experiment-integrity/README.md](reports/experiment-integrity/README.md). Every one of the
500,000 materialized row identities matches the sampling ledger; the eight completed primary runs
and 40 checkpoints pass deep validation; 89 frozen bindings match; and all 808 files in those eight
runs match their immutable Hugging Face upload commits by content digest. These are partial-study
checks: the complete primary retrieval, dimension and factorial outputs remain unavailable.

The earlier loss-audit full isolated-checkout suite passes all 1,081 tests without exclusions. The live-main suite
passes 883 tests with only the slow historical full-model paper audit deselected; the current
primary checkpoint audit was separately completed. Historical dimension outputs were regenerated
in a new namespace to restore implementation provenance, and their source-bound audit passes.
All pre-existing numerical cells are unchanged; these remain exploratory, not primary evidence.

A separate real-checkpoint CPU continuation audit now passes for AdamW, Muon and NorMuon at step
2345, covering all 134 parameters and optimizer states in each of the three selected models.
After an in-memory serialization between two fixed synthetic-gradient updates, continued weights,
optimizer states and scheduler states match exactly, with the declared learning-rate schedule.
The new diagnostic/reference selection passes 15 focused tests; it does not replace the earlier
1,040-test whole-project run. This is restoration evidence only: no training forward/backward,
data-loader or rank RNG replay, CPU/GPU equivalence, four-GPU resume or optimizer-quality claim.
See `reports/experiment-integrity/checkpoint-cpu-resume.json`. Source checkpoints remain unchanged.

A later [real loss/encoding check](reports/experiment-integrity/dense-loss-contract-real.json)
now exercises actual CPU forward/backward on those three checkpoint-2345 models. The production
collator and trainer feature extraction preserve all nine columns and exact query/document token
prefixes. For two short synthetic queries per checkpoint, the eight-candidate loss and all 134
trainable parameter-gradient tensors match an independent float64 row-wise reference. The actual
MTEB wrapper also reproduces normalized embeddings and cosine scores after forced length-budget
reordering. All source checkpoint inventories are unchanged. This is FP32/eval-mode engineering
coverage, not training-mode dropout, distributed accumulation, GPU BF16/FlashAttention, an 8192-token
forward pass, full BEIR or an optimizer-quality result. Twelve focused tests and all 1,081 full-suite
tests pass with no exclusions. The diagnostic's own initial tensor-comparison error is archived;
no training/evaluation numerical source or tolerance was changed. Full details and commands are in
the integrity-audit README, not the manuscript.

The [independent checkpoint-3126 HF audit](reports/experiment-integrity/normuon-3126-hf-digest-audit.json)
verifies both new NorMuon stages at their original immutable upload commits: all 34 files /
2,702,927,254 bytes match exactly, with stable local payloads and unchanged original receipts.
Remote coverage remains 48/60, not 12-run completion. At 01:44 UTC the artifact snapshot records
8/12 complete, active NorMuon steps 3430/3328 and zero fatal markers; named W&B handles were both
running at 01:39 UTC. HF removal and live deployment remain unperformed pending the prior approvals.

**New evaluation-to-summary blocker:** the real BEIR dry run accepts all 40 checkpoints from the
eight completed primary runs, deeply validates their inputs and formal runtime, and plans 560 task
units without launching an evaluator. Its exact ten-file source manifest is rejected by the live
outcome reader, which recognizes only the historical/general evaluator mappings. The targeted
consumer gate fails with `Unauthenticated evaluation source files`; this would block the frozen
`corrected_outcome_summary -> collect_evaluations` handoff after BEIR. The isolated repair recognizes
the exact corrected mapping and still requires every source's bytes/hash to match reachable Git
history. Actual input rehashing and repaired runtime authentication pass; missing, mixed, unknown
and changed-source cases still fail. The 65 focused tests and 1,052-test full suite pass, followed by
22 checks of the final source bytes. This does not execute the 840-unit summary or establish a
retrieval result. See `reports/experiment-integrity/evaluation-handoff-validation.json`.

This newly identified transitive helper was absent from the frozen explicit source lists. It must
be bound and included in a controlled non-scientific deployment, consistently with the separate
publication-header fix and successor dependency. Do not silently replace it in live main, and do
not treat the existing header-only migration proposal as a complete repair of finalization.
The owner-requested last-run early evaluation also needs a single-owner handoff: the main controller
will request all eight GPUs at 12/12 and does not coordinate its GPU ownership with a manual subset.
The runbook now makes that collision explicit. No early evaluator, live migration or signal was
issued. With two active and two future NorMuon runs, the early-evaluation condition is not met.

The combined six-file main repair is now prepared and rehearsed under
`reports/engineering-archive/main-handoff-v1/`. It binds that helper in the outcome protocol, updates
the three dependent protocol identities, and includes the three-header fix without changing any
scientific field, numerical source, command or argument. Forty focused checks pass, including ten
new cases: all candidate protocol loaders accept the exact dependencies, helper drift is rejected,
and the real migration function on a temporary copy preserves the original ledger byte-for-byte,
all eight backup records and prior migrations. `git apply --check` succeeds against live main.
The combined projected main target is `af646ecf...`, not header-only `6d991052...`. Neither is live.
The isolated successor now requires the combined target under a separate follow-up amendment,
with narrative/dimension/factorial-publication dependencies refreshed consistently. This rehearsal
takes no real controller lease and starts no process. Its original 40 focused checks remain a
separate receipt; the later 1,071-test full suite also includes the successor checks below.

The latest [combined successor projection](reports/experiment-integrity/combined-successor-contract-projection.json)
reconstructs both exact preparation layers, checks seven byte-identical predecessor protocols and
exercises all five actual consumer protocol loaders. All 47 prepared commands and arguments remain
unchanged, including the original 36 factorial steps and eleven dimension additions. The exact
isolated contract is `a64b2658...`; projected onto the original experiment root it is `37a4485c...`.
Both require fully complete main `af646ecf...`, with all 12 runs, all 17 ordered steps and backups.
Neither is deployed. The original `6d991052...` parent and `9b577964...` / `eada94f3...` successor
projections remain historical proposals, not active isolated requirements.

All 56 focused checks, 1,071 full-suite tests and 31 documentation/contract checks pass (zero
failures/errors/skips/exclusions); the exact source-bound record is
[combined-successor-validation.json](reports/experiment-integrity/combined-successor-validation.json).
The actual 47-step dry run reports main incomplete and starts no controller. Strict paper release
still exits 1 for missing real primary/dimension publications and pending scientific includes.
The retained first focused attempt found one stale factorial-publication dimension-amendment
binding; it was refreshed and explicit mutation rejection was added. Rehashed scientific edits,
changed dimension commands, modified source states and weaker or old main gates are rejected.
The scientific renderer and every numerical source are unchanged. At 01:18 UTC live main/factorial
remain `4152531e...` / `6605090d...`, with zero post-training steps, and all 12 checked live source
files match the original main repair archive. W&B's two named NorMuon handles are running at
3210/3120 at 01:19 UTC. No live source, ledger, controller, training job or HF artifact was changed.

The primary dimension exporter is now implemented and source-bound, with a read-only 61-state plan
that correctly reports the four missing NorMuon completions. The analyzer requires its checkpoint-
bound export handoff, and the publisher recomputes source tables, decisions and exact LaTeX during
audit. All 41 focused tests pass, including cache mutations and compilation of synthetic findings.
The automatic handoff and portable publication gate are now implemented and tested: eleven new steps
precede factorial calibration while preserving the original 36-step sequence. A renamed clean clone
can recompute the publication statistics and exact LaTeX from the small evidence closure; corruption
is rejected without falling back to the producer directory. This does not rerun model encoding or
coordinate ablations. The real 47-step dry run reports the main ledger incomplete and starts no
controller. Deployment of the controlled zero-step transition, actual primary execution and remote
analysis archival remain pending; the primary matrix has not yet run through this chain.
See [docs/dimension-utilization.md](docs/dimension-utilization.md) for the exact commands and boundary.

The new archive implementation stages only the source-derived evidence closure plus all 61 raw
vector exports, uses an additions-only content-addressed HF commit, and verifies exact paths, sizes
and digests at that immutable revision. Tests cover interrupted receipt recovery, conflicting
prefixes, source mutation, remote corruption, changing remote heads and independent download checks.
The existing analysis dataset was verified public at revision `54135027f4dec5e48c3822ad43f5f852fdef7d07`;
the new primary namespace is absent. No actual primary vectors or analysis results were uploaded.
The relocation-aware raw-feature replay command is now implemented. A renamed, network-disabled
synthetic archive test recomputes the full feature panel without loading model dependencies, and a
separate 61-state/768D historical recomputation matches every original numerical cell and attribution
array exactly. The rotation control now enforces both score and rank invariance, as the protocol
already required. These checks do not establish primary outcomes or a completed primary archive.
The archive/replay code and 47-step controller remain undeployed.

The new complete-results fixture exercises the actual Makefile with all three rendered scientific
includes. Both the draft and synthetic complete-result papers end their main text on page 6; the
68 plotted dimension means and exact vector coordinates are independently recomputed. The primary
publication now has a read-only exact reconstruction audit; populated or rehashed edited includes
cannot replace source-bound findings. Ordinary paper builds no longer invoke the obsolete historical
headline writer. Final release remains correctly blocked on actual primary outputs.
The source-bound receipt is `reports/experiment-integrity/dimension-figure-validation.json`.

**Live publication blocker:** the complete decoded-source scan finds three malformed table-header
newlines in the current main renderer. The two retained scientific tables are fixed in isolation.
The live syntax-only patch and exact main-contract proposal under `reports/experiment-integrity/`
remain undeployed drafts. Nine additional focused tests now reconstruct the exact two-file patch,
reproduce the original compile failure, compile all three fixed headers on synthetic evidence, and
rehearse the migration on a temporary copy of the real ledger. They preserve its eight backup
records and byte-identical pre-transition archive, and reject undeclared contract drift. The stale
factorial main-contract gate correctly rejects the projected migrated main ledger; only the exact
updated gate accepts it after all completion requirements are met. See
`reports/experiment-integrity/live-publication-header-rehearsal.json`. These nine tests are separate
from the earlier 1,005-test full suite, not a new full-suite run. No real lease or live transition
was exercised. Main cannot be assumed to complete its PDF release unchanged; resolve that narrow
transition and the factorial's exact main dependency before merging the narrative work. Training
and numerical evaluation code remain unchanged.

The prior header-only successor proposal required the repaired main identity under
`configs/dense_no_packing_state_operator_parent_publication_amendment.json`. All main completion
conditions and all original 36 factorial commands remain unchanged. The actual isolated dry run
uses the live `/usr/bin/python3` arguments and yields `9b577964...`; the exact four-path relocation
onto the experiment root yields projected deployment contract `eada94f3...`. Neither is live.
The earlier `3873d6cc...` plan is superseded for deployment, not an executed transition. The
[source-bound validation](reports/experiment-integrity/successor-parent-validation.json) records
1,040 passing full-suite tests, 37 focused checks and 30 documentation checks. Strict paper release
still fails correctly on missing real primary/dimension publications and pending result includes.
The live main/factorial remain `4152531e...` / `6605090d...`, each with zero post-training steps.
The combined follow-up above supersedes these header-only projected identities. Deploy the narrow
combined main repair separately, finish its exact main gate, then perform the approved
zero-step successor handoff and recompute its contract on the actual host. No runtime migration,
source commit or HF mutation was performed.

**HF withdrawal pending approval:** the owner requested removal of invalidated historical artifacts.
The verified plan selects 9,629 files / 366,252,912,201 logical bytes, preserving current checkpoints,
shared data and separately retained LateOn files. Safety review rejected execution pending explicit
scope confirmation. No HF file/card/history or local artifact has been deleted. See
`reports/hf-obsolete-cleanup/README.md`; do not retry or split the rejected deletion to bypass review.
The 23:45 read-only dependency audit finds 52 model LFS objects shared by withdrawn and retained
paths, affecting 225 current checkpoint paths, plus one shared untrained baseline vector object.
All 49 then-existing primary backup receipts' 39 distinct pinned commits are readable and match
the observed heads by file identity. The later second NorMuon checkpoint-2345 receipt is outside
that snapshot. This proves specific preservation dependencies, not safety to purge history;
refresh the audit and establish verified replacement references before any approved rewrite.
The focused dependency/backup regression selection passes 65 tests. No cleanup is executed.

This checkout is the isolated `narrative/weight-space-spine` branch. Its manuscript excludes all
implementation-debugging material and historical exploratory numbers. Its input-integrity and
paper-gate repairs are tested locally but have not been merged into the active experiment checkout.
The changes are not committed or pushed. Owner approval for an explicitly draft WIP branch was
requested because the existing pre-commit rule requires no pending manuscript results; it remains
unanswered. This does not authorize a live merge or relaxation of final publication gates.
The detailed historical operational timeline below is retained as provenance, not current status.
...
| Corrected Dense no-packing replication | First NorMuon pair producing training artifacts | 8/12 complete; NorMuon 1e-4/3e-4 at artifact steps 3430/3328 at 01:44 UTC on September 5; 48/60 resumable checkpoints |
| Corrected completion handoff | Ledger waiting for remaining training | Contract `4152531e...`; 8/8 complete runs uploaded; fresh independent content audit passes for every file |
...
| Sealed-checkpoint durability | Both first-pair NorMuon checkpoint-3126 uploads now independently verified | 48/60 covered; the new pair audit verifies 34 files / 2,702,927,254 bytes against original immutable commits, not full-run completion |
...
| Primary dimension utilization | Export, automatic handoff, publication/plot recomputation, archive and offline feature replay tested; parent dependency corrected in isolation; not deployed or executed on the primary grid | 61 planned states; original 36 factorial steps preserved inside the 47-step plan; full suite 1,040 passed; no primary dimension outcome |
| Corrected W&B provenance | Read-only partial audit at 00:34 UTC and exact active-handle poll at 00:39 UTC | 10/12 visible: 8 finished, 2 running; 0 identity/config/state problems; both active W&B steps advanced by 40 between observations |
| Public checkpoint backup | Historical archive previously audited; primary archive incremental | Fresh content audit: 8 complete primary runs, 808 files, 67,658,898,978 bytes; all remote digests match |
| Public result backup | Historical addendum complete; primary dimension archive implementation tested but awaiting real inputs | 49 historical addendum files, 24,378,651 bytes; primary dimension namespace absent |
| GitHub visibility | Public | default branch plus auditable work branches |
| Historical clean-clone evidence | Inventory complete locally; earlier main release passed CI | 2,785 files, 107,442,256 bytes, SHA-256 verified; the rewritten final paper still fails its pending-result gate |
| Corrected paper renderer | Source-reconstructed publication tested in isolation; live syntax hotfix still required | four upstream manifests, all 9 bridge features, exact paper/report/manifest read-only reconstruction |
...
| Manuscript narrative | Weight-state and dimension-use story on isolated branch; primary findings pending | Draft and full synthetic-result Makefile builds pass with main endpoint page 6; no Type 3 fonts or engineering-error narrative; final release remains gated |
| GitHub main CI | Green | see the repository Actions history for the current merge |

At 23:47 UTC the artifact-only snapshot observes 46 resumable stages and no fatal error markers;
the sealed-backup state also covers 46. The NorMuon 1e-4 checkpoint 2345 was uploaded at 23:36 UTC
in commit `b77ff85f422e638bee7443bc60b4bcc177eeb2fc`; NorMuon 3e-4 followed at 23:46 UTC in
`f7642c34b9866b1de44faa5b43fd1e1881ba6318`. Both original upload receipts pass all file/digest
checks and explicitly retain `scientific_completion=false`. This is intermediate durability,
not two additional completed runs.

The earlier 22:18 artifact-only snapshot observes 44 resumable stages and no fatal error markers. Remote
coverage also contains 44 stages. The new `padded-normuon-1e-4` checkpoint 1563 was sealed at 22:03 UTC
in Hugging Face commit `6c1f11fd22ae9f2199d5bfc9138adfecd77eb5ca`; `padded-normuon-3e-4` followed at
22:14 UTC in `7945be0bfaac13ef8bec88c6ca61e78a8e908f7b`. Both receipts have empty missing, extra,
size-mismatch and digest-mismatch sets and explicitly retain `scientific_completion=false`.
Their earlier checkpoint-782 receipts remain unchanged. An independent header audit also confirms
that all 134 model tensors in each of the 40 completed checkpoints are FP32, consistent with the
declared FP32 parameters and BF16 training autocast; this is not a numerical resume test.
...
The following superseded exploratory observations motivated the primary replication; they do not
provide the rewritten manuscript's conclusions. Best historical final nDCG@10 is 0.589867 for AdamW,
0.592309 for Muon, and
...
not evidence that Muon lacks a useful retrieval regime. An earlier, now retired narrative emphasized
a locally-weaker/globally-better puzzle: same-state norm-matched AdamW gives the larger immediate mean
...
frozen geometry features predicts it out of dose. The active manuscript instead asks how the
optimizers change reached weight states and their functional dimension use; it does not use the
immediate margin proxy to rank optimizer steps.
