...
def audit_materialized_rows(root: Path) -> dict[str, Any]:
    ...
    columns = [
        "sample_id", "source", "query_id", "positive_id",
    ...
    with (root / "rows.jsonl").open(encoding="utf-8") as handle:
        ...
        for row, declared in itertools.zip_longest(dataset, records):
            ...
            invalid_negatives += (
                len(set(negative_ids)) != 7 or row["positive_id"] in negative_ids
            )
            counts[row["source"]] += 1
    manifest = json.loads((root / "manifest.json").read_text(encoding="utf-8"))
    complete = (
        count == 500_000 and not mismatched and not duplicate_ids
        and not invalid_negatives and dict(counts) == manifest["quotas"]
    )
    return {
        "complete": complete, "rows": count, "unique_sample_ids": len(seen),
        "row_identity_mismatches": mismatched, "duplicate_sample_ids": duplicate_ids,
        "invalid_negative_groups": invalid_negatives, "source_counts": dict(counts),
...
def main() -> None:
    ...
    code_paths = [
        "train", "losses", "optimizers", "collators", "config", "data",
        "aggregate", "corrected_progress", "corrected_beir_evaluation",
        "corrected_outcome_summary", "decontamination",
    ]
    payload = {
        "schema_version": 1,
        ...
        "sources": [identity(repository / f"src/embed_optim/{name}.py", repository)
    ...
    print(json.dumps({
        "audited_portion_valid": bool(valid), "completed_runs": len(completed),
        "verified_checkpoints": training["verified_checkpoints"], "output": str(output),
