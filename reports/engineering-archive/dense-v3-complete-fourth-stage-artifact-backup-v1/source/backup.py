"""One-shot data-only backup of all twelve accepted step-3126 evaluations."""
...
TRAJECTORY_SHA = 'dd48e622c9cd1f28f8e93cab670349fc937e8e64aa50b98947104151dbda42c2'
...
PARENT = 'dd16fe4cfe3e21ba8c8b4583038ed93fe4ce6e92'
NAMESPACE = 'corrected-dense-correctness-v3'
ADDITION = NAMESPACE + '/complete-fourth-stage-evaluations'
...
def load_transport():
    ...
    spec = importlib.util.spec_from_file_location('complete_fourth_stage_transport_parent', TRANSPORT)
...
def select_rows(bundle):
    ...
    expected = {(run, step) for run in RUNS for step in (782, 1563, 2345, 3126, 3907)}
    require(len(records) == 60 and {(r['run_id'], r['step']) for r in records} == expected,
            'Require all twelve configurations at all five retained stages')
...
def check_accepted_receipts(verification, readout):
    require(verification['scope'] == 'complete_actual_retrieval_readout_and_host_local_preservation'
            and len(verification['payloads']) == 57
            and verification['checkpoint_count'] == 60
            and verification['task_score_count'] == 840
            and verification['prior_54_checkpoint_records_exactly_unchanged'] is True
            and verification['copied_source_csvs_byte_identical'] == 8
            and verification['native_original_files_reopened_and_matched'] == 2760
            and verification['native_original_worker_exit_zero_count'] == 840
            and verification['observer_primary_scores_exactly_matched'] == 840
            and verification['scalar_dynamics_checks'] == 99
            and verification['all_six_endpoint_contrasts_unchanged'] is True
            and verification['scientific_completion'] is False,
            'Wrong complete actual archive verification')
    require(verification['payloads']['actual/complete-checkpoint-readback.json']['sha256'] == BUNDLE_SHA
            and verification['payloads']['tables/readout.json']['sha256'] == TRAJECTORY_SHA,
            'Accepted numerical readback bindings differ')
    require(readout['scope'] == 'complete_retained_retrieval_trajectory_readout'
            and readout['native_bundle']['sha256'] == BUNDLE_SHA
            and readout['checkpoint_count'] == 60 and readout['task_score_count'] == 840
            and readout['outputs']['all_task_scores.csv']['rows'] == 840
            and readout['outputs']['run_stage_scores.csv']['rows'] == 60
            and readout['prior_54_records_exactly_unchanged'] is True
            and readout['all_six_endpoint_contrasts_unchanged'] is True
            and readout['model_or_retrieval_recomputed'] is False
            and readout['new_inferential_rule'] is False
            and readout['original_whole_grid_consumer_called'] is False
            and readout['original_whole_grid_admission_passed'] is False
            and readout['scientific_completion'] is False,
            'Wrong complete trajectory readout')


def source_inputs(t):
    verification = t.read(ORIGINAL / 'verification.json', VERIFICATION_SHA)
    readout = t.read(ORIGINAL / 'tables/readout.json', TRAJECTORY_SHA)
    check_accepted_receipts(verification, readout)
    for name, item in verification['payloads'].items():
        t.compare_file(ORIGINAL / t.safe_name(name), item)
    ...
    for row in rows:
        for item in [row['original_complete_receipt'], row['original_operational_receipt'],
            ...
            add('native/beir-fourth-stage', EXPERIMENT / 'evaluations/dense-primary-v3', item)
...
def derived_bytes(bundle, rows):
    ...
    acceptance = {
        'scope': 'accepted-complete-step-3126-beir-data-only', 'primary_protocol_sha256': PROTOCOL,
        ...
        'all_twelve_fourth_stage_configurations_complete': True,
        'accepted_complete_trajectory_readout_sha256': TRAJECTORY_SHA,
...
def prepare(t, work):
    ...
    manifest = {
        'schema_version': 1, 'scope': 'complete-primary-v3-fourth-stage-beir-artifact-backup',
        'created_at_utc': t.stamp(), 'primary_protocol_sha256': PROTOCOL,
        'accepted_bundle_sha256': BUNDLE_SHA, 'accepted_verification_sha256': VERIFICATION_SHA,
        'runs': list(RUNS), 'checkpoint_stages': [3126], 'beir_task_cells': 168,
        'cohort_selection': 'all_declared_optimizer_rate_configurations',
        'all_twelve_fourth_stage_configurations_complete': True,
        'accepted_complete_trajectory_readout_sha256': TRAJECTORY_SHA,
...
def prepared(t, work):
    ...
    require(set(manifest['files']) == set(selected) | GENERATED
            ...
            and manifest['checkpoint_stages'] == [3126] and manifest['beir_task_cells'] == 168
            and manifest['cohort_selection'] == 'all_declared_optimizer_rate_configurations'
            and manifest['all_twelve_fourth_stage_configurations_complete'] is True
            and manifest['accepted_complete_trajectory_readout_sha256'] == TRAJECTORY_SHA,
...
def upload(t, work):
    ...
    require(ADDITION not in old_sub, 'Existing complete-fourth-stage subtree: inspect, do not overwrite')
    ...
    commit = api.create_commit(REPO, repo_type='dataset', parent_commit=PARENT, operations=operations,
        token=True, num_threads=2, commit_message='Preserve all twelve corrected DenseOn step-3126 BEIR evaluations (data only)')
...
def download_verify(t, work):
    ...
    result = {'scope': 'actual-complete-anonymous-fourth-stage-beir-recovery',
...
def main():
    ...
    require(args.workdir.parent == Path('/tmp')
            and args.workdir.name.startswith('dense-v3-complete-fourth-stage-artifact-backup.')
