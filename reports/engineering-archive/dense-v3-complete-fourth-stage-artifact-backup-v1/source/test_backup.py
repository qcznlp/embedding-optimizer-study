...
SPEC = importlib.util.spec_from_file_location('complete_fourth_stage_backup_under_test', Path(__file__).with_name('backup.py'))
...
def test_actual_complete_fourth_stage_selection_and_safety(actual):
    t, bundle, rows, selected = actual
    assert len(rows) == 12 and [r['run_id'] for r in rows] == list(B.RUNS)
    assert len(selected) == 553
    assert sum(n.startswith('native/') for n in selected) == 216
    assert sum(n.startswith('provenance/') for n in selected) == 336
...
def test_incomplete_or_wrong_cohort_refused(actual, mutation):
    ...
    row = next(r for r in bundle['checkpoints'] if r['step'] == 3126)
    if mutation == 'missing':
...
def test_actual_data_tables_are_complete(actual):
    ...
    assert len(means) == 12 and len(tasks) == 168
...
def modes():
    names = [f'data/{i}.json' for i in range(557)]
...
def preservation():
    ...
    old = {B.NAMESPACE + '/' + n: {'kind': 'RepoFolder', 'tree_id': n}
           for n in ('complete-endpoint-evaluations', 'endpoint-statistics', 'training-dynamics', 'weight-space', 'complete-first-stage-evaluations', 'partial-second-stage-pool-b-evaluations', 'complete-second-stage-evaluations', 'partial-fourth-stage-pool-b-evaluations')}
...
@pytest.mark.parametrize('target,field,value', [
    ('verification', 'checkpoint_count', 54),
    ('verification', 'task_score_count', 756),
    ('verification', 'prior_54_checkpoint_records_exactly_unchanged', False),
    ('verification', 'copied_source_csvs_byte_identical', 7),
    ('verification', 'native_original_files_reopened_and_matched', 2759),
    ('verification', 'native_original_worker_exit_zero_count', 839),
    ('verification', 'observer_primary_scores_exactly_matched', 839),
    ('verification', 'payload_count', None),
    ('verification', 'payload_digest', '0' * 64),
    ('readout', 'checkpoint_count', 54),
    ('readout', 'task_score_count', 756),
    ('readout', 'prior_54_records_exactly_unchanged', False),
    ('readout', 'all_six_endpoint_contrasts_unchanged', False),
    ('readout', 'original_whole_grid_consumer_called', True),
    ('readout', 'original_whole_grid_admission_passed', True),
    ('readout', 'model_or_retrieval_recomputed', True),
])
def test_new_archive_schema_and_complete_population_refused(actual, target, field, value):
    transport = actual[0]
    verification = transport.read(B.ORIGINAL / 'verification.json', B.VERIFICATION_SHA)
    readout = transport.read(B.ORIGINAL / 'tables/readout.json', B.TRAJECTORY_SHA)
    changed = verification if target == 'verification' else readout
    if field == 'payload_count':
        changed['payloads'].pop('README.md')
    elif field == 'payload_digest':
        changed['payloads']['actual/complete-checkpoint-readback.json']['sha256'] = value
    else:
        changed[field] = value
    with pytest.raises(ValueError):
        B.check_accepted_receipts(verification, readout)
