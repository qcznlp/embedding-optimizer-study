"""Independently replay a supplied complete-fourth-stage data snapshot using only Python stdlib.
...
def verify(root, manifest_sha):
    ...
    require(manifest['scope'] == 'complete-primary-v3-fourth-stage-beir-artifact-backup'
    ...
    def mapped_snapshot(item):
        ...
        name = 'native/beir-fourth-stage/' + relative
    ...
    require(acceptance['accepted_bundle_sha256'] == manifest['accepted_bundle_sha256']
            ...
            and acceptance['step'] == 3126 and acceptance['task_cells'] == 168
            and acceptance['cohort_selection'] == 'all_declared_optimizer_rate_configurations'
            and acceptance['all_twelve_fourth_stage_configurations_complete'] is True
            and acceptance['accepted_complete_trajectory_readout_sha256'] == manifest['accepted_complete_trajectory_readout_sha256']
    ...
    native = {name for name in expected if name.startswith('native/beir-fourth-stage/')}
    ...
    for name in complete_names:
        ...
        require(name == 'native/beir-fourth-stage/' + safe_relative(plan['cache_key'])
    ...
    return {'scope': 'independent-supplied-complete-fourth-stage-snapshot-byte-and-score-reconstruction',
            ...
            'fourth_stage_means_0_to_100': {run: float(means[run] * 100) for run in RUNS},
