...
continuation remains zero of twelve. The new six outcomes and full-trajectory
tables/figures still need immutable data-only backup. Manuscript/source release
...
1. Back up the last six pool-A step-3126 outcomes and the complete trajectory
   tables/figures as new immutable data-only additions, then anonymously recover
   and reconstruct them. The other 54 outcome backups, all 60 model checkpoints,
   completed validation/selection and all prior snapshots remain unchanged.
