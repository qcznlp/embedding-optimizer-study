...
def saved_pair(name):
    ...
    return model, config, optimizer, scheduler, copy.deepcopy(optimizer.state_dict()), copy.deepcopy(scheduler.state_dict())
...
def test_fresh_actual_factory_observes_same_named_matrices_and_empty_state(name):
    ...
    assert [g["param_names"] for g in optimizer.param_groups] == [
        ["layers.0.weight", "layers.1.weight"], ["embeddings.weight"], ["norm.weight", "norm.bias"]
    ]
    assert optimizer.state_dict()["state"] == {}
    assert optimizer.completed_steps == 0
    assert optimizer.partition_summary["hidden"]["tensors"] == 2
    assert f.inspect_state(optimizer.state_dict(), optimizer.named_parameter_layout, config, step=0)["named_routing"]
...
@pytest.mark.parametrize("field,value", [
    ("lr", 0), ("lr", float("nan")), ("lr", True), ("aux_lr", 1e-5),
    ("weight_decay", 0.0), ("beta1", 0.8), ("beta2", 0.9), ("eps", 1e-6),
    ("aux_beta1", 0.8), ("aux_beta2", 0.9), ("aux_eps", 1e-6),
    ("momentum", 0.9), ("ns_steps", 4), ("adjust_lr_fn", "match_rms_adamw"),
])
...
@pytest.mark.parametrize("change", ["alias", "frozen", "dtype", "names", "shape", "gradient", "missing_group"])
...
def test_saved_resume_uses_actual_torch_payload_and_matches_uninterrupted_updates(name):
    ...
    buffer = io.BytesIO()
    torch.save({"model": model.state_dict(), "optimizer": saved_opt, "scheduler": saved_sched}, buffer)
    buffer.seek(0)
    ...
    receipt = f.restore_training_state(restored, restored_scheduler, saved["optimizer"], saved["scheduler"], step=4, horizon=10)
    ...
    f.inspect_scheduler(restored_scheduler.state_dict(), restored.state_dict(), config, step=10, horizon=10)
...
@pytest.mark.parametrize("change", [
    "names", "ids", "layout", "missing_name", "missing_state", "extra_state", "nonfinite",
    "negative_second", "shape", "dtype", "counter", "recipe", "metadata_step", "schema_bool",
    "group_setting", "scheduler_counter", "scheduler_lr", "scheduler_horizon", "untrusted_step",
])
def test_invalid_restore_refuses_before_mutating_either_fresh_object(name, change):
    ...
    if change == "names":
        ...
        expected_step = 5
    pristine_opt, pristine_sched = copy.deepcopy(target.state_dict()), copy.deepcopy(scheduler.state_dict())
    with pytest.raises(ValueError):
        f.restore_training_state(target, scheduler, saved_opt, saved_sched, step=expected_step, horizon=horizon)
...
def reference_muon(gradient, momentum):
    # Independent literal calculation, not a production update/partition helper.
    ...
    for _ in range(5):
        ...
        update = 3.4445 * update + (-4.7750 * gram + 2.0315 * (gram @ gram)) @ update
...
def test_muon_updates_match_independent_polynomial_and_aspect_scaling(shape):
    model = Tiny()
    model.layers[0].weight = nn.Parameter(torch.arange(math.prod(shape)).reshape(shape).float() / 17)
    ...
    for step in range(8):
        ...
        optimizer.step()
        # BF16 polynomial rounding can differ after the independent FP32
        # momentum expression; report the bounded numerical scope explicitly.
        torch.testing.assert_close(model.layers[0].weight, reference, rtol=0, atol=2e-5)
