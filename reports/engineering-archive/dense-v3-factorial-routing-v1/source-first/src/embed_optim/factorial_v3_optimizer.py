...
def configuration(config):
    ...
    actual = config.as_dict()
    expected = OptimizerConfig(name=config.name, lr=config.lr).as_dict()
...
def inspect_state(payload, layout, config, *, step):
    ...
    if (
        any(type(pid) is not int for pid in states)
        or (states if step == 0 else set(states) != set(expected_ids))
    ):
...
class FactorialOptimizer(EmbeddingOptimizer):
    ...
    def __init__(self, model, config, *, expected_shapes=None):
        ...
        configuration(config)
        self._layout = named_layout(model, denseon_shapes() if expected_shapes is None else expected_shapes)
...
def require_scheduler_function(scheduler, optimizer, horizon):
    ...
    _positive_integer(horizon)
    if type(scheduler) is not torch.optim.lr_scheduler.LambdaLR or scheduler.optimizer is not optimizer:
...
def restore_training_state(optimizer, scheduler, saved_optimizer, saved_scheduler, *, step, horizon=HORIZON):
    ...
    inspect_scheduler(scheduler.state_dict(), optimizer.state_dict(), optimizer._configuration, step=0, horizon=horizon)
    inspect_state(saved_optimizer, optimizer.named_parameter_layout, optimizer._configuration, step=step)
    inspect_scheduler(saved_scheduler, saved_optimizer, optimizer._configuration, step=step, horizon=horizon)
    optimizer.prepare_resume(step)
    optimizer.load_state_dict(saved_optimizer)
    scheduler.load_state_dict(saved_scheduler)
    require_scheduler_function(scheduler, optimizer, horizon)
    return inspect_scheduler(scheduler.state_dict(), optimizer.state_dict(), optimizer._configuration, step=step, horizon=horizon)
