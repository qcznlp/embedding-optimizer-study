"""Full-identity integration audit of actual four-GPU run_training save/resume.
...
def check_completion(config, resume_step):
    ...
    full_identity = run_contract.read_identity(root)
    if payload["run_identity_sha256"] != run_contract.digest(full_identity):
        raise ValueError("Completion does not bind the actual full run identity")
    run_contract.require_equal(run_contract.read_identity(root / "final"), full_identity)
    run_contract.require_equal(full_identity["recipe"], run_contract.recipe_identity(config))
    checkpoint_files = {}
    for step in range(resume_step + 1, 4):
        checkpoint = root / f"checkpoint-{step}"
        numerical.require_resume_receipt(checkpoint, contract)
        run_contract.require_checkpoint(checkpoint, full_identity)
    ...
    return {
        "full_run_identity": identity(root / run_contract.RECEIPT_NAME),
        "full_identity_sha256": run_contract.digest(full_identity),
        "all_checkpoint_payload_seals_passed": True,
...
def main():
    ...
    if (
        ...
        or not work.name.startswith("dense-identity-entry.")
        or not work.is_dir()
        or work.is_symlink()
    ):
    ...
    report = {
        "scope": "engineering_full_identity_actual_run_training_entrypoint",
        ...
        "handoff": handoff,
        "records": [],
        "boundary": "Actual unpatched prepared run_training including dataset loading/column selection, model/loss/collator construction, distributed output setup, optimizer, callbacks, checkpoint contracts, native restore, final save and completed.json. Three algorithms each run a three-step synthetic epoch and continue from step 2 with byte-identical relocated data and initial model in a separate output namespace. Actual full run identities and all-rank checkpoint payload seals are checked before native model/optimizer resume. Default DDP communication; no bitwise trajectory equivalence claim, CLI/frozen-protocol bypass, formal data run, independent restart or scientific result. W&B disabled; short fixture only.",
    }
    try:
        ...
        dist.barrier()
        relocated_data = work / "relocated-data"
        relocated_base = work / "relocated-base"
        if rank == 0:
            shutil.copytree(dataset_path, relocated_data)
            shutil.copytree(before["checkpoint_root"], relocated_base)
        dist.barrier()
        configs = {
        ...
        for run_id in RUNS:
            ...
            continuation = replace(
                baseline,
                output_root=str(work / "continuation"),
                dataset_path=str(relocated_data),
                model_name=str(relocated_base),
            )
            source_files = None
