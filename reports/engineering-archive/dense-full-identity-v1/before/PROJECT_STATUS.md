...
Last updated: **2026-09-06 10:29 UTC**; the machine-readable snapshot is refreshed at this handoff.

## Where the project stands

**The explicit correction passes integrated diagnostics; the formal matrix is still on scientific hold.**
...
The [new predeployment checks](reports/engineering-archive/dense-predeployment-checks-v1/README.md)
now also pass independent-process replay of this exact new candidate, without another baseline.
They confirm a separate missing full-run identity gate: changed recipe/data/runtime inputs pass
the current numerical-only preload check. A tested complete-identity comparator is prepared but
not integrated into the actual launcher/Trainer. All are engineering evidence, not retrieval
results or validation of the old matrix.
...
| Candidate optimizer conformance | NorMuon 96/96 exact updates on CPU and separately on GPU; Muon 96/96 unchanged per device | Model-quality conclusions or universal numerical equivalence |
| Candidate GPU gradients | 9/9 short-input and 9/9 maximum-length raw/clipped checks, all 134 parameter tensors | Every natural-text batch or full training horizon |
| Candidate checkpoint replay | 24/24 exact same-group and 24/24 exact new-process-group checks with the explicit fixed-layout diagnostic control | Default DDP bitwise equivalence, another physical host or a natural-data full horizon |
| Candidate real entrypoint | 3 baseline + 3 continuation calls, all algorithms; final export matches checkpoint | A full primary run or frozen CLI/protocol acceptance |
| Resume identity admission | 30 changed recipe/data/runtime cases pass the numerical-only preload gate; 6 negative controls correctly reject | A complete run/checkpoint identity contract or proof of historical configuration drift |
| Full-identity proposal | 42/42 comparisons: 6 original/byte-identical relocation positives, 36 changed-identity negatives | Integration into actual persistence/admission or formal readiness |
| Candidate regression suite | 923 cases: 912 pass, **11 source-contract failures**, no exclusions | Whole-repository acceptance; those gates remain unwaived |
| Isolated paper/audit suite | 1,415 cases, zero failures/errors/skips | Correctness of the different candidate source tree |
...
The new candidate's own independent-group replay finished at 09:55:29 UTC; it no longer relies
on the predecessor control's source identity. Its launcher exited zero and all 24 full
...
That numerical contract does not bind the whole run. A CPU-only probe authenticates nine actual
...
The prepared identity comparator binds the full supplied scientific recipe, observed data-file
digests and row/column identity, effective training arguments, numerical policy and source identity.
It permits byte-identical data/output relocation while rejecting changed contents or settings.
All 42 comparison inputs are archived and independently recomputed, not only reported as pass
flags. This is still a proposal: complete identity must be derived from actual inputs and sealed
at run creation, bound to every checkpoint, and enforced before resume side effects. It has not
been retrofitted into old checkpoints or integrated into either training checkout.
...
Prepared numerical candidate: `/tmp/dense-correction-source.0YHywN`, detached at the same base.
Its complete source copies and exact selected hashes are in the new archive. Do not confuse its
923-case suite with the isolated checkout's 1,415-case suite.
...
1. Review the [sealed candidate and transition proposal](reports/engineering-archive/dense-correction-candidate-v1/replication-transition-proposal.md).
   Its numerical integration and independent-process controlled replay are complete. Do not redo
   the resolved origin/replay controls. Integrate the now-tested complete recipe/data/runtime/source
   identity into actual run creation and checkpoint save/resume, with authenticated inputs and
   failure before any resume side effect. Follow the [new admission evidence](reports/engineering-archive/dense-predeployment-checks-v1/README.md);
   do not silently adopt the diagnostic communication strategy as a speed improvement.
2. Prepare new source-bound primary/downstream protocols and an evidence-preserving runtime handoff.
...
- [Current predeployment checks and admission counterexamples](reports/engineering-archive/dense-predeployment-checks-v1/README.md).
...
[the pre-update archive](reports/engineering-archive/dense-predeployment-checks-v1/before/PROJECT_STATUS.md).
