...
A separate admission probe confirms **30 changed recipe/data/runtime inputs pass the numerical-only
preload gate**, while six optimizer/accumulation negative controls reject. No changed configuration
was trained. The proposed complete identity passes **42/42 comparisons**, including byte-identical
data relocation, but is **not yet integrated** into actual run/checkpoint persistence and admission.

Keep the test results separate: the candidate's full suite has **923 cases / 11 unwaived old
source-contract failures**; the isolated paper/audit checkout passes **all 1,415 cases**.
...
integrate the complete run identity, verify its real save/resume path, and resolve the reviewed
deployment/publication gates. Do not resume
