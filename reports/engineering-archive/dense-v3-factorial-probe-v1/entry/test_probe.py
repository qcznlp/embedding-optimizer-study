...
class Controls(unittest.TestCase):
    def test_actual_source_namespace_and_probe_loading(self):
        c = entry.context()
        self.assertEqual(len(c['dataset']), 224)
        self.assertEqual(len(c['identities']), 224)
        self.assertGreaterEqual(len(c['numerical_sources']), 43)
        entry.check_imports(c['numerical_sources'])
...
if __name__ == '__main__':
    ...
    entry.write(HERE / 'tests-second.json', record)
