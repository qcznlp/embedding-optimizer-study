...
def context():
    ...
    numerical = {}
    for path, binding in read(REPORT / 'actual/readout.json')['source_bindings'].items():
        if path.startswith(str(STORY / 'src') + '/'):
            copied = SOURCE / Path(path).relative_to(STORY)
            numerical[str(copied)] = bound(copied, binding)
    need(len(numerical) == 42, 'Original copied source closure differs')
...
def prepare(args):
    ...
    auth = dict(scope=SCOPE, created_at_utc=now(), owner_message=OWNER, automatic_continuation=False,
        source=identity(__file__), tests=identity(HERE / 'tests.json'), test_source=identity(HERE / 'test_probe.py'),
