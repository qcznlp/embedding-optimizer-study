...
def _require_rotation_invariance(original: np.ndarray, rotated: np.ndarray, state: str) -> None:
    if original.shape != rotated.shape or not (
        np.isfinite(original).all() and np.isfinite(rotated).all()
    ):
        raise ValueError(f"Rotation score shape or finiteness differs for {state}")
    difference = float(np.max(np.abs(original - rotated)))
    if difference > 1e-6:
        raise ValueError(f"Rotation changed full scores by {difference} for {state}")
    if not np.array_equal(_query_metrics(original)[2], _query_metrics(rotated)[2]):
        raise ValueError(f"Rotation changed positive ranks for {state}")
...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return _compute_features(args, protocol, paths, expected_checkpoints, primary_handoff)
...
def _compute_features(
    ...
    primary_handoff: dict[str, Any] | None,
) -> dict[str, Any]:
    """Shared numerical kernel; authoring and replay keep distinct upstream gates/scopes."""
    roles = {
        "historical": "exploratory_method_development",
        "corrected": "prospective_primary_dimension_analysis",
        "reconstruction": "verified_archive_feature_reconstruction",
    }
    if args.analysis_scope not in roles:
        raise ValueError("Unknown dimension-computation scope")
    ...
    if not args.skip_rotation:
        for seed in protocol["rotation_control"]["seeds"]:
            ...
            for meta, groups, queries, documents in final_payloads:
                ...
                _require_rotation_invariance(original_scores, rotated_scores, str(meta["run_id"]))
    ...
    manifest = {
        ...
        "analysis_role": roles[args.analysis_scope],
    ...
    if args.analysis_scope == "reconstruction":
