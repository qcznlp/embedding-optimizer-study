...
def verify(root, manifest_sha256):
    ...
    for name, ident in manifest['files'].items():
        require(set(ident) == {'bytes','sha256','git_blob_sha1'}, 'Wrong identity schema')
        bound(root / name, ident)
    for name, (size, sha) in ORIGINALS.items():
        require({k:manifest['files'][name][k] for k in ('bytes','sha256')}
                == {'bytes':size,'sha256':sha}, 'Original anchor changed')
