...
SPEC = importlib.util.spec_from_file_location('fourth_stage_pool_b_backup_under_test', Path(__file__).with_name('backup.py'))
...
def test_actual_complete_fourth_stage_pool_b_selection_and_safety(actual):
...
@pytest.mark.parametrize('mutation', ['archive_count', 'archive_new', 'archive_raw',
    'archive_new_raw', 'archive_previous_changed', 'archive_scientific',
    'archive_bundle_binding', 'independent_count', 'independent_raw',
    'independent_workers', 'independent_previous_changed', 'independent_full_cohort'])
def test_new_archive_semantics_refused_after_mock_authenticated_read(actual, mutation):
    """Semantic refusal only: the proxy deliberately bypasses genuine hash authentication."""
    transport = actual[0]
    ...
    class Proxy:
        ...
        def read(self, path, *args, **kwargs):
            value = transport.read(path, *args, **kwargs)
            target = 'verification.json' if mutation.startswith('archive_') else 'independent-raw-reconstruction.json'
            if Path(path).name != target:
                return value
            value = copy.deepcopy(value)
            if mutation == 'archive_bundle_binding':
                value['payloads']['actual/complete-checkpoint-readback.json']['sha256'] = '0' * 64
            else:
                field, wrong = {
                    'archive_count': ('complete_checkpoint_outcomes', 53),
                    'archive_new': ('new_checkpoint_outcomes', 5),
                    'archive_raw': ('raw_task_values', 755),
                    'archive_new_raw': ('new_raw_task_values', 83),
                    'archive_previous_changed': ('previous_forty_eight_records_exactly_unchanged', False),
                    'archive_scientific': ('scientific_completion', True),
                    'independent_count': ('complete_checkpoint_records', 53),
                    'independent_raw': ('raw_task_scores_checked', 755),
                    'independent_workers': ('native_exit_zero_workers_checked', 755),
                    'independent_previous_changed': ('previous_forty_eight_records_exactly_unchanged', False),
                    'independent_full_cohort': ('all_twelve_fourth_stage_configurations_complete', True),
                }[mutation]
                value[field] = wrong
            return value

    with pytest.raises(ValueError):
        B.source_inputs(Proxy())
...
def preservation():
    ...
    old = {B.NAMESPACE + '/' + n: {'kind': 'RepoFolder', 'tree_id': n}
           for n in ('complete-endpoint-evaluations', 'endpoint-statistics', 'training-dynamics', 'weight-space', 'complete-first-stage-evaluations', 'partial-second-stage-pool-b-evaluations', 'complete-second-stage-evaluations', 'partial-third-stage-pool-b-evaluations', 'complete-third-stage-evaluations', 'pretrained-functional-vectors', 'training-completion-evidence')}
