"""Independently replay a supplied fourth-stage-pool-b data snapshot using only Python stdlib.
...
def verify(root, manifest_sha):
    ...
    require(manifest['scope'] == 'partial-primary-v3-fourth-stage-pool-b-beir-artifact-backup'
            and manifest['primary_protocol_sha256'] == PROTOCOL
            and manifest['runs'] == list(RUNS) and manifest['checkpoint_stages'] == [3126]
    ...
    def mapped_snapshot(item):
        ...
        name = 'native/beir-fourth-stage-pool-b/' + relative
    ...
    require(acceptance['accepted_bundle_sha256'] == manifest['accepted_bundle_sha256']
            ...
            and acceptance['step'] == 3126 and acceptance['task_cells'] == 84
            and acceptance['cohort_selection'] == 'original_pool_b_queue_not_scores'
            and acceptance['all_twelve_fourth_stage_configurations_complete'] is False
    ...
    native = {name for name in expected if name.startswith('native/beir-fourth-stage-pool-b/')}
    ...
    for name in complete_names:
        ...
        require(run in RUNS and run not in seen_runs and plan['checkpoint']['step'] == 3126
        ...
        require(name == 'native/beir-fourth-stage-pool-b/' + safe_relative(plan['cache_key'])
        ...
        for task_record in complete['tasks']:
            ...
            worker_task = task
            suffix = '/' + run + '-3126-' + worker_task + '.started.json'
            ...
            require(started['job'] == exited['job'] == [run, 3126, worker_task]
            ...
            require(command[command.index('--worker') + 1] == worker_task
                    and command[command.index('--models') + 1].endswith('/' + run + '/checkpoint-3126')
                    and command[command.index('--results_folder') + 1]
                    == plan['results_root'] + '/dense/' + run + '__checkpoint-3126', 'Worker command differs')
    ...
    for row in mean_rows + task_rows:
        ...
        require(row['optimizer'] == opt and row['learning_rate'] == rate and row['step'] == '3126',
    ...
    return {'scope': 'independent-supplied-fourth-stage-pool-b-snapshot-byte-and-score-reconstruction',
            ...
            'fourth_stage_pool_b_means_0_to_100': {run: float(means[run] * 100) for run in RUNS},
