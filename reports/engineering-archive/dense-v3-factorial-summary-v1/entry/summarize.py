...
def prepare(args):
    ...
    tests = e.read(HERE / 'tests-second.json', args.tests_sha256)
    ...
    value = dict(scope=SCOPE, created_at_utc=e.now(), owner_message=OWNER, automatic_continuation=False,
        source=e.identity(__file__), collector_source=e.identity(HERE / 'collect.py'), tests=e.identity(HERE / 'tests-second.json'),
...
def authenticate(args):
    ...
    for name, key in (('collect.py', 'collector_source'), ('tests-second.json', 'tests'), ('test_summary.py', 'test_source')):
