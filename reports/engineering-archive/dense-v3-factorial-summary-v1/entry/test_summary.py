...
class Controls(unittest.TestCase):
    ...
    def test_real_ready_path_is_pending_not_failed(self):
        with tempfile.TemporaryDirectory(prefix='factorial-summary-real-wait.') as tmp:
            with patch.object(entry.c, 'EVAL_ROOT', Path(tmp) / 'beir'), patch.object(entry.c, 'PROBE_ROOT', Path(tmp) / 'probe'):
...
if __name__ == '__main__':
    ...
    entry.e.write(HERE / 'tests-second.json', record)
