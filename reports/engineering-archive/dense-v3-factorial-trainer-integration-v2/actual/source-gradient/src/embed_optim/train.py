...
class OptimizerTrainer(SentenceTransformerTrainer):
    def __init__(
        ...
        *args,
        optimizer_config,
        numerical_policy=LEGACY_NUMERICAL_POLICY,
        run_identity=None,
        **kwargs,
    ) -> None:
        ...
        self.dense_run_identity = (
            json.loads(run_contract.canonical(run_identity)) if run_identity is not None else None
        )
    ...
    def _require_run_identity(self):
        if self.dense_run_identity is None:
            raise ValueError("Corrected checkpoint persistence requires complete run admission")
        expected = self.dense_run_identity
        run_contract.require_equal(expected["numerical_contract"], self._numerical_receipt())
        run_contract.require_equal(expected["source"], run_contract.source_identity())
        run_contract.require_arguments(self.args, expected["execution"])
        return expected

    def _save_checkpoint(self, model, trial):
        corrected = self.dense_numerical_policy == DENSE_NUMERICAL_POLICY
        if corrected:
            identity = self._require_run_identity()
            ...
            if output.exists() or output.is_symlink():
                raise ValueError("Checkpoint save must not overwrite existing evidence")
            if torch.distributed.is_initialized():
                torch.distributed.barrier()
        super()._save_checkpoint(model, trial)
        # Every rank writes its RNG payload. Seal only after all those writes finish.
        if corrected and torch.distributed.is_initialized():
            torch.distributed.barrier()
        if corrected and self.args.should_save:
            ...
            with (output / numerical.RECEIPT_NAME).open("x") as handle:
                ...
                handle.write("\n")
            run_contract.seal_checkpoint(output, identity, self.state.global_step)
        if corrected and torch.distributed.is_initialized():
            torch.distributed.barrier()

    def _load_from_checkpoint(self, resume_from_checkpoint):
        if self.dense_numerical_policy == DENSE_NUMERICAL_POLICY:
            run_contract.require_checkpoint(resume_from_checkpoint, self._require_run_identity())
        return super()._load_from_checkpoint(resume_from_checkpoint)

    def _load_optimizer_and_scheduler(self, checkpoint):
        corrected = self.dense_numerical_policy == DENSE_NUMERICAL_POLICY
        if corrected and checkpoint is not None:
            run_contract.require_checkpoint(checkpoint, self._require_run_identity())
...
def _training_argument_values(config: RunConfig) -> dict:
...
def _training_arguments(config: RunConfig) -> SentenceTransformerTrainingArguments:
    return SentenceTransformerTrainingArguments(**_training_argument_values(config))


def run_training(config: RunConfig, resume_from_checkpoint: str | None = None) -> Path:
    arguments = None
    admitted_identity, dataset = None, None
    if config.model_family == "dense":
        ...
        admitted_identity, dataset = run_contract.prepare(
            config, _training_argument_values(config), _world_size(), resume_from_checkpoint
        )
        ...
        run_contract.require_arguments(arguments, admitted_identity["execution"])
        if arguments.world_size > 1:
            if not torch.distributed.is_initialized():
                raise ValueError("Distributed output setup requires the initialized process group")
            identities = [None] * arguments.world_size
            torch.distributed.all_gather_object(identities, run_contract.digest(admitted_identity))
            if len(set(identities)) != 1:
                raise ValueError("Ranks disagree on admitted run/data/source identity")
            torch.distributed.barrier()
    ...
    if int(os.environ.get("RANK", "0")) == 0:
        ...
        if admitted_identity is not None:
            run_contract.write_identity(output_dir, admitted_identity)
    ...
    dataset = Dataset.load_from_disk(str(dataset_path)) if dataset is None else dataset
    ...
    if (
        ...
        and not (output_dir / "dataset_manifest.json").exists()
    ):
    ...
    trainer = OptimizerTrainer(
        ...
        run_identity=admitted_identity,
    )
    ...
    if trainer.is_world_process_zero():
        ...
        if config.numerical_policy == DENSE_NUMERICAL_POLICY:
            ...
            identity = trainer._require_run_identity()
            run_contract.write_identity(final_dir, identity)
            completion_payload["run_identity_sha256"] = run_contract.digest(identity)
