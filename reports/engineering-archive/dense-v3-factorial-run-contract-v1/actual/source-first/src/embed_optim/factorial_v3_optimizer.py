...
class FactorialOptimizer(EmbeddingOptimizer):
    ...
    def load_state_dict(self, state_dict):
        if (
            self._resume_step is None
            and self._steps == 0
            and not self.state
            and isinstance(state_dict, dict)
            and state_dict.get("state") == {}
        ):
            # Accelerate performs a state_dict/device-placement/load round trip
            # even for a fresh optimizer. With no tensors to move this may only
            # be an exact no-op: it cannot change routing, rates, settings or
            # counters, and never admits a checkpoint or calibration state.
            self._require_live_layout()
            inspect_state(state_dict, self._layout, self._configuration, step=0)
            if state_dict != self.state_dict():
                raise ValueError("Pristine optimizer placement must be an exact no-op")
            return
