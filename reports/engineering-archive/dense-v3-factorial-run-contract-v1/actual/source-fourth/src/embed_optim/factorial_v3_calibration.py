...
def _runtime(locations):
    from .runtime import verify_runtime_spec

    path = Path(locations.repository) / "configs/formal_runtime.json"
    if (
        file_identity(path)["sha256"]
        != "b90de16b3796bb8e9d0badcedec688da55a037d46244a0d2a7f5838795cfe77a"
    ):
        raise ValueError("Calibration runtime specification differs")
    return verify_runtime_spec(path)
...
def _verify_loaded_weights(model, checkpoint):
    ...
    with safe_open(
        str(Path(checkpoint) / "model.safetensors"), framework="pt", device="cpu"
    ) as store:
...
def export_gradients(locations, state, output):
    ...
    def load(family, checkpoint, *, dtype, device, flash_attention):
        if (
            family != "dense"
            or str(checkpoint) != request["source"]["checkpoint"]
            or dtype != torch.float32
            or device != "cuda:0"
            or flash_attention is not True
        ):
    ...
    try:
        manifest = gradient_probe.export_gradient_probe(
            request["source"]["checkpoint"],
            request["probe"],
            output / "gradients",
            family="dense",
            probe_spec=request["probe_spec"]["path"],
...
def calibrated_rates(rows, expected_shapes):
    ...
    if (
        not expected_shapes
        or len(rows) != len(expected_shapes)
    ...
    for row in rows:
        if (
            row["shape"] != expected_shapes[row["tensor"]]
            or type(row["gradient_steps"]) is not int
            or row["gradient_steps"] != 8
    ...
    ratios = {
        op: math.sqrt(
            sum(float(r["algorithms"][op]["frobenius_norm"]) ** 2 for r in rows) / weight_sq
        )
    ...
    return {
        ...
        "global_direction_to_weight": ratios,
        "hidden_learning_rates": rates,
        "weight_decay_included": False,
        "fixed_probe_not_first_training_batch": True,
    }
...
def direction_rows(manifest, request, gradient_root, *, device):
    ...
    with ExitStack() as stack:
        weights = stack.enter_context(
            safe_open(
                request["source"]["checkpoint"] + "/model.safetensors", framework="pt", device="cpu"
            )
        )
        shards = [
            stack.enter_context(
                safe_open(str(Path(gradient_root) / shard["path"]), framework="pt", device="cpu")
            )
            ...
            rows.append(
                {
                    ...
                    "algorithms": {
                        op: {
                            "frobenius_norm": float(torch.linalg.vector_norm(directions[op]).item())
                        }
                        for op in ("adamw", "muon")
                    },
                }
...
def replay_calibration(locations, state, directory, gradient_receipt_binding):
    ...
    runtime = _runtime(locations)
    root = Path(directory)
    verify_file(root / "gradient-receipt.json", gradient_receipt_binding)
    receipt = read_json(root / "gradient-receipt.json")
    require_same(receipt["request"], request)
    require_same(receipt["sources"], source)
    require_same(receipt["runtime"], runtime)
    if (
        receipt.get("scope") != SCOPE
        or receipt.get("status") != "gradients_complete"
    ...
    rows, rates = direction_rows(
        read_json(root / "gradients/manifest.json"), request, root / "gradients", device="cuda:0"
    )
    ...
    result = {
        ...
        "rows": file_identity(output / "rows.json"),
        "calibration": rates,
...
def read_calibration(inputs, state, directory, binding):
    ...
    request = calibration_request(inputs, state)
    root = Path(directory)
    verify_file(root / "directions/calibration.json", binding)
    ...
    runtime = inputs["runtime_spec"]
    if runtime["sha256"] != "b90de16b3796bb8e9d0badcedec688da55a037d46244a0d2a7f5838795cfe77a":
        raise ValueError("Calibration receipt uses an unbound runtime specification")
    verify_file(runtime["path"], runtime)
    if runtime_problems(load_runtime_spec(runtime["path"]), receipt["runtime"]):
        raise ValueError("Calibration receipt does not have the pinned numerical runtime")
    require_same(receipt["runtime"], result["runtime"])
    ...
    shapes = {
        r["name"].removeprefix("0.model."): r["shape"]
...
def load_calibrated_cells(locations, directories):
    ...
    inputs = load_inputs(locations)
    calibrated = {
        state: read_calibration(
            inputs, state, directories[state]["path"], directories[state]["calibration_binding"]
        )
        for state in STATES
    }
                ...
                jobs.append(
                    {
                        ...
                        "steps": 391,
                        "checkpoint_steps": [79, 157, 235, 313, 391],
