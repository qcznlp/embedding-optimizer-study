...
class FactorialTrainingArguments(SentenceTransformerTrainingArguments):
    ...
    def __post_init__(self):
        ...
        if isinstance(self.accelerator_config, dict) and (
            set(self.accelerator_config) - set(AcceleratorConfig.__dataclass_fields__)
        ):
            raise ValueError("Unknown factorial Accelerator configuration field")
...
def inspect_loader(loader, args):
    ...
    if (
        type(loader) is not DataLoaderShard
        or len(loader) != 1564
        or len(loader.dataset) != 50000
        or loader.use_stateful_dataloader is not False
    ):
