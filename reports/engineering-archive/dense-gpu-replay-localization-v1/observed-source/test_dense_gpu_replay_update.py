...
class ReplayUpdateGuards(unittest.TestCase):
    ...
    def test_scheduled_step_has_exact_pre_and_post_scheduler_state(self):
        ...
        config = OptimizerConfig(name="muon", lr=0.001)
        optimizer, _ = build_optimizer(model, config)
        scheduler = get_linear_schedule_with_warmup(optimizer, 1, 3)
        named = list(model.named_parameters())
        for _, p in named:
        ...
        gradients = [torch.ones_like(p) * 0.1 for _, p in named]
        replayed = direct_step(
            config,
            entry,
            [n for n, _ in named],
            optimizer_layout(optimizer, named),
            gradients,
