...
ANCHOR = '17b90cd5d8b2d9b47ba2849b5a9e3d0ffd17998e5ffcb9fc8b1f176e55ea0e7b'
SPEC = importlib.util.spec_from_file_location('independent_first_stage_reader', Path(__file__).with_name('verify_recovered.py'))
...
def test_actual_staged_native_scores_and_all_workers():
    ...
    assert proof['files_verified'] == 557
    assert proof['raw_task_scores_reconstructed'] == 168
    assert proof['native_exit_zero_workers_checked'] == 168
    assert proof['complete_checkpoint_means_reconstructed'] == 12
...
def test_semantic_corruption_refused_even_after_complete_rehash(tmp_path, mutation):
    ...
    if mutation in ('task_table', 'mean_table'):
        ...
        if mutation == 'checkpoint_step':
            value['plan']['checkpoint']['step'] = 1563
        else:
            value['tasks'][9]['task'] = 'Quora'
        path.write_text(json.dumps(value, indent=2) + '\n')
    else:
        manifest['checkpoint_stages'] = [1563]
