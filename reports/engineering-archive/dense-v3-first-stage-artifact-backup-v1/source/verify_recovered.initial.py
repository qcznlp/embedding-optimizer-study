"""Independently replay a supplied first-stage data snapshot using only Python stdlib.
...
def verify(root, manifest_sha):
    ...
    require(manifest['scope'] == 'complete-primary-v3-first-stage-beir-artifact-backup'
            and manifest['primary_protocol_sha256'] == PROTOCOL
            and manifest['runs'] == list(RUNS) and manifest['checkpoint_stages'] == [782]
    ...
    def mapped_snapshot(item):
        ...
        name = 'native/beir-first-stage/' + relative
    ...
    require(acceptance['accepted_bundle_sha256'] == manifest['accepted_bundle_sha256']
            ...
            and acceptance['step'] == 782 and acceptance['task_cells'] == 168
    ...
    native = {name for name in expected if name.startswith('native/beir-first-stage/')}
    ...
    for name in complete_names:
        ...
        require(run in RUNS and run not in seen_runs and plan['checkpoint']['step'] == 782
        ...
        require(name == 'native/beir-first-stage/' + safe_relative(plan['cache_key'])
        ...
        for task_record in complete['tasks']:
            ...
            worker_task = 'Quora' if task == 'QuoraRetrieval' else task
            suffix = '/' + run + '-782-' + worker_task + '.started.json'
            ...
            require(started['job'] == exited['job'] == [run, 782, worker_task]
            ...
            require(command[command.index('--worker') + 1] == worker_task
                    and command[command.index('--models') + 1].endswith('/' + run + '/checkpoint-782')
                    and command[command.index('--results_folder') + 1]
                    == plan['results_root'] + '/dense/' + run + '__checkpoint-782', 'Worker command differs')
    ...
    for row in mean_rows + task_rows:
        ...
        require(row['optimizer'] == opt and row['learning_rate'] == rate and row['step'] == '782',
    ...
    return {'scope': 'independent-supplied-first-stage-snapshot-byte-and-score-reconstruction',
            ...
            'first_stage_means_0_to_100': {run: float(means[run] * 100) for run in RUNS},
