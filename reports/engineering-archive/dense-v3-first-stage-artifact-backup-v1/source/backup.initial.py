"""One-shot data-only backup of all twelve accepted step-782 BEIR evaluations."""
...
ORIGINAL = ROOT / 'reports/engineering-archive/dense-v3-first-stage-evaluations-v1'
...
BUNDLE_SHA = 'ed537f9678dba6af8b32de3ad04a24a5a0275e6c2105b87bc727b546045630e0'
VERIFICATION_SHA = '1053bf60fd193cfe4170887961c7abdc1420111615ee65572b16ab1fa86726e1'
...
PARENT = '13b110a0d067948ff518a0f720ee84c2e2f8c6b1'
NAMESPACE = 'corrected-dense-correctness-v3'
ADDITION = NAMESPACE + '/complete-first-stage-evaluations'
...
TASKS = ('ArguAna', 'ClimateFEVER', 'DBPedia', 'FEVER', 'FiQA2018', 'HotpotQA',
         'MSMARCO', 'NFCorpus', 'NQ', 'Quora', 'SCIDOCS', 'SciFact', 'TRECCOVID', 'Touche2020')
...
def load_transport():
    ...
    spec = importlib.util.spec_from_file_location('first_stage_transport_parent', TRANSPORT)
...
def select_rows(bundle):
    ...
    require(len(records) == 24 and {(r['run_id'], r['step']) for r in records}
            == {(run, step) for run in RUNS for step in (782, 3907)},
            'Require the exact complete first-and-final configuration grid')
    rows = [next(r for r in records if r['run_id'] == run and r['step'] == 782) for run in RUNS]
    for row in rows:
        require(row['task_count'] == 14 and len(row['original_workers']) == 14
                and [r['task'] for r in row['native_complete_reread']['tasks']] == list(TASKS),
                'Incomplete first-stage tasks or worker records')
    return rows


def source_inputs(t):
    verification = t.read(ORIGINAL / 'verification.json', VERIFICATION_SHA)
    require(verification['complete_checkpoint_records'] == 24
            and verification['raw_task_scores_independently_checked'] == 336
            and verification['scientific_completion'] is False, 'Wrong accepted readback')
    ...
    for row in rows:
        for item in [row['original_complete_receipt'], row['original_operational_receipt'],
            ...
            add('native/beir-first-stage', EXPERIMENT / 'evaluations/dense-primary-v3', item)
...
def derived_bytes(bundle, rows):
    ...
    for row in rows:
        ...
        common = dict(run_id=run, optimizer=optimizer, learning_rate=rate, step=782)
    ...
    acceptance = {
        'scope': 'accepted-complete-step-782-beir-data-only', 'primary_protocol_sha256': PROTOCOL,
        'accepted_bundle_sha256': BUNDLE_SHA, 'accepted_verification_sha256': VERIFICATION_SHA,
        'native_readback_at_utc': bundle['observed_at_utc'], 'runs': list(RUNS), 'step': 782,
...
def prepare(t, work):
    ...
    manifest = {
        'schema_version': 1, 'scope': 'complete-primary-v3-first-stage-beir-artifact-backup',
        'created_at_utc': t.stamp(), 'primary_protocol_sha256': PROTOCOL,
        'accepted_bundle_sha256': BUNDLE_SHA, 'accepted_verification_sha256': VERIFICATION_SHA,
        'runs': list(RUNS), 'checkpoint_stages': [782], 'beir_task_cells': 168,
...
def prepared(t, work):
    ...
    require(set(manifest['files']) == set(selected) | GENERATED
            ...
            and manifest['checkpoint_stages'] == [782] and manifest['beir_task_cells'] == 168,
...
def upload(t, work):
    ...
    require(ADDITION not in old_sub, 'Existing first-stage subtree: inspect, do not overwrite')
    ...
    commit = api.create_commit(REPO, repo_type='dataset', parent_commit=PARENT, operations=operations,
        token=True, num_threads=2, commit_message='Preserve all corrected DenseOn first-stage BEIR results (data only)')
...
def download_verify(t, work):
    ...
    result = {'scope': 'actual-complete-anonymous-first-stage-beir-recovery',
...
def main():
    ...
    require(args.workdir.parent == Path('/tmp')
            and args.workdir.name.startswith('dense-v3-first-stage-artifact-backup.')
