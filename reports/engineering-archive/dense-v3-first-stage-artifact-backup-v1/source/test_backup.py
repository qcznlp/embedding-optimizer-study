...
SPEC = importlib.util.spec_from_file_location('first_stage_backup_under_test', Path(__file__).with_name('backup.py'))
...
def test_actual_complete_first_stage_selection_and_safety(actual):
...
def test_incomplete_or_wrong_cohort_refused(actual, mutation):
    ...
    row = next(r for r in bundle['checkpoints'] if r['step'] == 782)
    if mutation == 'missing':
...
def preservation():
    ...
    old = {B.NAMESPACE + '/' + n: {'kind': 'RepoFolder', 'tree_id': n}
           for n in ('complete-endpoint-evaluations', 'endpoint-statistics', 'training-dynamics', 'weight-space')}
