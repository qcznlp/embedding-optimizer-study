"""Build the current declared distribution in a separate CPU-only directory."""
...
STAGE = HERE / 'distribution-baseline'
OUTPUT = HERE / 'built-baseline'
...
write(HERE / 'distribution-inputs.json', {'created_at_utc': datetime.now(timezone.utc).isoformat(),
      'scope': 'current_declared_distribution_baseline_not_final_scientific_release',
...
with (HERE / 'distribution-build.log').open('x') as stream:
...
write(HERE / 'distribution-build.json', {'completed_at_utc': datetime.now(timezone.utc).isoformat(),
