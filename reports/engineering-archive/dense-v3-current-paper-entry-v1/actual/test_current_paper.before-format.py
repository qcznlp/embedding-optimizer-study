...
def test_original_complete_component_is_byte_identical():
    assert hashlib.sha256(Path(document.__file__).read_bytes()).hexdigest() == entry.COMPONENT_SHA256
...
def test_all_current_document_inputs_are_declared_in_distribution():
    ...
    required = {"paper/current/" + n for n in (*document.BUILD_INPUTS, entry.SNAPSHOT_NAME, "README.md")}
    assert required <= declared
    assert config["project"]["scripts"]["embed-optim-build-current-paper"] == "embed_optim.current_paper:main"
