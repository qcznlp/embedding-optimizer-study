...
FUNCTIONAL_TABLE_LABEL = "\\label{tab:dimension-utilization}\n"
FUNCTIONAL_TABLE_CAPTION = (
    "\\caption{All nine final-stage co-primary contrasts, averaging every declared rate. "
    "One common-resample max-$T$ family covers all entries; task resampling does not "
    "estimate training-seed variation.}\n"
)


def bind_functional_table(raw):
    """Bind the actual co-primary table, preserving the immutable inference text separately."""
    macro = "\\newcommand{\\DimensionUtilizationAppendixTable}"
    if (
        raw.count(macro) != 1
        or raw.count(FUNCTIONAL_TABLE_CAPTION) != 1
        or "\\label{tab:dimension-utilization}" in raw
        or raw.index(FUNCTIONAL_TABLE_CAPTION) < raw.index(macro)
    ):
        raise ValueError("Unexpected functional co-primary table structure")
    return raw.replace(
        FUNCTIONAL_TABLE_CAPTION, FUNCTIONAL_TABLE_CAPTION + FUNCTIONAL_TABLE_LABEL, 1
    )
