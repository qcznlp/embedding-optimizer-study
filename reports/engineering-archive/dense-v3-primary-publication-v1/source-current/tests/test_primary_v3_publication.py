...
from embed_optim import primary_v3_publication_render as rendering
...
def test_noncanonical_rational_fields_refuse(tmp_path, value):
    archive = ROOT / "reports/engineering-archive/dense-v3-primary-renderer-v1"
    tables = read_json(archive / "probe-result.json")["cases"][0]["complete_kernel_tables"]
    tables["feature_prediction_summary"][0]["pooled_mse_reduction_exact"] = value
    for name, rows in tables.items():
        (tmp_path / (name + ".csv")).write_bytes(csv_bytes(rows))
    with pytest.raises(ValueError):
        publication.read_tables(tmp_path, publication.inference.original.TABLE_COUNTS)
...
@pytest.fixture(scope="module")
def functional_text():
    archive = ROOT / "reports/engineering-archive/dense-v3-primary-publication-v1"
    receipt = read_json(archive / "complete-retry.json")
    rows = [
        row
        for row in receipt["generated_artifacts"]
        if row["path"].endswith("/synthetic-evidence/dimension-utilization.tex")
    ]
    assert len(rows) == 1
    path = archive / "layout-attempt-1/functional-inference.tex"
    assert file_identity(path)["sha256"] == rows[0]["sha256"]
    return path.read_text()


def test_actual_functional_text_changes_only_by_required_table_label(functional_text):
    result = rendering.bind_functional_table(functional_text)
    assert result.count(rendering.FUNCTIONAL_TABLE_LABEL) == 1
    assert result.replace(rendering.FUNCTIONAL_TABLE_LABEL, "", 1) == functional_text
    assert rendering.FUNCTIONAL_TABLE_CAPTION + rendering.FUNCTIONAL_TABLE_LABEL in result


@pytest.mark.parametrize("kind", ["missing", "duplicate", "already_bound", "before_macro"])
def test_functional_table_binding_refuses_wrong_structure(functional_text, kind):
    caption = rendering.FUNCTIONAL_TABLE_CAPTION
    changed = {
        "missing": functional_text.replace(caption, ""),
        "duplicate": functional_text + caption,
        "already_bound": functional_text + rendering.FUNCTIONAL_TABLE_LABEL,
        "before_macro": caption + functional_text.replace(caption, ""),
    }[kind]
    with pytest.raises(ValueError, match="co-primary table structure"):
        rendering.bind_functional_table(changed)
