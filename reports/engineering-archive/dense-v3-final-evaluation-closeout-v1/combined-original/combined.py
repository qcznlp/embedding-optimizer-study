...
def admit_completed_document():
    ...
    need(inferred['scope'] == 'owner-authorized-complete-genuine-v3-factorial-inference-v1'
         and inferred['source']['sha256'] == SUMMARY_SHA
         and inferred['authorization_sha256'] == SUMMARY_AUTH
         and inferred['actual_collector_exits'] == {'beir': 0, 'probe': 0}
    ...
    for kind, count in (('beir', 168), ('probe', 60)):
        ...
        need(value['scope'] == 'actual-complete-genuine-v3-factorial-' + kind + '-readback'
             ...
             and value['collector_source']['sha256'] == '988c71fd9ad24576a017864ce9b99eeeb0ed6e5d48327483607b185fe4175366'
             and read(SUMMARY / 'run' / (kind + '.exited.json'))['exit_code'] == 0,
             'Incomplete actual native collector')
        need(value['fresh_original_task_reader'] is True if kind == 'beir'
             else value['raw_arrays_and_metrics_reconstructed'] is True, 'Missing native numerical readback')
...
def build_bundle(target):
    ...
    for index, (path, value) in enumerate(sorted(scientific['factorial'].items())):
        add(f'native-factorial-bindings/{index:03d}-' + Path(path).name, Path(path), value)
...
def isolated_replay(args):
    ...
    manifest = inventory(bundle, args.manifest_sha256, SCOPE)
    bound(__file__, manifest['files']['replay_complete.py'])
    need(not any(name == 'embed_optim' or name.startswith('embed_optim.') for name in sys.modules),
         'A project package was imported before the fresh replay')
...
def prepare(args):
    ...
    bound(__file__, args.source_sha256)
    parent_inputs()
    tests = read(HERE / 'tests-final.json', args.tests_sha256)
    need(tests['source'] == identity(__file__) and tests['test_source'] == identity(HERE / 'test_combined.py')
         and tests['tests_run'] >= 13 and tests['actual_factorial_or_paper_admission'] is False
         and tests['failures'] == tests['errors'] == tests['skips'] == 0,
    ...
    inputs[str(HERE / 'tests-final.json')] = identity(HERE / 'tests-final.json')
