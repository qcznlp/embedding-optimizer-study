...
def compare_display_projection(current, previous, path=''):
    """Original renderers retain raw CSV strings; native tables are typed.

    Decode only a numeric string opposite a native numeric field. All values,
    keys and ordering remain exact; no tolerance, boolean or missing-value cast.
    Returns explicit paths of the representation-only differences.
    """
    from embed_optim.primary_contract import require_same
    if isinstance(current, dict) and isinstance(previous, dict):
        need(set(current) == set(previous), 'Display summary fields differ')
        return [p for key in sorted(current)
                for p in compare_display_projection(current[key], previous[key], path + '/' + key)]
    if isinstance(current, list) and isinstance(previous, list):
        need(len(current) == len(previous), 'Display summary population differs')
        return [p for index, (a, b) in enumerate(zip(current, previous, strict=True))
                for p in compare_display_projection(a, b, path + '/' + str(index))]
    if type(current) in (int, float) and isinstance(previous, str):
        def invalid(_):
            raise ValueError('Nonfinite display field')
        decoded = json.loads(previous, parse_constant=invalid)
        need(type(decoded) in (int, float), 'Display field is not a numeric CSV string')
        require_same(current, decoded)
        return [path]
    require_same(current, previous)
    return []
...
def assemble(output):
    ...
    numeric_csv_fields = compare_display_projection({key: summary[key] for key in previous_summary}, previous_summary)
    ...
    write_new(output / 'completed.json', {'scope': 'current_native_primary_scientific_result_assembly_v1',
        ...
        'display_summary_numeric_csv_fields_decoded_exactly': numeric_csv_fields,
