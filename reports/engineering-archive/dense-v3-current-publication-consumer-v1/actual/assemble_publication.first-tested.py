...
def join_native(primary, values):
    ...
    need(len(geometry['rows']) == 12, 'Incomplete geometry whole-run admission')
    ...
    need(len(jobs) == 60, 'Incomplete native functional checkpoint joins')
...
def assemble(output):
    ...
    for name in ('all_task_scores', 'primary_task_effects', 'primary_summary',
        ...
        require_same(outcome_tables[name], exact_bridge.typed_csv(previous / (name + '.csv')))
