...
def test_declared_scalar_revision_preserves_other_full_tensor_index_seeded_metrics(tmp_path):
    ...
    for step in (1, 2):
        ...
        actual = read_json(output / f"checkpoint-{step}.json")["records"]
        for old, new in zip(records, actual, strict=True):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("implementation_revision", "tolerances_changed"), True),
        (("implementation_revision", "version"), 1),
    ],
)
