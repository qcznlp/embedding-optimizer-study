...
def compare_unchanged_records(new_records, old_records):
    new_records, old_records = copy.deepcopy(new_records), copy.deepcopy(old_records)
    for new, old in zip(new_records, old_records, strict=True):
        for field in ("weight", "delta_from_reference", "delta_from_previous"):
            if field in old:
                for scalar in ("frobenius_norm", "cosine_with_weight"):
                    new[field].pop(scalar, None)
                    old[field].pop(scalar, None)
    require_same(new_records, old_records)


def independent_scalars(run, reference, steps, index, records):
    step = steps[index]
    previous_path = run / f"checkpoint-{steps[index - 1]}" if index else reference
    norms = cosines = undefined = 0
    max_relative = max_cosine_absolute = 0.0
    with (
        TensorStore(run / f"checkpoint-{step}") as current,
        TensorStore(reference) as initial,
        TensorStore(previous_path) as previous,
    ):
        for record in records:
            ...
            values = {
                "weight": weight,
                "delta_from_reference": weight - initial.tensor(name).float(),
            }
            if index:
                values["delta_from_previous"] = weight - previous.tensor(name).float()
            weight_array = weight.numpy().astype(np.float64).ravel()
            weight_norm = float(np.linalg.norm(weight_array))
            for key, value in values.items():
                array = value.numpy().astype(np.float64).ravel()
                norm = float(np.linalg.norm(array))
                compare_scalar(record[key]["frobenius_norm"], norm)
                norms += 1
                if norm:
                    max_relative = max(
                        max_relative, abs(record[key]["frobenius_norm"] - norm) / norm
                    )
                if key != "weight":
                    if not norm:
                        assert record[key]["cosine_with_weight"] is None
                        undefined += 1
                    else:
                        cosine = float(np.dot(array, weight_array) / norm / weight_norm)
                        compare_scalar(record[key]["cosine_with_weight"], cosine)
                        max_cosine_absolute = max(
                            max_cosine_absolute, abs(record[key]["cosine_with_weight"] - cosine)
                        )
                        cosines += 1
    return {
        "norms": norms,
        "defined_cosines": cosines,
        "undefined_cosines": undefined,
        "max_norm_relative_error": max_relative,
        "max_cosine_absolute_error": max_cosine_absolute,
    }
...
def run_audit(root, training_root, work):
    ...
    (work / "runs").mkdir()
    (work / "legacy").mkdir()
    checked_rows, records, scalar_checks = [], [], []
    for run, expected, checked in admitted:
        run_id = expected["recipe"]["run_id"]
        ...
        for stage, step in enumerate(checked["steps"]):
            ...
            compare_unchanged_records(value["records"], old)
            scalar_checks.append(
                independent_scalars(run, reference, checked["steps"], stage, value["records"])
            )
    ...
    changed_fields = {
        "saved_segment_nonzero_parameter_fraction",
        "weight_frobenius_norm",
        "saved_segment_frobenius_norm",
        "saved_segment_to_weight_ratio",
        "cumulative_displacement_frobenius_norm",
        "cumulative_displacement_to_weight_ratio",
    }
    for old, new in zip(old_rows, tables["checkpoint_geometry"], strict=True):
        require_same(
            {key: new[key] for key in old if key not in changed_fields},
            {key: value for key, value in old.items() if key not in changed_fields},
        )
        assert (
    ...
    return {
        ...
        "old_raw_records_unchanged_except_declared_global_scalar_precision": 792,
        "entry_amendment_record_comparisons": 792,
        "independent_global_scalar_checks": scalar_checks,
        "independent_norm_comparisons": sum(row["norms"] for row in scalar_checks),
        "independent_defined_cosine_comparisons": sum(
            row["defined_cosines"] for row in scalar_checks
        ),
        "independent_undefined_cosine_checks": sum(
            row["undefined_cosines"] for row in scalar_checks
        ),
