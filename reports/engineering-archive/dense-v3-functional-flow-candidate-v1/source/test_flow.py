...
class Fixture:
    def __init__(self,root,fail=None):
        ...
        def matrix(path,admitted,sha):
            ...
            same(set(m['states']),set(CELLS));return m
    ...
    def observe(self,exact=None):
        ...
        return flow.observe_records(self.root/'run',CELLS,{j['plan']['state']['cell']:digest(j['plan']) for j in self.context.jobs},
            ...
            coordinator_pid=-100,output_root=self.entry.OUTPUT,expected_reuse_origin=self.origin,
            observe_exact=exact or (lambda _:(_ for _ in ()).throw(AssertionError('Unexpected process read'))))
...
class Tests(unittest.TestCase):
    ...
    def test_observer_rejects_changed_reuse_origin(self):
        f=self.fixture();f.run();p=f.root/'run/pretrained.reused.json'
        record=json.loads(p.read_bytes());record['origin']['authorization_sha256']='changed'
        p.write_text(json.dumps(record))
        with self.assertRaisesRegex(ValueError,'external recovery binding'):f.observe()

    def test_observer_rejects_corrupt_copied_pretrained_bytes(self):
        f=self.fixture();f.run();p=f.entry.OUTPUT/'vectors/states/pretrained/vectors.npz'
        p.write_bytes(b'corrupted synthetic data')
        with self.assertRaisesRegex(ValueError,'Bound origin'):f.observe()

    def test_observer_rejects_corrupt_feature_payload(self):
        f=self.fixture();f.run();p=f.entry.OUTPUT/'features/states'/CELLS[-1]/'manifest.json'
        p.write_text('{}')
        with self.assertRaisesRegex(ValueError,'Bound origin'):f.observe()
