...
Last updated: **2026-09-06 15:12 UTC**; the machine-readable snapshot is refreshed at this handoff.

## Where the project stands

**Actual query text is not fully partition-disjoint: 84 training records match 51 validation queries, and one training query matches BEIR. The formal matrix remains on scientific hold.**
...
The [new full query-partition audit](reports/engineering-archive/dense-data-partition-v1/README.md)
...
Next implement the declared original-population seeded candidate order and materialization,
reserving all original train/validation query IDs/texts and all BEIR evaluation query texts.
**No replacement query or revised dataset has yet been produced.** The v2 draft still binds old
data; a reviewed explicit amendment is required, not refreshed expected hashes or erased evidence.
...
| Isolated paper/audit suite | 1,606 cases, zero failures/errors/skips; nine new query-protection tests pass | Text eligibility of actual inputs or correctness of the different candidate source tree |
...
1,606 cases / zero failures. The older candidate's 923-case receipt remains valid historical evidence.
...
2. First implement/review the expanded six-training/52-validation-group common-data revision
   linked above. Reconstruct original source populations and seeded orders before exclusions;
   preserve all non-target row contents/quotas and independently reproduce the new selections.
   Re-audit full text eligibility and train/validation/BEIR query-text separation. Preserve the
   failed prefix; do not substitute another prefix or weaken its check. No replacement query
   or revised data exists yet. Then prepare declared
...
- [Latest full query-partition audit and independent raw-string replay](reports/engineering-archive/dense-data-partition-v1/README.md);
  [expanded 6/52-group proposal, not materialized](reports/engineering-archive/dense-data-partition-v1/revision-proposal.json).
