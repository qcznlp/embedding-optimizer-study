    ...
    for path in sorted(root.rglob('*')):
        ...
        if path.suffix == '.md':
            for target in re.findall(r'\]\(([^)]+)\)', raw.decode()):
                ...
                # A preserved before-copy retains its original document-relative links.
                origin = story if path == root / 'before/CURRENT_EXPERIMENT.md' else path.parent
                assert (origin / local).exists(), (path, local)
