"""One-shot data-only backup of all twelve accepted step-2345 evaluations."""
...
BUNDLE_SHA = 'cc7419b025af23952e1e0ef813849677d71c1d1756607dbdd4e3c262552a1179'
VERIFICATION_SHA = '6d44145232234a8602dbb01ec02535ef1d46932dcf7abc1561ea6b16574c636c'
INDEPENDENT_SHA = '3ec6dd093d837e10f2445bb82cc0ddd686ec3857c8428b3664c0edb1ddf06da1'
...
ADDITION = NAMESPACE + '/complete-third-stage-evaluations'
...
def load_transport():
    ...
    spec = importlib.util.spec_from_file_location('complete_third_stage_transport_parent', TRANSPORT)
...
def select_rows(bundle):
    ...
    expected = {(run, step) for run in RUNS for step in (782, 1563, 2345, 3907)}
    require(len(records) == 48 and {(r['run_id'], r['step']) for r in records} == expected,
            'Require all twelve configurations at steps 782, 1563, 2345 and 3907')
    rows = [next(r for r in records if r['run_id'] == run and r['step'] == 2345) for run in RUNS]
    for row in rows:
        require(row['task_count'] == 14 and len(row['original_workers']) == 14
                and [r['task'] for r in row['native_complete_reread']['tasks']] == list(TASKS),
                'Incomplete third-stage tasks or worker records')
    return rows

def check_accepted_receipts(verification, independent):
    require(verification['scope'] == 'complete_third_stage_local_archive_verification'
            and len(verification['payloads']) == 14
            and verification['complete_checkpoint_outcomes'] == 48
            and verification['new_checkpoint_outcomes'] == 6
            and verification['raw_task_values'] == 672
            and verification['new_raw_task_values'] == 84
            and verification['previous_forty_two_records_exactly_unchanged'] is True
            and verification['three_archived_replay_tables_byte_identical'] is True
            and verification['scientific_completion'] is False,
            'Wrong accepted archive verification')
    ...
    require(independent['scope'] == 'independent_complete_third_stage_raw_score_reconstruction'
            and independent['native_bundle']['sha256'] == BUNDLE_SHA
            and independent['complete_checkpoint_records'] == 48
            and independent['new_checkpoint_records'] == 6
            and independent['new_raw_task_values'] == 84
            and independent['raw_task_scores_checked'] == 672
            and independent['native_exit_zero_workers_checked'] == 672
            and independent['raw_score_and_metadata_snapshots_checked'] == 768
            and independent['previous_forty_two_records_exactly_unchanged'] is True
            and independent['all_twelve_third_stage_configurations_complete'] is True
            and independent['third_stage_checkpoint_rows'] == 12
            and independent['third_stage_raw_task_values'] == 168
            and independent['all_original_rational_means_exact'] is True
            and independent['new_model_execution'] is False
            and independent['new_statistical_inference'] is False
            and independent['scientific_completion'] is False,
            'Wrong independent accepted cohort')
...
def source_inputs(t):
    ...
    check_accepted_receipts(verification, independent)
    authority = t.read(EXPERIMENT / 'launch/evaluation-handoff/authorization.json', AUTH_SHA)
    require(set(authority['queues']) == {'a', 'b'}
            and len(authority['queues']['a']) == len(authority['queues']['b']) == 6
            and not (set(authority['queues']['a']) & set(authority['queues']['b']))
            and set(authority['queues']['a']) | set(authority['queues']['b']) == set(RUNS),
            'Cohort differs from all twelve original queue configurations')
    ...
    for row in rows:
        for item in [row['original_complete_receipt'], row['original_operational_receipt'],
            ...
            add('native/beir-third-stage', EXPERIMENT / 'evaluations/dense-primary-v3', item)
    ...
    require(len(selected) == 552, 'Require 216 native files and 336 original worker records')
...
def derived_bytes(bundle, rows):
    ...
    for row in rows:
        ...
        common = dict(run_id=run, optimizer=optimizer, learning_rate=rate, step=2345)
    ...
    acceptance = {
        'scope': 'accepted-complete-step-2345-beir-data-only', 'primary_protocol_sha256': PROTOCOL,
        'accepted_bundle_sha256': BUNDLE_SHA, 'accepted_verification_sha256': VERIFICATION_SHA,
        'native_readback_at_utc': bundle['observed_at_utc'], 'runs': list(RUNS), 'step': 2345,
        'tasks': list(TASKS), 'task_cells': 168, 'native_measurement_files': 216,
        'cohort_selection': 'all_declared_optimizer_rate_configurations',
        'all_twelve_third_stage_configurations_complete': True,
        'accepted_independent_readback_sha256': INDEPENDENT_SHA,
        'original_worker_records': 336, 'full_primary_task_cells_required': 840,
...
def prepare(t, work):
    ...
    manifest = {
        'schema_version': 1, 'scope': 'complete-primary-v3-third-stage-beir-artifact-backup',
        'created_at_utc': t.stamp(), 'primary_protocol_sha256': PROTOCOL,
        'accepted_bundle_sha256': BUNDLE_SHA, 'accepted_verification_sha256': VERIFICATION_SHA,
        'runs': list(RUNS), 'checkpoint_stages': [2345], 'beir_task_cells': 168,
        'cohort_selection': 'all_declared_optimizer_rate_configurations',
        'all_twelve_third_stage_configurations_complete': True,
        'accepted_independent_readback_sha256': INDEPENDENT_SHA,
        'native_measurement_files': 216, 'original_worker_records': 336,
        'source_code_included': False, 'new_model_or_statistical_execution': False,
...
def prepared(t, work):
    ...
    require(set(manifest['files']) == set(selected) | GENERATED
            ...
            and manifest['checkpoint_stages'] == [2345] and manifest['beir_task_cells'] == 168
...
def upload(t, work):
    ...
    require(ADDITION not in old_sub, 'Existing complete-third-stage subtree: inspect, do not overwrite')
    ...
    commit = api.create_commit(REPO, repo_type='dataset', parent_commit=PARENT, operations=operations,
        token=True, num_threads=2, commit_message='Preserve all twelve corrected DenseOn step-2345 BEIR evaluations (data only)')
...
def download_verify(t, work):
    ...
    result = {'scope': 'actual-complete-anonymous-third-stage-beir-recovery',
...
def main():
    ...
    require(args.workdir.parent == Path('/tmp')
            and args.workdir.name.startswith('dense-v3-complete-third-stage-artifact-backup.')
