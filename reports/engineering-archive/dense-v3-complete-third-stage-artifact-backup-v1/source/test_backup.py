...
SPEC = importlib.util.spec_from_file_location('complete_third_stage_backup_under_test', Path(__file__).with_name('backup.py'))
...
def test_actual_complete_third_stage_selection_and_safety(actual):
...
def test_incomplete_or_wrong_cohort_refused(actual, mutation):
    ...
    row = next(r for r in bundle['checkpoints'] if r['step'] == 2345)
    if mutation == 'missing':
...
def preservation():
    ...
    old = {B.NAMESPACE + '/' + n: {'kind': 'RepoFolder', 'tree_id': n}
           for n in ('complete-endpoint-evaluations', 'endpoint-statistics', 'training-dynamics', 'weight-space', 'complete-first-stage-evaluations', 'partial-second-stage-pool-b-evaluations', 'complete-second-stage-evaluations', 'partial-third-stage-pool-b-evaluations')}
...
@pytest.mark.parametrize('target,field,value', [
    ('verification', 'complete_checkpoint_outcomes', 36),
    ('verification', 'raw_task_values', 504),
    ('verification', 'previous_forty_two_records_exactly_unchanged', False),
    ('verification', 'three_archived_replay_tables_byte_identical', False),
    ('verification', 'payload_count', None),
    ('verification', 'payload_digest', '0' * 64),
    ('independent', 'complete_checkpoint_records', 36),
    ('independent', 'raw_score_and_metadata_snapshots_checked', 767),
    ('independent', 'previous_forty_two_records_exactly_unchanged', False),
    ('independent', 'third_stage_checkpoint_rows', 11),
    ('independent', 'third_stage_raw_task_values', 167),
    ('independent', 'all_original_rational_means_exact', False),
])
def test_new_archive_schema_and_complete_population_refused(actual, target, field, value):
    ...
    independent = transport.read(B.ORIGINAL / 'actual/independent-raw-reconstruction.json', B.INDEPENDENT_SHA)
    changed = verification if target == 'verification' else independent
    if field == 'payload_count':
        ...
        changed[field] = value
    with pytest.raises(ValueError):
        B.check_accepted_receipts(verification, independent)
