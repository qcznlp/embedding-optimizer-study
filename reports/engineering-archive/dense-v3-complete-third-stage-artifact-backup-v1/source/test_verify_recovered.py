...
SPEC = importlib.util.spec_from_file_location('independent_complete_third_stage_reader', Path(__file__).with_name('verify_recovered.py'))
...
def test_semantic_corruption_refused_even_after_complete_rehash(tmp_path, mutation):
    ...
    if mutation in ('task_table', 'mean_table'):
        ...
        if mutation == 'checkpoint_step':
            value['plan']['checkpoint']['step'] = 3126
        else:
            value['tasks'][9]['task'] = 'Quora'
        path.write_text(json.dumps(value, indent=2) + '\n')
    else:
        manifest['checkpoint_stages'] = [3126]
