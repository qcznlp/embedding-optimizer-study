...
Last observed **2026-09-12 00:24 UTC** (observer-reported timestamp). This is a dated execution snapshot, not a
live heartbeat or a scientific conclusion. Exact separately timestamped process
observations are in the [latest snapshot](reports/engineering-archive/dense-v3-complete-third-stage-evaluations-v1/observations.json);
...
| Primary BEIR | **672 / 840 task cells**, eight exact primary workers live in this snapshot | 168 cells remain; all 12 rates × five stages × 14 tasks stay required |
| Complete primary checkpoint BEIR | **[48 / 60](reports/engineering-archive/dense-v3-complete-third-stage-evaluations-v1/README.md)**, all twelve configurations at steps 782, 1563, 2345 and 3907; native and independent readback verify all 672 raw task values | All twelve step-3126 states remain; no full-trajectory claim |
| Complete third-stage score table | **[12 / 12 configurations](reports/engineering-archive/dense-v3-complete-third-stage-evaluations-v1/tables/third-stage-checkpoint-scores.md)**, all 168 task scores; only six pool-A checkpoints / 84 values are new, and the previous 42 complete records are unchanged | The six new pool-A outcomes are locally archived, not yet added to HF; the earlier six-state pool-B snapshot remains unchanged |
...
evaluation dispatchers retain their queues. The functional coordinator is also
...
1. Finish the remaining 168 cells of the existing full 840-cell evaluation matrix
   without changing corpora, scoring, context or frozen workers. Preserve the
   completed validations and fixed selection. Add the six newly accepted pool-A
   step-2345 outcomes to a new immutable data-only backup; the original six-state
   pool-B backup and all prior snapshots must remain unchanged.
