...
The owner has been asked to authorize pausing the last two runs and automatic post-processing while
preserving all checkpoints. No answer or pause has occurred; the live jobs/controllers still run,
and the independent backup supervisor remains active. The former post-processing-only handoff is
not sufficient to resolve this new training-stack issue. This section supersedes the earlier advice
to proceed directly to final evaluation. Engineering findings stay out of the manuscript.
...
The artifact-only [current snapshot](CURRENT_PROGRESS.json) is dated 06:10 UTC on September 5.
...
| --- | --- | --- |
| Primary execution | **10/12 runs finished:** all four AdamW, all four Muon, NorMuon 1e-4 and 3e-4 | Resolve the normalization finding; two NorMuon runs still execute |
| Checkpoints | **54/60 resumable and remotely covered**; all ten complete-run backups exist | Preserve remaining stages; backup is not scientific validity |
| Accumulated Trainer gradients | **Failed** actual four-rank CPU reference check; paired diagnostic compensation passes | Authorized pause, correction and scientific-impact/re-run decision |
| Training/data integrity | 50 completed stages pass deep payload checks; all 500,000 row identities agree; 89 bindings match | Remaining runs; broader runtime checks are not implied |
...
The two active NorMuon runs last recorded steps 1744 and 1634 of 3907. A fresh query of their exact
W&B handles independently confirms both running at 06:09 UTC (steps 1740 and 1630). The snapshot contains zero
fatal error markers; this is an artifact observation, not exhaustive runtime certification. The
existing matrix owns both four-GPU pools and all pool handoffs. Do not start another matrix.
