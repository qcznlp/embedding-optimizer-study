...
def _validate_export(path: Path, dimension: int) -> tuple[dict[str, np.ndarray], dict[str, Any]]:
...
def analyze(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected_checkpoints = int(
        protocol["inputs"][f"{args.analysis_scope}_expected_checkpoints"]
    )
    ...
    for path in paths:
        ...
        arrays, _manifest = _validate_export(path, dimension)
            ...
            checkpoint_row.update(
                {
                    ...
                    for key, value in _covariance_summary(vectors).items()
                }
            )
...
def _audit_identity(record: dict[str, Any], repository: Path) -> None:
...
def audit_outputs(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _audit_identity(manifest["implementation"], args.repository)
