...
| Reviewed manuscript | Complete result text, eight-page main, 158-word abstract and all 13 pages checked; [current build](../paper/current/README.md) | Document acceptance is not full source publication; legacy default release is separate |
| Source/distribution | Primary numerics integrated; original distribution audit passes; actual version-isolated complete numerical/current-paper wheel execution passes, with 194 integration tests; [composition guide](versioned-paper-reproduction.md) | Preserve original fresh-worker admission boundaries; reviewed default-entry/release-parent transition and actual source publication remain required |
