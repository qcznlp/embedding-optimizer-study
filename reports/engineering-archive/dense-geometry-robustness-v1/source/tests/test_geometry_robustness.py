...
def test_full_spectrum_metrics_are_scale_invariant_and_analytic(factor):
    values = np.array([4, 3, 0], dtype=np.float64) * factor
    result = spectrum_summary(values)
    # Decimal scaling and intermediate division are rounded. The first draft
    # incorrectly required real-arithmetic equality; keep its failed receipt.
    # Use the exact rational reference for the actual binary inputs and permit
    # one ULP, rather than confusing representational roundoff with a rank error.
    exact = float(sum(Fraction(float(x)) ** 2 for x in values) / Fraction(float(values[0])) ** 2)
    assert abs(result["stable_rank"] - exact) <= np.spacing(exact)
...
@pytest.mark.parametrize("factor", [2.0**-400, 1.0, 2.0**400])
def test_exactly_representable_power_of_two_scaling_retains_exact_stable_rank(factor):
    result = spectrum_summary(np.array([4.0, 3.0, 0.0]) * factor)
