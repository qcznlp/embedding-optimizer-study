...
**New correctness hold (September 5, 06:10 UTC):** a real four-rank CPU check of the unchanged
training stack finds double gradient-accumulation normalization (raw gradients are one quarter of
the intended global-batch reference). A paired diagnostic-only compensation passes; no live fix or
pause has occurred. The logged clipping regimes make a harmless-scaling assumption unjustified.
Read the [diagnosis and boundaries](reports/engineering-archive/dense-ddp-accumulation-v1/README.md).
Current run/backup counts describe execution and preservation, not cleared scientific evidence.
Do not promote these results or launch further evaluation until resolution; the requested pause is
awaiting owner confirmation. This engineering finding is not manuscript content.

The active work is a 12-run, DenseOn-only primary matrix. Ten runs are complete: all four AdamW,
all four Muon, and NorMuon 1e-4/3e-4. The existing matrix is running the last two NorMuon rates.
All 50 completed-run checkpoints and their five-stage raw weight-space features are verified on HF;
the independent whole-run content audit covers 1,010 files at original immutable commits.
Both final runs also have remotely verified checkpoints 782 and 1563, bringing coverage to 54/60.
The full retrieval and mechanism results are still pending. The implementation was
...
matrix is still incomplete; neither a final optimizer ranking nor a weight-space explanation is
