...
def build_conclusion(evidence: dict[str, Any]) -> str:
    ...
    return (
        ...
        "The validation-selected comparisons are "
        f"{secondary[('muon', 'adamw')]['support']} for Muon and "
        f"{secondary[('normuon', 'adamw')]['support']} for NorMuon. "
        "Under the frozen held-out bridge rule, predictively useful "
        f"geometry features are: {supported_text}. These are one-seed, pinned-grid results; the "
        "geometry bridge is not causal mediation."
    )


def build_corrected_finding(evidence: dict[str, Any]) -> str:
    primary = {(row["treatment"], row["baseline"]): row for row in evidence["primary"]}
    muon = primary[("muon", "adamw")]
    normuon = primary[("normuon", "adamw")]
    return (
        "Across the four-rate surface, Muon versus AdamW is "
        f"{muon['support']} at {_ci(muon)} nDCG@10, and NorMuon versus AdamW is "
        f"{normuon['support']} at {_ci(normuon)}."
    )


def build_weight_space_finding(evidence: dict[str, Any]) -> str:
    geometry = {row["optimizer"]: row for row in evidence["final_geometry"]}
    cumulative = ", ".join(
        f"{LABELS[optimizer]} {geometry[optimizer]['cumulative_displacement_to_weight_ratio']:.6f}"
        for optimizer in OPTIMIZERS
    )
    segments = ", ".join(
        f"{LABELS[optimizer]} {geometry[optimizer]['saved_segment_to_weight_ratio']:.6f}"
        for optimizer in OPTIMIZERS
    )
    return (
        "At the final stage, all-rate mean cumulative hidden-weight displacement relative to "
        f"weight norm is {cumulative}; the corresponding last-segment ratios are {segments}."
    )


def build_geometry_bridge_finding(evidence: dict[str, Any]) -> str:
    supported = [row["label"] for row in evidence["bridge"] if row["supported"]]
    if supported:
        names = ", ".join(supported)
        return (
            "Beyond optimizer, stage, and dose, the frozen out-of-dose test supports these "
            f"weight-space predictors of retrieval: {names}."
        )
    return (
        "None of the nine frozen weight-space features improves pooled held-out retrieval "
        "prediction while also improving at least three of four dose folds."
    )
...
def render_latex(evidence: dict[str, Any]) -> str:
    ...
    weight_finding = _tex_escape(build_weight_space_finding(evidence))
    bridge_finding = _tex_escape(build_geometry_bridge_finding(evidence))
    return (
        "% Generated primary optimizer evidence; do not edit manually.\n"
        "\\newcommand{\\CorrectedAbstractFinding}{" + finding + "}\n"
        "\\newcommand{\\CorrectedConclusionFinding}{" + finding + "}\n"
        "\\newcommand{\\CorrectedWeightSpaceFinding}{" + weight_finding + "}\n"
        "\\newcommand{\\CorrectedGeometryBridgeFinding}{" + bridge_finding + "}\n"
        "\\newcommand{\\CorrectedGeometryBridgeTable}{%\n"
        "\\begin{table*}[t]\n\\centering\n\\small\n"
        "\\begin{tabular}{lrrrl}\n\\toprule\n"
        r"Frozen geometry feature & $\Delta$RMSE & Folds & Supported & Residual $\rho$ \\"
        "\n\\midrule\n" + bridge_rows + "\n\\bottomrule\n\\end{tabular}\n"
        ...
        "The primary estimand averages all four frozen rates within optimizer from a common "
        "model, dataset, training schedule, and evaluation suite.\n\n"
        "\\begin{table}[t]\n\\centering\n\\scriptsize\n\\setlength{\\tabcolsep}{3pt}\n"
        "\\begin{tabular}{lrrl}\n\\toprule\n"
        r"Contrast & Mean & 95\% CI & Decision \\"
        "\n\\midrule\n" + primary_rows + "\n\\bottomrule\n\\end{tabular}\n"
        "\\caption{Final-stage retrieval inference over 14 decontaminated BEIR tasks "
        "with simultaneous intervals.}\n"
        "\\label{tab:optimizer-primary}\n\\end{table}\n\n"
        "\\paragraph{Primary conclusion.} " + conclusion + "\n"
        "}\n"
    )


def _publication_manifest(args, protocol, evidence) -> dict[str, Any]:
    return {
        ...
        "protocol": _file_record(args.protocol.resolve()),
        "sources": evidence["sources"],
        "outputs": {
            "standalone_markdown": _file_record(args.output_dir / "corrected_dense_results.md"),
            "paper_latex": _file_record(args.latex_output),
        },
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    protocol = _load_publication_protocol(args.protocol.resolve(), repository)
    evidence = load_publication_evidence(args, protocol)
    standalone = (
        "# Corrected independently padded DenseOn replication\n\n"
        + render_markdown(evidence)
        + "\n"
    )
    output_dir = args.output_dir.resolve()
    _atomic_text(output_dir / "corrected_dense_results.md", standalone)
    _atomic_text(args.latex_output.resolve(), render_latex(evidence))
    manifest = _publication_manifest(args, protocol, evidence)
    _atomic_json(output_dir / "summary_manifest.json", manifest)
    return manifest


def audit_report(args: argparse.Namespace) -> dict[str, Any]:
    """Reconstruct exact published text from its bound inputs without writing files."""
    repository = Path(__file__).resolve().parents[2]
    protocol = _load_publication_protocol(args.protocol.resolve(), repository)
    evidence = load_publication_evidence(args, protocol)
    output_dir = args.output_dir.resolve()
    ...
    if (output_dir / "corrected_dense_results.md").read_text() != expected_text:
        raise ValueError("Primary publication report differs from reconstructed evidence")
    if args.latex_output.read_text() != render_latex(evidence):
        raise ValueError("Primary manuscript differs from reconstructed evidence")
    expected = _publication_manifest(args, protocol, evidence)
    observed = json.loads((output_dir / "summary_manifest.json").read_text())
    if observed != expected:
        raise ValueError("Primary publication manifest differs from reconstructed evidence")
    return observed
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--latex-output",
        type=Path,
        default=Path("paper/generated/optimizer-primary.tex"),
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(
        json.dumps(
            audit_report(args) if args.audit_only else build_report(args), indent=2, sort_keys=True
        )
    )
