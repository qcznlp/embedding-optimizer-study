...
The artifact-only [current snapshot](CURRENT_PROGRESS.json) is dated 05:30 UTC on September 5.
...
| --- | --- | --- |
| Primary training | **10/12 complete:** all four AdamW, all four Muon, NorMuon 1e-4 and 3e-4 | NorMuon 1e-3 and 3e-3; both started, none unstarted |
| Checkpoints | **52/60 resumable and remotely covered**; all ten complete-run backups exist | Last eight scheduled stages |
...
The two active NorMuon runs last recorded steps 1413 and 1303 of 3907. A fresh query of their exact
W&B handles independently confirms both running at 05:30 UTC (steps 1410 and 1300). The snapshot contains zero
...
Three separate approvals remain unresolved:
...
1. Let the existing matrix finish the last two runs; follow artifacts and exact W&B handles.
   Refresh progress with the original `embed_optim.corrected_progress` entry point. Keep the
   existing whole-run finalizer and sealed-checkpoint backup as the sole owners of their work.
