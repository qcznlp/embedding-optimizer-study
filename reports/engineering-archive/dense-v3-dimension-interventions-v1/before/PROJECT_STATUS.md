...
Last updated: **2026-09-06 22:43 UTC**; the artifact-only snapshot was refreshed at 22:42:34 UTC.

## Where the project stands

**Prepared data/core/validation/outcome, exact/approximate geometry, original-feature bridge and exact-feature sensitivity pass their bounded checks. The next implementation work is v3 functional-dimension export/interventions/publication and the reviewed deployment handoff. A shared numerical helper corrects the dimension bridge's null-feature boundary, but that formal dimension chain is not yet migrated. No new valid primary results exist, and the old matrix stays on scientific hold.**
...
The [separate exact-feature sensitivity](reports/engineering-archive/dense-v3-exact-bridge-v1/README.md)
is now prepared under its own draft protocol. Five additional spectral/projector counterparts keep
distinct names, while the original nine-feature panel remains untouched. Full-spectrum entropy and
nonzero-matrix-weighted ranks retain their explicit estimand differences and denominator coverage.
Exact overlap uses all required same-stage AdamW comparators; one undefined pair makes the entire
focal mean undefined, without substituting a conditional or available-only mean. Missing rows are
errors; legitimate undefined measurements preserve complete rows and yield undefined full inference.

Its complete adapter freshly recomputes both the original bridge and exact geometry before joining
all rates/stages and rechecking raw inputs. Outputs contain all 300 measurement correspondences,
five predictive-sensitivity comparisons and every one of 300 held-out predictions. Independent full
augmented systems exactly match those predictions, twenty fold errors and five pooled decisions;
ten residual association values also match. Fresh CPU replay repeats all outputs and diagnostics.
Each process rejects four rehashed corruptions, ten altered protocols and both missing-primary CLI
modes. All 48 focused / 2,153 isolated tests pass. Positive retrieval/wiring panels are synthetic.

A separate real-data check verifies only 27 named-field mappings from nine already accepted short
diagnostic checkpoint geometry rows: eighteen defined and nine undefined values. No full SVD is
repeated, no retrieval score is added and no diagnostic acquires a primary identity. The incomplete
three-run population is explicitly refused as a primary AdamW-comparator population.

The old dimension publisher's separate floating regression reproduces the same fixed null-feature
false-positive boundary in four repeated feature slots. These are one duplicated counterexample,
not four independent findings. The named-family helper eliminates it using distinct dimension
feature names, without touching original weight features. **Only that numerical component is
verified; the primary dimension export, intervention, rotation, publication and release chain is
not yet migrated to v3 inputs or the correction.** Old dimension sources/protocols remain unchanged.
...
| Exact-feature bridge sensitivity | Five additional named features; 300 independent exact predictions, full replay, all comparisons/undefined coverage and real diagnostic field mapping | Valid primary sensitivity findings or functional-dimension explanations |
| Dimension numerical component | The four-name duplicate null control is corrected by the named helper; old code stays intact | V3 dimension export/intervention/publication integration or actual dimension utility |
| Isolated paper/audit suite | 2,153 cases, zero failures/errors/skips with absolute source paths; old failed attempts preserved | Correctness of the different numerical candidate source tree or formal scientific acceptance |
...
2,153 cases / zero failures. The older candidate's 923-case receipt remains valid historical evidence.
...
2. The expanded six-training/52-validation-group revision is now materialized and independently
   ...
   full-system/replay controls. Exact-measurement sensitivity now also passes independent complete
   prediction/replay and real geometry-only mapping controls, retaining all original features.
   Next integrate v3 functional-dimension export/interventions/publication/reconstruction with the
   explicit named-family numerical policy; its four-name null control is already verified, but
   its primary upstream/downstream identity chain is not. Neither
...
- [Exact-feature sensitivity, named families, independent replay and dimension numerical boundary](reports/engineering-archive/dense-v3-exact-bridge-v1/README.md);
  [separate draft exact-measurement sensitivity protocol](configs/dense_primary_v3_exact_bridge_protocol.json).
