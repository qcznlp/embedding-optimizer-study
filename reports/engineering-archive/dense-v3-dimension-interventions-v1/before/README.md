...
paper/audit checkout passes **all 2,153 cases**, including 48 exact-feature sensitivity checks.
...
now adds five explicitly named spectral/projector counterparts, keeping every original feature and
all denominator/undefined-comparator information. Full-spectrum entropy is not relabelled top-64
entropy. All 300 held-out predictions match independent full regression systems and fresh replay;
the complete adapter refuses absent primary inputs. Positive retrieval panels remain synthetic.
A real geometry-only check maps 27 fields from the existing nine diagnostic checkpoints, without
adding scores or primary identities. The old four-feature dimension bridge reproduces the same
null-feature numerical boundary; the new named helper fixes that component, but formal v3 dimension
export/interventions/publication are not yet integrated. These checks are not optimizer findings.
...
then integrate v3 functional-dimension/export/publication consumers, the separately routed
