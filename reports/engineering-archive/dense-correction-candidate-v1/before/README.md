...
The [new isolated GPU control](reports/engineering-archive/dense-canonical-replay-v1/README.md)
passes **9/9 short-input and 9/9 maximum-length gradient checks**, all 134 parameter tensors,
under the original tolerance. It also passes **24/24 strict same-process-group and 24/24
independent-process checkpoint replay checks**, preserving all later states, gradients, rows and
rank RNG. The full suite passes **1,367 JUnit cases**. The controlled reduction/backward policy
is a diagnostic intervention, not a formal-training/performance recommendation or an optimizer
quality finding. [Earlier failures and controls](reports/engineering-archive/dense-gpu-correctness-v1/README.md)
remain intact; the old primary matrix is not retroactively repaired.
...
unchanged. Next prepare the normalization/reference correction and a reviewed replication
transition. Do not resume a competing controller, use the obsolete zero-step migration or promote
...
unresolved; [issue #41](https://github.com/qcznlp/embedding-optimizer-study/issues/41) is the current
public progress handoff.
