...
## Preparation-only checkout — 2026-09-06

This detached temporary worktree is for a prospective numerical correction, not a live runtime.
Current authoritative status is `/root/embedding-optimizer-story-refactor/PROJECT_STATUS.md`.
The original 12-run matrix is complete/preserved but on scientific hold. Its exact BEIR handles
remain stopped; all prior checkpoint and default-path failure evidence must remain untouched.
Do not launch formal training/evaluation, alter the retained controller, commit/push WIP, delete HF
artifacts or interact with the protected GPU helper. The old zero-step migration is inapplicable.
Use new temporary diagnostic outputs and explicit source paths. No implementation incident belongs
in any part of the manuscript. Existing publication and safety gates below remain in force.
