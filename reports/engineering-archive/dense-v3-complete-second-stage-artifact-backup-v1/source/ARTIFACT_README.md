# Complete corrected DenseOn second-stage BEIR data snapshot

This immutable data-only snapshot contains all twelve declared optimizer/rate
configurations at checkpoint step 1563 (the second of five retained stages).
...
Six pool-B states / 84 task values overlap the earlier partial second-stage
...
The accepted source readback contains 36 checkpoint outcomes (all twelve at
steps 782, 1563 and 3907); this snapshot selects only the full twelve-state
step-1563 cohort using the declared configuration set, never scores.
