...
from embed_optim_restore.gate import validate
...
def values():
    ...
    reference = dict(
        warm_gradient_event_fingerprints=copy.deepcopy(local),
        warm_ddp_gradient_fingerprints=copy.deepcopy(post),
...
def test_any_boundary_difference_refuses_before_update(change):
    ...
    if change == "parameter_count":
        ...
        v[3][0] = 0
    elif change == "cold_local":
        v[2]["0"][0] = "different"
    elif change == "reference_local":
        v[0]["warm_gradient_event_fingerprints"]["0"][0] = "different"
    elif change == "post_ddp":
        v[5]["0"] = "different"
    elif change == "rng":
        v[7] = 456
...
@pytest.mark.parametrize(
    "field",
    [
        "same_weights_optimizer_scheduler",
        "same_first_input_rng",
