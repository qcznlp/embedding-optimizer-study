...
def validate(
    ...
    if (
        len(warm_local) != 134
        or len(warm_post_ddp) != 134
        or any(len(v) != 4 for v in warm_local.values())
    ):
    ...
    if (
        not reference["same_weights_optimizer_scheduler"]
        or not reference["same_first_input_rng"]
        or not reference["exact_token_features"]
        or reference["optimizer_updates"] != 0
    ):
        raise ValueError("Invalid paired reference conditions")
    recursive_equal(rng_after_cold, rng_after_warm, "post-pass-rng")
    return {
        "all_134_post_ddp_tensors_exact_to_reference": True,
        "all_536_local_contributions_exact_to_reference": True,
        "post_pass_rng_equal": True,
        "actual_token_features_equal": True,
        "new_tolerance": False,
    }
