...
def repeated_backward(trainer, raw_batches, rng, require_boundary):
    ...
    if len(raw_batches) != 4 or trainer.compute_loss_func is not None:
        raise ValueError("Require four fixed batches and the original explicit-loss dispatch")
    batches, item_count = trainer.get_batch_samples(
        iter(copy.deepcopy(raw_batches)), 4, trainer.args.device
    )
    if len(batches) != 4 or item_count is not None:
        raise ValueError("Require the original observed None item count for accumulation scaling")
    ...
    for index, batch in enumerate(batches):
        ...
        context = (
            nullcontext()
            if index == 3
            else trainer.accelerator.no_sync(model=trainer.model_wrapped)
        )
        with context:
