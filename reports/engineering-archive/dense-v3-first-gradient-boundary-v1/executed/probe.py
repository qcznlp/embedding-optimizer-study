"""Four-rank first-backward diagnosis; intentionally stops before clipping/update.

Original source/run/input admission and numerical training functions are unchanged.
Read-only feature/leaf hooks observe the first four resumed microbatches. A scoped
clip-boundary trap stops before calling clipping, saving the actual DDP gradients.
...
def prepare(args):
    ...
    if unit['tests'] != '10' or any(unit[k] != '0' for k in ('errors', 'failures', 'skipped')):
    ...
    r.write(HERE / 'authorization.json', dict(scope=SCOPE, owner_message=r.OWNER,
        source=identity(__file__), trace=identity(HERE / 'trace.py'),
        ...
        backward_microbatches_per_rank=4, optimizer_updates=0,
...
def authenticate(args, cpu=True):
    ...
    if (auth['scope'] != SCOPE or auth['source'] != identity(__file__)
            ...
            or auth['backward_microbatches_per_rank'] != 4 or auth['owner_message'] != r.OWNER):
...
def worker(args):
    ...
    try:
        ...
        expected_features = [trainer.collect_features(trainer.data_collator([dataset[i] for i in row]))[0]
                             for row in indices]
