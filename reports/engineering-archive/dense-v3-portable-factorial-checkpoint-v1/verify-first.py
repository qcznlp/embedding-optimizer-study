...
run('audit', ['/usr/bin/python', '-m', 'embed_optim.distribution_audit', '--repo-root', str(root), '--dist-dir', str(work / 'dist')])
...
runtime = work / 'formal_runtime.json'
shutil.copyfile(root / 'configs/formal_runtime.json', runtime)
for case in ('a', 'b'):
    output = work / ('read-' + case + '.json')
