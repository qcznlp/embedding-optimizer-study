...
COLLATOR_POLICY = {
    "class": "embed_optim.collators.DenseGroupCollator",
    "preprocess": "same_model_SentenceTransformer.preprocess",
    "prompts": {"query": "query: ", **{n: "document: " for n in TEXT_COLUMNS if n != "query"}},
}
CHECKPOINT_POLICY = {
    "class": "embed_optim.callbacks.FractionalCheckpointCallback",
...
def final_inventory(directory):
    """A final inference export is not itself a component checkpoint."""
    directory = Path(directory)
    excluded = directory / checkpoints.NAME
    if excluded.exists() or excluded.is_symlink():
        raise ValueError("Final export contains an undeclared component receipt")
    inventory = checkpoints.inventory(directory)
    if {r["path"] for r in inventory} != set(INFERENCE_FILES) | {"README.md", "training_args.bin"}:
...
def inspect_complete_run(root, binding, identity):
    ...
    if root.is_symlink() or not root.is_dir():
        raise ValueError("Require an ordinary complete-run directory")
    ...
    if set(completed) != {
        ...
        "dataset_rows",
        "checkpoints",
        "final_inference_files",
        "final_files",
        "trainer_state_final",
    }:
        raise ValueError("Incomplete or extended run completion receipt")
    ...
    if (
        ...
        or type(completed.get("optimizer_steps")) is not int
        or completed["optimizer_steps"] != 391
        or type(completed.get("dataset_rows")) is not int
        or completed["dataset_rows"] != 50000
    ):
    ...
    require_same(final_inventory(root / "final"), completed["final_files"])
    import torch
    ...
    require_final_state(read_json(root / "checkpoint-391/trainer_state.json"))
    require_same(
...
def read_complete_run(
    locations, calibration_directories, *, state, operator, seed, source_root, root, binding
):
    """Join actual input/calibration admission to the deep saved-run consumer.

    The external completion binding alone cannot select a source, calibrated LR
    or data identity. Both actual calibration chains are independently reread.
    Scheduling authority, GPU pilot acceptance and scientific publication remain
    separate; no such permission is inferred from a successful artifact read.
    """
    identity, _ = prepare_run(
        locations, calibration_directories, state, operator, seed, source_root
    )
    return {
        "scope": SCOPE,
        "input_calibration_and_run_verified": True,
        "scientific_admission": False,
        "execution_authorized": False,
        "run_identity": identity,
        "artifacts": inspect_complete_run(root, binding, identity),
    }
