...
def train_segment(config, fixture, output, trainer_kind, dropout, resume=None):
    ...
    del trainer, model
    gc.collect()
    return snapshot, {"argument_overrides": overrides, "dropout_modules": len(dropouts)}
...
def verify_replay(reference, resumed, checkpoint_step):
    ...
    expected["gradients"] = [x for x in reference["gradients"] if x["global_step"] >= checkpoint_step]
...
def main():
    ...
    required = {"CUDA_VISIBLE_DEVICES": "", "HF_HUB_OFFLINE": "1", "TRANSFORMERS_OFFLINE": "1", "WORLD_SIZE": "4"}
    if any(os.environ.get(k) != v for k, v in required.items()):
        raise ValueError("Require four offline CPU-only torchrun ranks")
    if work.parent != Path("/tmp") or not work.name.startswith("dense-trainer-resume.") or not work.is_dir():
    ...
    try:
        ...
        for kind in ("live", "candidate"):
            for dropout in (0.0, 0.1):
                for run_id in previous.RUN_IDS:
                    ...
                    reference, metadata = train_segment(configs[run_id], work / "tiny-model", case / "uninterrupted", kind, dropout)
                    if len(reference["trace"]) != 9 or len(reference["gradients"]) != 3:
                        raise ValueError("Incomplete baseline microbatch/update schedule")
                    gathered_ids = [None] * 4
                    dist.all_gather_object(gathered_ids, [i for x in reference["trace"] for i in x["row_ids"]])
                    ...
                    for step in (1, 2):
                        ...
                        resumed, _ = train_segment(configs[run_id], work / "tiny-model", case / f"resume-{step}", kind, dropout, checkpoint)
                        verify_replay(reference, resumed, step)
                        ranks = [None] * 4
                        dist.all_gather_object(ranks, {"rank": rank, "trace": resumed["trace"], "gradients": resumed["gradients"], "rng": resumed["rng"]})
                        checks.append({"resume_from_step": step, "exact_replay_on_all_ranks": True, "ranks": ranks})
                    if rank == 0:
                        report["records"].append({"trainer": kind, "dropout": dropout, "run_id": run_id, **metadata, "checks": checks})
                        print(f"Exact CPU Trainer resume verified: {kind}, dropout={dropout}, {run_id}", flush=True)
                    dist.barrier()
        ...
        if rank == 0:
            sources = [Path(__file__), Path(inspect.getsourcefile(previous)), Path(inspect.getsourcefile(normalization)), Path(inspect.getsourcefile(SingleNormalizationTrainerCandidate))]
            sources += [root / f"src/embed_optim/{name}.py" for name in ("train", "callbacks", "losses", "collators", "optimizers", "config")]
            report.update(observed_at_utc=datetime.now(UTC).isoformat(), source_bindings=[previous.identity(p.resolve()) for p in sources])
