...
DEPLOY = EXPERIMENT / 'launch/functional-features-recovery-v3'
OUTPUT = EXPERIMENT / 'analyses/dense-primary-v3-functional-features-recovery-v3'
...
def original():
    require_inputs()
    # Its existing proposal validator imports the adjacent source-bound layout
    # helper. Keep that exact directory available in both fresh CPU processes.
...
def prepare_parent(location, root):
    ...
    need(location.is_relative_to(root) and location != root and '..' not in location.parts
