# Actual primary training, not a short diagnostic

## Current handoff — 2026-09-10 04:34 UTC
...
## Historical milestones — current fifth-pair state above takes precedence

The second-pair children 1663679 / 1663704 finished at 21:02:39 / 21:06:05.
Their full completion receipts are `logs/<run>.view-verified.json`, explicitly
distinct from the old single-selection guard. Final W&B read **65067** is
terminal/pass: both finished, all 391 expected records through step 3900, finite
values and zero independently calculated LR error. Whole-run state proves 3907.
Initial current-pair W&B read **19048** is terminal/pass through actual endpoints
70 / 40; both are online, with exact cadence, finite values and zero LR error.

The unchanged CPU backup watcher **79029**, PID **1599515**, remains live.
All twenty-eight checkpoints passed immutable remote audits: **560 files /
42,242,488,755 logical bytes**. Second-pair final-stage commits are
`61218ad3c3569eb5a169cb58be8df879c7a531c8` (NorMuon 3e-4) and
`0e7fbc537a8fd8e69b2cda07ab8dd8f254868836` (AdamW 1e-5).
Earlier read-only receipt integration **85915** is terminal/pass at 21:10: every receipt,
identity, inventory and recorded hash matches; every new final-stage local file
was rehashed. This reads the original watcher's immutable network-audit receipts;
it is not another network or deep-tensor audit.

Exact commands, complete receipts, actual new worker identities and live-handle
observations are in
[the completed-pair transition](observations/second-pair-complete-third-pair-running-20260908.json).
Observation **210** is terminal with that actual transition verified; **205** and
all older observation cells are also closed. These are not training jobs.
The actual sessions **52244 / 97554 / 79029** remain live; do not duplicate them.

Post-warmup read **69593** is terminal/pass: 44 / 41 expected W&B rows through
actual endpoints 430 / 400, finite metrics and zero LR error. Observation **227**
is terminal with both workers past warmup, not a stopped training job. The 21:56
local read observes 442 / 417 steps with all four ranks admitted. Evidence:
[post-warmup observation](observations/third-pair-post-warmup-20260908.json).
That earlier warmup observation preceded the current pair's first checkpoints.

Both actual current step-782 deep-state reads **64977 / 46665** are terminal/pass:
134 parameter states, three groups [88,1,45], scheduler step 782, unchanged sealed
payloads and no CUDA context. Their immutable HF commits are
`96c42189cee58a52615f1fd834a17d40d2f8cd4c` (Muon 1e-3) and
`916108e06bf2eef4c4aac301dec71c7673ebcc5d` (NorMuon 1e-3).
Receipt integration **36953** is terminal/pass at 22:44 for all twenty-two exact
receipts and every new local file hash. The original watcher performed the network
audits; this integration did not repeat them. Post-save W&B read **32912** passes
all 84 / 82 expected rows through actual endpoints 830 / 810, with finite metrics
and zero LR error. Observation **233** is terminal with both first checkpoints
audited and training continuing; the actual training/backup sessions remain live.
Evidence: [first checkpoints](observations/third-pair-first-checkpoints-20260908.json).
These are completed single-stage checks, not complete current runs or findings.

Both current step-1563 checkpoints also passed immutable remote audits, at commits
`e41efce14ec736bb7061f9d68cee51a97c32a776` (Muon 1e-3) and
`cadfa256088822cbf383bfc5b1847107a7533619` (NorMuon 1e-3).
Receipt integration **33974** is terminal/pass at 00:16 for all twenty-four exact
receipts and every new local file hash. This reads the original watcher's network
audits, not another network or deep-tensor audit. Post-save W&B read **20932**
passes all 159 / 158 expected rows through actual endpoints 1580 / 1570, with
finite metrics and zero LR error. Observer **251** is terminal with both second
checkpoints audited; actual training/backup sessions remain live at 00:17.
Evidence: [second checkpoints](observations/third-pair-second-checkpoints-20260909.json).
No new deep-tensor read is claimed for this stage; the unchanged automatic full-run
reader reads all five stages on completion. Those second-stage checks are complete.

Both current step-2345 checkpoints passed immutable remote audits, at commits
`245fd8fd2ad6d70d3e073ae80a7b7afd1e2cddb4` (Muon 1e-3) and
`606cf33223e0b98d2098dd1b5b2653339d362035` (NorMuon 1e-3).
Receipt integration **64329** is terminal/pass at 01:49 for all twenty-six exact
receipts and every new local file hash. It reads the original watcher's network
audits; it is not another network or deep-tensor audit. Post-save W&B read
**41976** passes all 236 / 238 expected rows through actual endpoints 2350 / 2370,
with finite metrics and zero LR error. Observer **265** is terminal with both
third checkpoints audited; actual training/backup sessions remain live at 01:50.
Evidence: [third checkpoints](observations/third-pair-third-checkpoints-20260909.json).
Both current full runs remained incomplete; the next save at that observation was step 3126.

Both current step-3126 checkpoints passed immutable remote audits, at commits
`f4cd570f0f8af0e154756700a2813b07a0fbefcf` (Muon 1e-3) and
`aab2fac6abb3542e378601215bd86ceff59c6d3c` (NorMuon 1e-3).
Receipt integration **85587** is terminal/pass at 03:23 for all twenty-eight exact
receipts and every new local file hash. It reads the original watcher's network
audits; it is not another network or deep-tensor audit. Post-save W&B read
**34479** passes all 314 / 317 expected rows through actual endpoints 3130 / 3160,
with finite values and zero LR error. Observer **273** is terminal with both fourth
checkpoints audited; actual training/backup sessions remain live at the 03:23 handoff.
Evidence: [fourth checkpoints](observations/third-pair-fourth-checkpoints-20260909.json).
Continue current training toward final step 3907. Both full runs remain incomplete;
the existing coordinators own full-run checks and the next AdamW 1e-6 / 3e-6 dispatches.
Do not launch duplicates or restart a live coordinator at the eventual transition.

Follow [the active continuation handoff](view-history-continuation/RUNNING.md)
for remaining queue order, resolved W&B IDs, and immutable authority/proof bindings.
Keep both GPU lease namespaces protected and old stopped controllers untouched.
Never inspect or touch the protected helper. No training source, recipe, paper,
historical artifact, Git publication or external source release changed.
Training priority continues. No validation/BEIR or scientific completion is claimed.

## Historical initial-queue record — superseded above

Started September 8, 2026 at 05:14 UTC following the owner's explicit instruction
to train now and defer code handling. No local Git commit or repository publication
is claimed. The unchanged numerical core, complete source inventory and exact
data/base/recipe/run contracts remain enforced. The original DRAFT is unchanged.

## Initial owned queues (now terminal; do not restart)

| Pool | Tool session | Coordinator PID | First torchrun PID | First run |
| --- | --- | --- | --- | --- |
| 0–3 | 28273 | 1596840 | 1596862 | verified-v3-adamw-3e-5 |
| 4–7 | 55261 | 1596879 | 1596885 | verified-v3-muon-3e-4 |

At 11:34 UTC both sessions were live: AdamW had reached 3162/3907 steps and Muon
3164/3907. Both saved the first four scheduled stages and continued training normally.
The actual schedules contain 782, 1563, 2345, 3126 and 3907. Logged loss and
gradient values remain finite; no fatal log markers were observed. There are
zero completed full runs, two active runs and ten queued runs. The first eight of
sixty scheduled checkpoints are retained and remotely verified. These are
training-health and durability observations, not optimizer-quality findings.

Use the exact tool sessions for liveness, and `logs/<run_id>.log` for numerical
progress. The logs contain carriage-return progress bars; `tr '\r' '\n'` before
`tail` keeps output bounded. Do not restart a queue solely because a poll yields
no output. A live coordinator is silent while its torchrun child works. Individual
exit and deep-verification receipts are created only after actual termination.

The two six-run queues are fixed in authorization.json. Every optimizer has four
rates, and each pool has two runs per optimizer. A nonzero child exit or failed
complete-run check stops that queue without overwriting/restarting the failed run.
The other disjoint queue can continue. A continuation requires explicit inspection
of the actual failure and sealed checkpoints; this initial entry is fresh-run-only.

## Integrity and resource boundary

- Launcher SHA-256: dd823408de44d0696d0cac3a9d865ffc41936abdf2542ae815c3f5eda8dffcf3.
- Authorization SHA-256: 00000e5a49b510a1d2c2f9ce28ab8c0ce13929b3b2acdda551bf0356a6b8f01d.
- Source assembly SHA-256: e603ef175c0f691a1dd83c4d8b474d90da2fc67c8c65f5a77f3c3180e78332b8.
- The complete local source snapshot is `source-snapshot/`; the running source is
  `/root/embedding-optimizer-primary-v3`. Do not edit its 56 bound files or the
  running launcher/authorization. Future code cleanup is not in the launch path.
- Both common and primary GPU leases are held by the coordinator and inherited
  by torchrun. Existing lock payloads are not overwritten. Independent CPU checks
  verified descriptor inheritance, cross-process exclusion, refusal of a competing
  holder, and release/reacquisition before launch.
- Only the three old approved stopped dispatchers were inspected. They remain T;
  the old ledger digest remains 95c69483f60badddd3c13e175951bcb52682523589c62bf898a57eb786d4e4ef.
  No old controller, protected helper or historical artifact was changed.

The actual environment is pinned `/usr/bin/python`, Torch 2.9.1+cu129, eight L20Z
GPUs split 4+4, FP32 parameters and BF16 deterministic FlashAttention training.
W&B is online in stevezenguom/embedding-optimizer-study. HF data/model reads use
the verified local cache. There are no short-horizon overrides and no automatic
elastic restart. All 500K groups and all five complete checkpoints are retained.

Outputs: `../outputs/dense-correctness-v3/dense/<run_id>`.
Checkpoint remote durability and all subsequent evaluation/analysis/paper work
remain required; neither a started process nor a finite early loss completes them.

## Live checkpoint backup watcher

Session **79029**, PID **1599515**, is now watching all sixty declared stages.
Its source is `backup_checkpoints.py`, SHA-256
`5196ef2ceec1e7f679d670738911ad77056958af84a726ae212f5f90977d99f5`.
Do not edit or duplicate this live watcher. It runs with CUDA hidden, one CPU
thread and lower CPU scheduling priority; it cannot consume a training GPU.

The read-only HF preflight confirms the expected account and public model repo
`qcz/embedding-optimizer-study-checkpoints`. The new prefix is
`corrected-dense-correctness-v3/dense/4400f1ce26ef423873a4eb509db48e046a07b90159f6d5ef10da084f5171107b`;
it was empty at remote commit fdd851584951e60457a2a83bb0380a9c11b969d3.
That was the pre-upload observation. The first eight actual uploads have now
completed and passed independent remote audits; no historical prefix was changed.

| Run / stage | Verified immutable HF commit |
| --- | --- |
| verified-v3-adamw-3e-5 / 782 | 2ccdfe820bb37532dcc2d7cd29571d5b213440d6 |
| verified-v3-muon-3e-4 / 782 | 7caa05112da31e502d010bec0cf70a8cdb0d466e |
| verified-v3-adamw-3e-5 / 1563 | b5f71db03f0af85cd5a32bf1d954fecebc5385d5 |
| verified-v3-muon-3e-4 / 1563 | ef974ee635e9b876623628eefb1d1096260ce698 |
| verified-v3-adamw-3e-5 / 2345 | 80d080a6bca1209226463f1c9ff8962d6624c94b |
| verified-v3-muon-3e-4 / 2345 | 3b20ad40ac7cea5029b3058468381051a91ec7af |
| verified-v3-adamw-3e-5 / 3126 | f87de3ec587a660a64ed059506e26410016e5997 |
| verified-v3-muon-3e-4 / 3126 | f5cbbe5db2229a05199a505e7d82233a6a7ee82a |

Each has matching `.upload.json`, final `.json` and `.audit.json` records in
`backup-receipts/`. All eight independent audit results set `durability_verified=true`
and `scientific_completion=false`. The receipts cover 160 files / 12,572,051,160
logical bytes, including the model, optimizer, scheduler and four ranks' RNG
states for every stage. The watcher is still live as of 11:34 UTC;
do not launch another uploader. Monitoring cells 32, 37, 39, 40, 45, 46, 48, 49,
54, 55, 58, 59, 63, 64, 69, 70, 72, 73, 82, 84 and 85 are finished; these
were observation loops only, not training jobs. Continue monitoring the three
live sessions above, not those completed cells.

The latest verified W&B read-only check 45163 is terminal/exit 0 at 11:35 UTC: both runs
were online/running with 317 metric rows each through step 3160. Every declared
logging step (1, then each tenth step) is present exactly once. All metrics are
finite, with zero error against the independently calculated LR schedule. The
earlier checks 2329, 81826, 18377, 8407 and 27787 are also terminal. These are logging/training-health checks,
not retrieval findings. No GPU was used for them. The next saved stage is 3907;
keep all three live training/backup sessions running and do not repeat completed
monitors or start a duplicate queue.

## Resolved monitoring readback discrepancy

The 10:52 UTC page-size-250 monitor (52224, terminal) returned a false cadence
flag after the trace exceeded one page. Read-only diagnosis 50745 showed a
missing global step 1680 and duplicate step 2500 at the page boundary in both
runs. Bounded single-page and independent narrow read 53059 verified the complete
original trace through step 2800, including exactly one identical step-1680 row.
Changing only the monitoring query to page size 1000 then passed the original
strict cadence/finite/LR checks through step 2840 (27787, terminal). Do not sort,
deduplicate or fill records to make a coverage check pass. Use the verified
single-page query for this declared horizon and still test actual coverage.

All original and diagnostic outputs, plus the before/after read-only commands,
are preserved in `observations/wandb-readback-20260908T105218Z.json`, SHA-256
40237e9f49d3272cfa0c7d210cc6b4f8baa6219337cbd8f98a02afd0705c5547.
The evidence establishes page-size-dependent readback behaviour, not a specific
SDK/server root cause. No training record was shown lost or duplicated; no
training, launcher, backup watcher, optimizer or recipe was changed or stopped.
This is operational provenance only and must not enter the manuscript.

Each upload waits for the seal metadata to be stable across two 30-second polls,
then uses the unchanged complete checkpoint reader. It only adds files under the
new exact run/stage prefix, pins the remote parent, refuses any existing target,
retains the returned immutable commit, and compares every remote file's size and
SHA-256 or Git blob identity. The original read-only audit independently checks
the completed upload. Receipts appear under `backup-receipts/`. An upload receipt
without its final verification is not proof of durable completion. Remote/API
failures stop this watcher without stopping or modifying training; no blind
write retry or delete is attempted. Initial checks cover the exact sixty paths,
four invalid-scope refusals and an intentionally incorrect source-hash refusal.
