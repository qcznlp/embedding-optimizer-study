...
simulated upstream admission. No primary model was encoded; numerical reconstruction from the full
raw-vector archive and final publication remain pending. These are not optimizer findings. Choices are retained
...
The final isolated suite passes 2,381 cases; the new source/transport milestone adds 47 focused checks.
The inference parent's 64 checks and first failed independent-oracle attempt remain preserved. On this host the
...
[reproduction record](reports/engineering-archive/dense-v3-portable-reconstruction-v1/commands.md).
