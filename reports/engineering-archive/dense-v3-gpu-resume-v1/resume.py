"""Two real four-rank resumes, after the complete scientific GPU queues.

Restore the original native component from an anonymous immutable HF download,
then use its existing bound Trainer resume hooks. No original source, output,
worker receipt or numerical definition is changed. These are verification jobs,
...
OUTPUT = EXP / 'outputs/dense-v3-factorial-resume-verification-v1'
SCOPE = 'owner-authorized-two-genuine-factorial-gpu-resume-checks-v1'
...
def worker(args):
    ...
    downloaded = read(HERE / 'downloaded.json', admission['downloaded'])
    need(downloaded['authorization_sha256'] == args.authorization_sha, 'Wrong downloaded input authority')
    parent.handoff()
...
def coordinate(args):
    ...
    downloaded = read(HERE / 'downloaded.json', args.downloaded_sha)
    need(downloaded['authorization_sha256'] == args.authorization_sha, 'Wrong download authority')
    root = HERE / 'run' / ('pool-' + args.pool)
    no_existing(root)
    ...
    write(root / 'started.json', dict(**owned.process_identity(os.getpid()), started_at_utc=now(),
        source_sha256=args.source_sha, authorization_sha256=args.authorization_sha,
        downloaded=identity(HERE / 'downloaded.json'), pool=args.pool, gpu_leases_requested=False))
    children = []
    try:
        ...
        with ExitStack() as stack:
            ...
            write(root / 'admission.json', dict(coordinator_pid=os.getpid(), port=port,
                ...
                authorization_sha256=args.authorization_sha, downloaded=identity(HERE / 'downloaded.json'),
...
def main():
    ...
    p.add_argument('mode', choices=['prepare', 'download', 'coordinate', 'worker', 'compare'])
    ...
    need(args.mode in ('prepare', 'download') or args.pool is not None, 'Missing exact pool')
