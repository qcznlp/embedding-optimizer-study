...
The [latest real-data preflight](reports/engineering-archive/dense-natural-readiness-v1/README.md)
...
all match their original ledgers and train/validation queries remain disjoint. The empty texts
trace to three authenticated upstream documents. Original data is unchanged; a uniform,
seeded train/validation revision is proposed, not executed. This is data hygiene, not an
optimizer finding, and does not replace the separate numerical scientific hold.
...
paper/audit checkout passes **all 1,597 cases**, including 59 focused data-audit checks. Passing
tests do not make the observed empty-text inputs eligible for the natural-data preflight.
...
review the [six-group common-data amendment](reports/engineering-archive/dense-natural-readiness-v1/data-revision-proposal.md),
then prepare natural-data execution readiness and integrate validation selection and downstream scientific consumers,
and the evidence-preserving runtime handoff. Resolve the existing deployment/publication gates. Do not resume
