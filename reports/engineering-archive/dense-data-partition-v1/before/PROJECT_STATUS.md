...
Last updated: **2026-09-06 14:32 UTC**; the machine-readable snapshot is refreshed at this handoff.

## Where the project stands

**The real-data preflight stopped before GPU launch: five training groups and one validation group contain empty document text. The formal matrix remains on scientific hold.**
...
The [latest real-data audit](reports/engineering-archive/dense-natural-readiness-v1/README.md)
...
four negatives. The frozen 4,096-query validation set retains zero training-query overlap and
...
No GPU worker or new training run occurred in this milestone.

Next implement/review the [uniform data-revision proposal](reports/engineering-archive/dense-natural-readiness-v1/data-revision-proposal.md):
preserve eligible rows and quotas, replace the six whole groups through declared source-specific
seeded order, and protect both training/validation and all other declared held-out partitions.
No replacement query or new dataset has yet been produced. The current v2 proposal binds the
old data and remains unready; do not simply change its expected hashes or erase old evidence.
...
| Isolated paper/audit suite | 1,597 cases, zero failures/errors/skips; 59 focused data-audit checks pass | Text eligibility of actual inputs or correctness of the different candidate source tree |
...
1,597 cases / zero failures. The older candidate's 923-case receipt remains valid historical evidence.
...
2. First implement/review the six-group common-data revision linked above. Re-audit both full
   datasets, preserve all valid row contents/quotas and prevent newly selected training queries
   from entering held-out partitions. Preserve the failed prefix; do not substitute another
   prefix or weaken the nonempty-text check. No revised data exists yet. Then prepare declared
   seven-source natural-data execution readiness and integrate the new identity/grid with validation
   selection, outcome, weight-space/dimension and publication consumers. Prepare an
...
- [Latest real-data failure, full-text/ID/holdout checks and source origins](reports/engineering-archive/dense-natural-readiness-v1/README.md);
  [proposed uniform data amendment, not executed](reports/engineering-archive/dense-natural-readiness-v1/data-revision-proposal.md).
