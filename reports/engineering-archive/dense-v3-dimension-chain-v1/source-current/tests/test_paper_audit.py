...
def test_current_dense_paper_constants_match_strict_sources(checked_in_dense_audit):
    ...
    # The current README intentionally withholds the historical final conclusion.
    # Its old outcome manifest must fail closed, not force held claims back into
    # the public entry point to satisfy a legacy positive fixture.
    assert result["incomplete_evidence"] == ["ConfirmationHeadline", "InterventionHeadline"]
    assert "FINAL_CONCLUSION_PENDING" in (REPOSITORY / "README.md").read_text()
