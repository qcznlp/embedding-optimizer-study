...
def validate(args):
    ...
    cli_path = archive / "cli.json"
    assert (
        file_identity(cli_path)["sha256"]
        == "226b96ee21c94a983d096035afbba03f7886e9de972f45daec98d4b80cdd1800"
    )
    cli = read_json(cli_path)
    assert cli["passed"] is True and cli["scientific_completion"] is False
    assert {(row["module"], row["action"]) for row in cli["records"]} == {
    ...
    for row in cli["records"]:
        ...
        assert row["expected_boundary"] in row["stderr"]
        assert not Path(row["command"][row["command"].index("--output") + 1]).exists()
    for row in cli["sources"]:
        verify_file(root / row["path"], row)
    verify_file(Path(cli["protocol"]["path"]), cli["protocol"])
    ...
    return {
        ...
        "real_primary_cli_refusals_verified": True,
