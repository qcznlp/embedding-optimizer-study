...
def saved_state_fingerprints(checkpoint):
    with TensorStore(Path(checkpoint)) as store:
        ...
        for name in store.keys():
            ...
            # The saved Transformer has bare names; SentenceTransformer owns it
            # under this exact module path. Unexpected extra modules still fail.
            result[MODEL_KEY_PREFIX + name] = _tensor_record(value)
        return result
