...
def model_fixture(tmp_path):
    ...
    model = SimpleNamespace(
        ...
        state_dict=lambda: {"0.auto_model.weight": parameter},
    )
...
def test_observed_loading_exact_prefix_and_bf16_conversion(tmp_path):
    ...
    assert list(value["state_tensors"]) == ["0.auto_model.weight"]
    assert value["state_tensors_sha256"] == digest(value["state_tensors"])


@pytest.mark.parametrize(
    "kind", ["training", "length", "flatten", "fp32", "weight", "extra", "bare"]
)
def test_loaded_model_mismatch_refuses(tmp_path, kind):
    ...
    if kind == "training":
        ...
        model.state_dict = lambda: {"0.auto_model.weight": parameter, "extra": parameter}
    else:
