...
def validate(args):
    ...
    assert smoke["passed"] is True and smoke["scientific_completion"] is False
    for row in smoke["sources"]:
        path = Path(row["path"])
        snapshot = (
            archive / "source-initial" / path.relative_to(root)
            if path.is_relative_to(root)
            else archive / "first-smoke/run.py"
        )
        verify_file(snapshot, row)
...
def finish(args, root, archive, previous, preserved, records, tests, failures, before):
    ...
    failed_tests = Path("/tmp/dense-v3-outcome-failed-tests.zOfRNh")
    for label, anchor in (
        ("focused", "8a7cebeb79a5f8277f85268eaa51702896fee4ca8e2d0f8980fa5c44f85808bb"),
        ("full", "9f97e92a42650aa2e69eb5d46393ba225f1c267deee6a1c4192cbc70d5496ce9"),
    ):
        files.inspect(failed_tests / (label + "-fixture/fixture/archive"), anchor)
    for retained_root in (
        Path("/tmp/dense-v3-outcome-reconstruction-cold.EDmj0q"),
        failed_tests,
    ):
