...
At **2026-09-13 02:10 UTC**, **all 12/12 crossed continuations are complete**;
...
BEIR is **18/168**: twelve ClimateFEVER, four FEVER and both SciFact pilots have
completed. Eight exact full-corpus workers are active on FEVER at 02:09:29;
all six downstream waiters are live at 02:10:02, with no failure or actual
downstream result yet. The [latest recovery/observation archive](reports/engineering-archive/dense-v3-packaged-input-relocation-v1/README.md)
also records a genuine unchanged-wheel input read: 101 bound files / 5.63 GB,
both original source states and actual exit-zero CPU receipts. The original
native guards are retained; this component check is not full distribution
admission, GPU resume or a physical second-host proof. The ten original
distribution findings remain failed. Existing queues and scientific inputs
are unchanged; no optional experiment is added.
The [publication-access clarification](reports/engineering-archive/dense-v3-source-publication-access-v1/README.md)
distinguishes the original issue-comment/update 403 from untested code-push
permissions. Current read access/account metadata succeed, but application write
recovery is not verified. A non-blocking owner question is pending; no permission,
credential or remote content changed. The inspected remote snapshot is historical,
not the current corrected v3 deliverable; final source-release gates still apply.
The [closest-work review](reports/paper-review/dense-v3-literature-positioning-v1/README.md)
adds six missing references and a separately compiled Related Work candidate.
It distinguishes prior spectral/dimension findings from this study's actual
trajectory and prediction tests; it is not a priority claim or installed paper.
The [current recovery-package work](reports/engineering-archive/dense-v3-distribution-recovery-surface-v1/README.md)
adds missing v3 configurations, restore scripts and guides to the distribution.
Actual builds and a complete sdist-to-wheel payload round trip pass; the original
full distribution audit still has ten producer-path findings. This is not a
published or fully admitted source release, and no live experiment was modified.
The [complete training-log inventory](reports/engineering-archive/dense-v3-final-tracking-audit-v1/README.md)
independently verifies all **24 native/W&B run histories: 5,172 rows / 15,516
scalar values**, exactly equal with the original learning-rate schedules.
Missing online continuation research fields and tiny serialized argument-rate
differences are explicitly preserved alongside the full native recipes. No
online metadata was rewritten; this is tracking verification, not a new finding.
Both [five-stage probe queues](reports/engineering-archive/dense-v3-complete-document-handoff-v1/README.md)
are complete: **60/60** new checkpoint probes, one verified reused reference,
and actual exit-zero coordinator results. Their released GPU slots joined BEIR.
All [continuation probe payloads](docs/continuation-probe-restoration.md) are now
durable: **638 files / 586 MB** were anonymously downloaded and rehashed from
an immutable public revision. Copied-source CPU replay exactly reproduces all
**61 score matrices and 5,490 metric values**; the recovery script also indexes
all sixty continuation checkpoint locations. These are shortlist metrics, not
the still-pending full-corpus factorial effects.
...
The [complete-outcome backup](reports/engineering-archive/dense-v3-factorial-outcome-durability-v1/README.md)
is queued immediately after that native summary: all 168 raw task results,
provenance and six statistical/probe tables will receive an additions-only HF
snapshot and actual anonymous-download verification. No complete result snapshot
has been uploaded yet; this does not release executable source or the paper.
...
The [complete primary numerical replay](docs/paper-results-reproduction.md)
now recomputes this entire result graph from a closed source/data directory:
all ten outcome tables, fourteen weight predictors, nine functional table
families, all 108 controls, seventeen publication files and both sixty-state
figures match exactly. No original producer fallback or GPU was used. This
completes primary numerical portability, not the pending factorial/full-paper
reconstruction or source release.
The [complete current-document consumer](reports/engineering-archive/dense-v3-complete-document-handoff-v1/README.md)
is now queued behind actual factorial inference. It binds the full input/figure/table
inventory and clean constants and preserves all strict abstract/page/font checks.
Its test-only synthetic layout controls are not experimental findings or a final paper.
The [combined-paper numerical/PDF replay](reports/engineering-archive/dense-v3-combined-paper-replay-handoff-v1/README.md)
is now also queued behind that real native paper. It will reconstruct primary
and factorial results from copied source/data and verify the freshly compiled
full document. No actual combined replay has completed yet; this does not add
GPU work or change the scientific rules.
...
The [post-result cosine-sensitivity analysis](reports/paper-review/dense-v3-cosine-sensitivity-v1/README.md)
now clarifies the positive degrading-mass association using all 61 states and an
independent readback from restored HF vectors. It explains a property of the
measurement, not Muon's retrieval advantage; all original results remain unchanged.
Its citation/prose corrections are saved for final integration, not installed.
...
for the active evaluation/downstream entries; do not restart completed work.
...
- [All continuation probes and sixty additional checkpoint locations](docs/continuation-probe-restoration.md)
