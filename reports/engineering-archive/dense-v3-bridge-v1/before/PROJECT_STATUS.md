...
Last updated: **2026-09-06 21:40 UTC**; the artifact-only snapshot was refreshed at 21:39:35 UTC.

## Where the project stands

**Prepared data/core/validation/outcome and exact/approximate geometry consumers pass their bounded checks. Complete exact geometry is independently verified on all hidden matrices of nine diagnostic checkpoints. A new synthetic counterexample confirms a numerical false-positive boundary in the unchanged retrieval bridge; repair and integration of that bridge, dimensions/publication and formal deployment remain incomplete. The old matrix stays on scientific hold.**
...
one pathology, not nine independent failures. **The bridge issue is confirmed, not repaired.**
Declare and test numerical identifiability/zero-residual handling before reusing the bridge on real
outcomes, then integrate its v3 inputs and separately declared exact-measurement sensitivity.
No bridge solver, threshold, feature or old protocol was changed to make this diagnostic pass.
...
| Isolated paper/audit suite | 2,051 cases, zero failures/errors/skips with absolute source paths; old failed attempts preserved | Correctness of the different numerical candidate source tree, unfixed bridge boundary or formal scientific acceptance |
...
2,051 cases / zero failures. The older candidate's 923-case receipt remains valid historical evidence.
The newly confirmed bridge null-feature issue has not yet been repaired or added as a passing regression.
...
2. The expanded six-training/52-validation-group revision is now materialized and independently
   ...
   consumer now passes all-hidden diagnostic verification and full fresh replay. First fix the
   newly confirmed bridge null-feature identifiability/zero-residual boundary under an explicit
   amendment with independent null, near-null and real-signal controls. Then integrate the v3
   bridge/exact sensitivity, dimension and publication consumers. Neither
...
- [Complete exact-geometry consumer, all-hidden independent checks and fresh replay](reports/engineering-archive/dense-v3-exact-geometry-v1/README.md);
  [new uncorrected bridge null-feature counterexample](reports/engineering-archive/dense-v3-exact-geometry-v1/bridge-null-feature.json);
