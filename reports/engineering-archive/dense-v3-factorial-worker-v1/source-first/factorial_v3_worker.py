...
def request(
    locations, calibration_directories, state, operator, seed, source_root,
    output_root, record_root, *, project, entity, expected_worker_source,
):
    ...
    protected = [
        source_root, output_root, locations.primary_source, locations.data_store,
        locations.evidence, *(Path(v["path"]) for v in calibrated.values()),
...
def _require_created(trainer, creation, declared, sources):
    ...
    identity = contract.require_identity(trainer._bound_run)
    require_same(
    ...
    require_same(
        identity["sources"],
        {k: v for k, v in sources.items() if not k.endswith(("factorial_v3_factory.py", "factorial_v3_worker.py"))},
    )
    config, arguments = factory.recipe(
        identity, Path(declared["locations"]["data_store"]) / factory.BRANCH,
    ...
    if (
        ...
        or any(creation[k] is not False for k in (
            "scientific_admission", "execution_authorized", "training_executed",
            "source_release_or_resource_gate_waived",
        ))
        or creation["loaded_model"]["diagnostic_cpu"] is not False
        or creation["loaded_model"]["forward_or_backward_executed"] is not False
        or Path(trainer.args.output_dir) != Path(declared["run_root"])
    ):
    ...
    if (
        trainer.state.global_step != 0 or trainer.state.epoch not in (None, 0, 0.0)
        or trainer.optimizer is not None or trainer.lr_scheduler is not None
        or trainer._resume_binding is not None or trainer._resume_step is not None
...
def _rank_result(rank, result, trainer):
    ...
    return {
        "rank": rank, "global_step": result.global_step,
        "training_loss": result.training_loss, "metrics": dict(result.metrics),
    }
...
def _require_rank_results(rows):
    ...
    for rank, row in enumerate(rows):
        if (
            ...
            or type(row["global_step"]) is not int or row["global_step"] != 391
...
def _read_native(locations, declared, binding):
    return contract.read_complete_run(
        locations, declared["calibrations"], state=declared["state"],
        operator=declared["operator"], seed=declared["seed"],
        source_root=declared["source_root"], root=declared["run_root"], binding=binding,
    )
...
def run_branch(
    ...
    declared = collective_phase(lambda: request(
        locations, calibration_directories, state, operator, seed, source_root,
        output_root, record_root, project=project, entity=entity,
        expected_worker_source=expected_worker_source,
    ))
    sources = collective_phase(lambda: require_sources(locations, source_root, expected_worker_source))
    ...
    def start():
        if rank == 0:
            ...
            _write_new(records / STARTED, {
                "scope": SCOPE, "status": "started", "started_at_utc": _utc(),
                "request": declared, "sources": sources,
    ...
    try:
        trainer, creation = factory.prepare_trainer(
            locations, declared["calibrations"], state, operator, seed, source_root,
            output_root, project=project, entity=entity,
        )
        ...
        collective_phase(lambda: _write_new(records / "factory.json", {
            "creation": creation, "component_identity": component,
        }) if rank == 0 else None)
        ...
        def finish():
            ...
            require_same(native["run_identity"], component["bound_factorial_run"])
            completed = {
                "scope": SCOPE, "status": "training_and_native_readback_complete",
                "completed_at_utc": _utc(), "request": declared,
                "started": file_identity(records / STARTED),
                "factory_record": file_identity(records / "factory.json"),
                "native_run_completion": _binding(binding), "native_readback": native,
                "rank_results": results, "training_executed": True,
                "process_exit_observed": False, "scientific_admission": False,
                "source_release_or_resource_gate_waived": False,
        ...
        if not failure.exists():
            try:
                _write_new(failure, {
                    "scope": SCOPE, "status": "failed", "rank": rank,
                    "phase": phase, "observed_at_utc": _utc(),
                    "exception_type": type(exc).__name__, "scientific_admission": False,
                })
            except OSError:
                pass
        raise
...
def read_execution(locations, declared, binding):
    ...
    """
    expected = request(
        locations, declared["calibrations"], declared["state"], declared["operator"],
        declared["seed"], declared["source_root"], declared["output_root"],
    ...
    if (
        ...
        or any(complete[k] is not False for k in (
            "process_exit_observed", "scientific_admission",
            "source_release_or_resource_gate_waived", "independent_fresh_process_readback",
        ))
    ):
        raise ValueError("Not a complete bounded worker execution record")
    if {p.name for p in root.iterdir()} != {STARTED, "factory.json", COMPLETE}:
    ...
    if (
        started["scope"] != SCOPE or started["status"] != "started"
    ...
    contract.require_component(created["component_identity"], identity)
    if created["creation"]["run_identity_sha256"] != digest(identity):
        raise ValueError("Worker factory and completed native run have different identities")
    require_same(
        created["creation"]["factory_source"], sources["src/embed_optim/factorial_v3_factory.py"],
    )
    verify_file(root / COMPLETE, binding)
    return {
        "scope": SCOPE, "worker_and_native_artifacts_verified": True,
        "worker_completion": dict(binding), "native_readback": native,
        "scientific_admission": False, "execution_authorized": False,
        "process_exit_observed": False, "training_reexecuted": False,
    }
