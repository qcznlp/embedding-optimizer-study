...
def parameters(tmp_path, state="adamw_state", operator="adamw", seed=314159):
    ...
    return (
        ...
        SOURCE,
        tmp_path / "runs",
        tmp_path / "worker-records",
    ), {
        "project": "worker-synthetic-fixture",
...
def created_fixture(declared):
    ...
    config, arguments = worker.factory.recipe(
        identity,
        Path(declared["locations"]["data_store"]) / worker.factory.BRANCH,
        declared["output_root"],
        project=declared["project"],
        entity=declared["entity"],
    )
    creation = {
        "scope": worker.factory.SCOPE,
...
def mocked_worker(monkeypatch, tmp_path, state="adamw_state", operator="adamw", seed=314159):
    ...
    def native(locations, request, binding):
        ...
        # Synthetic consumer output, not an actual native-reader invocation.
        return {
            "scope": worker.contract.SCOPE,
            "input_calibration_and_run_verified": True,
            "scientific_admission": False,
            "execution_authorized": False,
            "run_identity": trainer._bound_run,
            "artifacts": {
                "scope": worker.contract.SCOPE,
                "whole_run_artifacts_verified": True,
                "scientific_admission": False,
                "run_identity_sha256": digest(trainer._bound_run),
                "optimizer_steps": 391,
                "dataset_rows": 50000,
                "checkpoints": [{"step": step} for step in worker.contract.STEPS],
            },
        }
...
def test_mocked_complete_lifecycle_and_fresh_reader(monkeypatch, tmp_path, state, operator, seed):
    args, kwargs, declared, _, _, _, calls = mocked_worker(
        monkeypatch, tmp_path, state, operator, seed
    )
...
def test_actual_cpu_call_refuses_before_any_write_or_factory(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        worker.factory, "prepare_trainer", lambda *a, **k: pytest.fail("early factory")
    )
...
@pytest.mark.parametrize(
    "problem",
    (
        "changed_factory_recipe",
        "changed_factory_arguments",
        "extra_failure",
        "claimed_exit",
        "changed_native",
        "changed_rank",
        "missing_calibration_on_real_reader",
        "changed_factory_model",
        "reversed_time",
        "extra_field",
        "changed_start_authority",
    ),
)
def test_fresh_reader_controls_even_after_rehash(monkeypatch, tmp_path, problem):
    ...
    if problem.startswith("changed_factory_"):
        ...
        elif problem == "changed_factory_arguments":
            value["creation"]["requested_arguments"]["max_steps"] = 10
        else:
            value["creation"]["loaded_model"]["maximum_context"] = 512
        ...
        complete["native_readback"]["artifacts"]["optimizer_steps"] = 390
    elif problem == "changed_rank":
        complete["rank_results"][0]["global_step"] = 300
    elif problem == "reversed_time":
        complete["completed_at_utc"] = "2000-01-01T00:00:00+00:00"
    elif problem == "extra_field":
        complete["gpu_admission_passed"] = True
    elif problem == "changed_start_authority":
        started = read_json(records / worker.STARTED)
        started["execution_authority_checked_by_this_component"] = True
        replace_json(records / worker.STARTED, started)
        complete["started"] = file_identity(records / worker.STARTED)
    else:
