...
78-update check as missing work. Portable adapter/source delivery remains.
...
1. Integrate the completed scientific consumers and explicitly resolve factorial source
   ...
   New actual publication-context read 20618 / ff7df0 / exit 1 fails at config.py.
   A complete census finds exactly config.py and optimizers.py differ in the old
   56/60-file publication source sets after genuine primary-source integration.
   Preserve those failures and old protocols; implement explicit versioned consumers,
   not hash replacement or reversal of the verified numerical source.
...
3. Integrate the proven restoration-only scalar-counter/reducer procedure into an
   explicit portable entry. Both original exact checks pass; no new GPU repetition
   is required merely to restate that evidence. Retain bounded coverage and original outputs.
