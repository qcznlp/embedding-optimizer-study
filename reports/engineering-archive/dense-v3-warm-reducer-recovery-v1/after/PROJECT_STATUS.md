...
checks are complete; portable recovery/source delivery remains. Read [CURRENT_EXPERIMENT.md](CURRENT_EXPERIMENT.md)
...
1. **Complete scientific-consumer/factorial source integration.** Primary numerics and all
   ...
   A new actual publication-context read fails at config.py; a complete census finds
   exactly config.py and optimizers.py differ in both historical 56/60-file publication
   source sets. Those now contain the verified primary-training versions. Retain the
   existing complete multi-source numerical replay and use an explicit version transition.
...
3. **Portable training recovery.** Both earlier device-only attempts reach step 391, but both exact
   ...
   cases; a portable scoped restoration entry remains. Do not repeat successful diagnostics
