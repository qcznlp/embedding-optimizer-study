...
## Active primary study and manuscript

This section defines completion for the current paper. The older checklist below is retained for
historical reproducibility only: its completed discovery and follow-up outputs cannot substitute
for the current primary matrix. Read `PROJECT_STATUS.md` and `AGENTS.md` before running commands.
