...
import pytest
import torch
from trace import Capture, cpu_tree, fingerprint, require_stop_boundary
...
def test_hooks_observe_but_never_replace_or_mutate_inputs_and_gradients():
    ...
    assert torch.equal(capture.local_sum['weight'], torch.tensor([6., -6.]))
...
def test_wrong_stop_boundary_refused(step, raw, accumulation):
    trainer = SimpleNamespace(state=SimpleNamespace(global_step=step),
...
def test_cpu_copy_is_independent_and_recursive():
    value = {'a': [torch.tensor([1.])], 'b': (None, 3)}
