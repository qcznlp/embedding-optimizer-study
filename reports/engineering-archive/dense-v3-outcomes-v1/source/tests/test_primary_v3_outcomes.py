...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("implementation_revision", "statistical_kernel_changed"), True),
    ],
)
...
def test_csv_replay_survives_json_key_reordering(primary, fixture, tmp_path):
    rows, selection = fixture
    original = outcome.outcome_tables(primary, rows, selection)
    evidence = {"score_rows": rows, "selection": selection}
    plan = {"scope": "engineering_fixture", "scientific_completion": False}
    target = tmp_path / "fixture-bundle"
    outcome.save_bundle(target, plan, evidence, original)
    reloaded = json.loads(json.dumps(evidence, sort_keys=True))
    fresh = outcome.outcome_tables(primary, reloaded["score_rows"], reloaded["selection"])
    assert outcome.inspect_bundle(target, plan, reloaded, fresh)["recomputed_bytes_verified"]
    assert outcome.csv_bytes([{"b": 2, "a": 1}]) == outcome.csv_bytes([{"a": 1, "b": 2}])
