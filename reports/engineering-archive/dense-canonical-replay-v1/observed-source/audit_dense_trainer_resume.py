...
def train_segment(config, fixture, output, trainer_kind, dropout, resume=None, canonical_cpu=False):
    ...
    def observe_clip(parameters, max_norm, norm_type=2):
        ...
        step = trainer.state.global_step
        gradient_values[step] = {"raw": [p.grad.detach().clone() for p in parameters]}
        result = original_clip(parameters, max_norm, norm_type)
        gradient_values[step]["clipped"] = [p.grad.detach().clone() for p in parameters]
...
def verify_replay(reference, resumed, checkpoint_step):
    ...
    for key in ("entry_states", "gradient_values"):
        expected[key] = {k: v for k, v in reference[key].items() if k >= checkpoint_step}
    assert_exact_tree(expected, resumed)


def measure_tensor_tree(left, right, name="state"):
    """Measure differences without relaxing the failed bitwise-replay requirement."""
    if type(left) is not type(right):
        raise AssertionError(f"{name}: mismatched type")
    if isinstance(left, torch.Tensor):
        if left.shape != right.shape or left.dtype != right.dtype:
            raise AssertionError(f"{name}: mismatched tensor topology")
        ...
        return [
            {
                ...
                "relative_l2_error": float(
                    difference.norm() / right.double().norm().clamp_min(1e-30)
                ),
                "within_prior_gradient_tolerances": torch.allclose(
                    left,
                    right,
    ...
    if isinstance(left, dict):
        if left.keys() != right.keys():
            raise AssertionError(f"{name}: mismatched keys")
        return [
            r for key in left for r in measure_tensor_tree(left[key], right[key], f"{name}/{key}")
        ]
    if isinstance(left, (tuple, list)):
        ...
        return [
            ...
            for r in measure_tensor_tree(a, b, f"{name}/{i}")
        ]
    assert_exact_tree(left, right, name)
    return []


def measure_replay(reference, resumed, checkpoint_step):
    # State loaded before the first replayed microbatch must be EXACT, even if
    # subsequent fresh-DDP floating-point reduction order differs.
    assert_exact_tree(
        reference["entry_states"][checkpoint_step],
        resumed["entry_states"][checkpoint_step],
        "restored_entry",
    )
    for key in ("scheduler", "rng", "global_step", "epoch"):
        assert_exact_tree(reference[key], resumed[key], key)
    expected_trace = [x for x in reference["trace"] if x["global_step"] >= checkpoint_step]

    def trace_without_loss(rows):
        return [{k: v for k, v in row.items() if k != "loss"} for row in rows]

    assert_exact_tree(
        trace_without_loss(expected_trace), trace_without_loss(resumed["trace"]), "row_rng_trace"
    )
    expected_gradients = [x for x in reference["gradients"] if x["global_step"] >= checkpoint_step]

    def schedule_only(rows):
        return [{k: row[k] for k in ("global_step", "learning_rates")} for row in rows]

    assert_exact_tree(
        schedule_only(expected_gradients), schedule_only(resumed["gradients"]), "schedule"
    )
    tensors = measure_tensor_tree(reference["weights"], resumed["weights"], "final_weights")
    tensors += measure_tensor_tree(reference["optimizer"], resumed["optimizer"], "final_optimizer")
    expected_values = {
        k: v for k, v in reference["gradient_values"].items() if k >= checkpoint_step
    }
    tensors += measure_tensor_tree(expected_values, resumed["gradient_values"], "gradients")
    strict = {"passed": True}
    try:
        verify_replay(reference, resumed, checkpoint_step)
    except AssertionError as exc:
        strict = {"passed": False, "first_mismatch": str(exc)}
    return {
        "bitwise_replay": strict,
        "exact_loaded_entry_state": True,
        "exact_row_order_and_rank_rng": True,
        "exact_scheduler_and_final_progress": True,
        "max_loss_error": max(
            abs(a["loss"] - b["loss"])
            for a, b in zip(expected_trace, resumed["trace"], strict=True)
        ),
        "tensor_differences": tensors,
        "all_tensors_within_prior_gradient_tolerances": all(
            t["within_prior_gradient_tolerances"] for t in tensors
        ),
    }
...
def main():
    ...
    parser.add_argument("--collect-numerical-differences", action="store_true")
    ...
    report = {
        ...
        "collect_numerical_differences": args.collect_numerical_differences,
        "prior_gradient_tolerances": previous.TOLERANCES,
        "records": [],
            ...
            for dropout in (0.0, 0.1):
                for run_id in previous.RUN_IDS:
                    ...
                    for step in (1, 2):
                        ...
                        measured = measure_replay(reference, resumed, step)
                        if not args.collect_numerical_differences:
                        ...
                        checks.append(
                            {
                                ...
                                "exact_replay_on_all_ranks": all(
                                    r["comparison"]["bitwise_replay"]["passed"] for r in ranks
                                ),
                    ...
                    if rank == 0:
                        ...
                        print(
                            f"CPU Trainer resume measured: {kind}, dropout={dropout}, {run_id}",
                            flush=True,
                        )
                    dist.barrier()
        report["audit_executed_completely"] = True
        report["complete_for_cpu_fixture"] = all(
            c["exact_replay_on_all_ranks"] for r in report["records"] for c in r["checks"]
        )
    ...
    return 0 if report["complete_for_cpu_fixture"] else 1
