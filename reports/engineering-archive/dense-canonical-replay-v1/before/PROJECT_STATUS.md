...
Last updated: 2026-09-06 07:39 UTC (artifact snapshot refreshed during handoff)
...
## Current result: first-gradient origin explained on the fixture

The [paired GPU origin probe](reports/engineering-archive/dense-gradient-origin-v1/README.md)
shows that local backward gradients and local accumulated sums are identical in uninterrupted and
resumed training. Actual DDP uses **16 versus one gradient bucket** at the first replayed update.
Reconstructing these exact layouts from captured local sums reproduces both reduced gradients;
holding the sums fixed and changing only the layout exactly reproduces their difference on every
rank. Default and deterministic-backward variants agree, and both match the preceding capture
without tensor-observation hooks. The maximum raw difference is 1.1920929e-7 in both variants.

This closes the specific "unknown first-gradient source" question: it is bucket-dependent
floating-point reduction on this short-input fixture. Together with the preceding exact optimizer
replay, it explains this numerical propagation chain. It is **not** a new serialization/optimizer
bug, retrieval mechanism, independent replication or harmlessness guarantee. The separate true
double-normalization defect in unchanged production remains the decisive scientific hold.

Both parent campaigns retain their strict full-replay failures (0/16 rank checks pass), with no
threshold changes. All **1,344 JUnit regression cases pass**. Eight local-sum payloads total
9,537,539,384 bytes and are digest-bound in temporary storage, not on HF. Original receipts and
all 64 bindings from the preceding validation are preserved.

The next bounded task is an isolated fixed-order reduction control with deterministic backward,
followed by fresh-process checkpoint replay. It must verify the same global-average objective and
must not replace a failed default-path receipt. No formal deployment/retraining or WIP source push
is authorized by this diagnostic. The matrix is still **12/12 executed, 60/60 preserved**, not
scientifically accepted; BEIR's exact retained dispatch chain remains paused. All GPU diagnostics
have exited and live numerical source/checkpoint/ledger bytes are unchanged.

## Current result: replay update propagation localized

The [new GPU localization](reports/engineering-archive/dense-gpu-replay-localization-v1/README.md)
reproduces the first post-resume optimizer-plus-scheduler update exactly from the observed entry
state and captured gradient: **24/24 reproductions**, plus **12/12 same-gradient repeat controls**.
This covers AdamW, Muon and NorMuon on all four ranks; rank replicas are not independent trials.

On the shared Muon-family input pair, only 30 hidden-matrix BF16 values differ after conversion;
five Newton–Schulz iterations spread the difference to 4,795,454 values in 17/88 matrices. NorMuon
row rescaling subsequently affects 21,776,734 output values. All 352 traced helper output/state
comparisons match production bitwise. This accounts for the observed immediate update difference,
but does not identify the first gradient-difference source, prove a serialization defect, establish
harmlessness, or explain retrieval quality. It is **engineering-only**, not a new paper result.

The parent still fails 24/24 strict full-replay checks. Original tolerances and failed receipts
remain intact. The full absolute-source-path regression run passes **1,337 JUnit cases**, with no
failures/errors/skips. Two diagnostic-wrapper failures and one wrong-import-path full-suite attempt
are separately retained. The three captured gradient files total 7,153,135,725 bytes and remain in
temporary storage, not on HF; their digests and exact locations are in the new archive.

The artifact-only snapshot remains **12/12 runs, 60/60 resumable checkpoints**. BEIR remains paused
with the original verified dispatcher handles, lease and completed-step prefix. No production
numerical source, checkpoint or live ledger was changed. The next bounded task is to distinguish
backward-kernel and distributed-reduction sources of the first gradient discrepancy, then test a
fresh-process restart. Formal correction/retraining and source publication remain separate reviewed
transitions; existing scores are still not cleared for scientific inference.

## Correctness-first GPU verification — 2026-09-06 05:56 UTC

The owner explicitly renewed work with correctness and scientific validity as the priority. An
announced, exact-process project handoff has now paused BEIR: three CPU dispatch processes remain
stopped in place, retaining their lease and four completed steps; their eight verified leaf
evaluators have exited. All 124 archived source/result/state files match the unchanged originals.
Checkpoints and production source were not changed. The receipt is in
[`dense-gpu-correctness-v1/handoff/`](reports/engineering-archive/dense-gpu-correctness-v1/handoff/result.json).
This supersedes the historical "pause unanswered/not performed" statements below.

The first GPU acceptance campaign has completed in isolated temporary namespaces:

- Unchanged Trainer: all nine raw-gradient checks fail at a ratio of approximately **0.25**.
  Diagnostic multiplication by four passes separately; it is not an old-path acceptance pass.
- Single-normalization candidate, short inputs: all nine raw/clipped comparisons pass across all
  134 parameter tensors; maximum raw error is **1.1921e-7**, below the unchanged tolerance.
- 8192-token input: all four ranks reach the maximum context and finish without OOM. Default
  backward comparisons fail the old tolerance. A paired control changes only the owned attention
  determinism flags; all nine comparisons then pass, maximum raw error **4.7684e-7**. This supports
  a kernel-reproducibility explanation on the fixture, not acceptance of the original failed path.
- GPU save/resume: six continuations/24 rank comparisons with eight loader workers per rank restore
  entry weights, optimizer/scheduler, rows and CUDA/CPU/Python/NumPy RNG exactly. Subsequent values
  differ: maximum final weight differences are 2.981e-8 (AdamW), 6.190e-5 (Muon), and 3.440e-5
  (NorMuon). Replay-1 starts with only 1.1921e-7 maximum raw-gradient difference, which grows after
  a Muon-family update. Its amplification mechanism is not yet isolated; no harmlessness or
  serialization-bug conclusion is justified.

All **1,319 regression tests pass**. These are engineering checks, not scientific model results.
The diagnostic launchers have exited; BEIR's original chain remains paused. No formal training,
repair deployment, ledger migration, source publication or HF deletion is implied. The primary
matrix remains 12/12 executed and 60/60 preserved, **not cleared for scientific inference**.
Do not start or resume an evaluator while diagnostics own the released pool. Original evaluation
workers' in-flight units will require recomputation; their completed result files remain intact.
Next: isolate the replay amplification, finish the optimizer-reference acceptance boundary, then
freeze the reviewed correction and replication plan before any formal relaunch. The old zero-step
migration still does not apply to the retained completed-step ledger.
