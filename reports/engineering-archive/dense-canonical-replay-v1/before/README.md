...
**Correctness-first hold (September 6):** all **12/12 primary runs have finished** and **60/60
checkpoints are preserved**, but the matrix is not cleared for scientific inference. Real four-GPU
BF16/FlashAttention-2 tests reproduce the double-normalization failure: unchanged-path gradients
are one quarter of the independent reference. The isolated candidate passes all nine short-input
raw/clipped gradient checks. Default-kernel long-input comparisons fail the original tolerance;
a paired deterministic-backward control passes, without changing that tolerance. GPU checkpoint
entry state, row order and RNG restore exactly, but later numerical replay differs. The
[new single-update localization](reports/engineering-archive/dense-gpu-replay-localization-v1/README.md)
reproduces all 24 optimizer/scheduler updates and 12 same-gradient controls exactly: on this
fixture, a few BF16 rounding differences spread through Newton–Schulz. The
[paired first-gradient probe](reports/engineering-archive/dense-gradient-origin-v1/README.md)
now reproduces their source: identical local gradients reduced through 16 versus one DDP bucket.
Changing only that layout reproduces the difference exactly; deterministic backward alone does
not remove it on these short inputs. This is floating-point reproducibility evidence, not a new
optimizer defect, quality finding or acceptance of the still-failed full replay. All 1,344 regression
cases pass with absolute audited source paths. Read the
[earlier failed/control evidence](reports/engineering-archive/dense-gpu-correctness-v1/README.md).

BEIR is now paused through a verified, project-only resource handoff; its original dispatch chain
and lease are retained. Completed results, checkpoints, production code and `gpu.py` are untouched.
Do not start a competing controller, resume evaluation into a diagnostic pool, deploy an unreviewed
repair or promote existing scores into paper conclusions. These checks are engineering provenance,
not manuscript content or new optimizer-quality results.

The separate independent ten-run HF audit still covers 1,010 files at original immutable commits;
it is not silently extended by the automatic 60-checkpoint backup count. Raw five-stage geometry
covers ten runs/50 stages. Full audited retrieval and mechanism results remain pending. The implementation was
