...
def inputs():
    need(os.environ.get('CUDA_VISIBLE_DEVICES') == '' and os.environ.get('PYTHONPATH') == str(STORY / 'src'),
         'Use explicit current source and hide CUDA')
    from embed_optim.config import RunConfig, source_wandb_run_id
    from embed_optim.primary_contract import digest
    from embed_optim import corrected_wandb_audit as original
    need(Path(original.__file__).resolve() == STORY / 'src/embed_optim/corrected_wandb_audit.py', 'Wrong tracking helper source')
    for module in ('config.py', 'corrected_wandb_audit.py', 'primary_contract.py'):
        path = STORY / 'src/embed_optim' / module
    ...
    for run_id, proof in sorted(primary_rows.items()):
        ...
        result.append({'family': 'primary', 'run_id': run_id, 'remote_id': source_wandb_run_id(config),
             ...
             'primary_auditor': original.audit_run, 'expected_arguments': {}, 'native_factory': None})
