...
def main():
    ...
    parent_path = repository / 'reports/engineering-archive/dense-v3-primary-launch-v1/source-assembly.json'
    if file_identity(parent_path)['sha256'] != 'e603ef175c0f691a1dd83c4d8b474d90da2fc67c8c65f5a77f3c3180e78332b8':
        raise ValueError('Original primary source assembly changed')
    parent = read_json(parent_path)
    files = {}
    for relative, binding in parent['files'].items():
        verify_file(args.source / relative, binding)
        verify_file(Path('/root/embedding-optimizer-primary-v3') / relative, binding)
        files[relative] = file_identity(args.source / relative)
    extra = ('factorial_v3_inputs', 'factorial_v3_calibration', 'factorial_v3_optimizer',
             'factorial_v3_batches', 'factorial_v3_trainer', 'factorial_v3_checkpoint',
             'gradient_probe', 'probe_export', 'update_geometry', 'probes')
    for name in extra:
    ...
    locations = inputs.Locations(repository, Path('/root/embedding-optimizer-primary-v3'),
    ...
    result = {'scope': 'actual-v3-factorial-input-consumer-readback',
              'observed_at_utc': datetime.now(timezone.utc).isoformat(),
              'source_root': str(args.source), 'source_files': files,
              'primary_files_unchanged': len(parent['files']),
              'loaded_numerical_sources': sources, 'inputs': actual, 'requests': requests,
              'genuine_source_states': 2, 'branch_rows': 50000, 'calibration_rows': 32,
              'actual_model_or_gradient_execution': False,
              'formal_calibration_or_branch_admission': False, 'scientific_completion': False}
    with args.output.open('x') as stream:
        json.dump(result, stream, sort_keys=True, indent=2, allow_nan=False)
        stream.write('\n')
    print(json.dumps({'output': str(args.output), **file_identity(args.output),
                      'source_files': len(files), 'genuine_sources': 2,
                      'gpu_execution': False, 'scientific_completion': False}), flush=True)
