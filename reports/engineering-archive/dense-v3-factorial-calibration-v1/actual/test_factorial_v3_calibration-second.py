...
def complete_fixture(tmp_path, state):
    ...
    receipt = {"scope": calibration.SCOPE, "status": "gradients_complete", "request": request,
               "sources": sources, "runtime": {"synthetic_wiring_fixture": True},
               "manifest": file_identity(root / "gradients/manifest.json"),
               "fresh_gpu_gradient_history": True, "loaded_weights_unchanged": True,
               "historical_gradients_reused": False, "formal_branch_admission": False}
    write(root / "gradient-receipt.json", receipt)
    rows = [{"tensor": r["checkpoint_name"], "shape": r["shape"],
             "parameters": math.prod(r["shape"]), "gradient_steps": 8,
             "weight_frobenius_norm": 2.0 + index,
             "algorithms": {op: {"frobenius_norm": (index + 1.) * (j + 1.)}
    ...
    result = {"scope": calibration.SCOPE, "status": "complete", "request": request,
              "sources": sources, "runtime": receipt["runtime"],
              "gradient_receipt": file_identity(root / "gradient-receipt.json"),
              "rows": file_identity(root / "directions/rows.json"), "calibration": rates,
...
def test_complete_consumer_and_twelve_cell_expansion_are_synthetic_wiring(tmp_path):
    ...
    for state in inputs_module.STATES:
        for operator in ("adamw", "muon"):
            rates = {j["hidden_lr"] for j in jobs if j["state"] == state and j["operator"] == operator}
            assert rates == {calibrated[state]["calibration"]["hidden_learning_rates"][operator]}


@pytest.mark.parametrize("case", ["rate", "norm_rows", "stale_request", "old_gradients", "cpu", "source", "missing_row", "parent_binding"])
def test_rehashed_semantic_changes_are_not_accepted_as_calibration(tmp_path, case):
    ...
    if case in ("rate", "parent_binding"):
        ...
        if case == "norm_rows": rows[0]["weight_frobenius_norm"] *= 10
        else: rows.pop()
        write(root / "directions/rows.json", rows)
        result["rows"] = file_identity(root / "directions/rows.json")
    elif case == "stale_request": result["request"]["source"]["checkpoint_step"] = 3126
    ...
    elif case == "cpu": result["operator_device"] = "cpu"
    elif case == "source": result["sources"]["optimizers.py"]["sha256"] = "0" * 64
    write(root / "directions/calibration.json", result)
    if case != "parent_binding": binding = file_identity(root / "directions/calibration.json")
    with pytest.raises(ValueError): calibration.read_calibration(data, "adamw_state", root, binding)


def test_actual_assembled_numerical_source_is_the_corrected_one():
    sources = calibration.require_sources()
    assert sources["optimizers.py"]["sha256"] == "7d7d4f200c410cb1b24e1773ae72811582f43a0c794a5d1b2af269c123e69b1e"


def test_gpu_producers_refuse_hidden_cuda_before_creating_outputs(tmp_path):
    assert not torch.cuda.is_available()
    with pytest.raises(ValueError): calibration.export_gradients(None, "adamw_state", tmp_path / "missing")
    with pytest.raises(ValueError): calibration.replay_calibration(None, "adamw_state", tmp_path / "missing", {})
