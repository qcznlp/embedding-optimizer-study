...
def _runtime(locations):
    ...
    if file_identity(path)["sha256"] != "b90de16b3796bb8e9d0badcedec688da55a037d46244a0d2a7f5838795cfe77a":
...
def load_calibrated_cells(locations, directories):
    ...
    inputs = load_inputs(locations)
    calibrated = {state: read_calibration(inputs, state, directories[state]["path"],
                                         directories[state]["calibration_binding"])
                  for state in STATES}
