...
CALIBRATION_CONFIG = {
    "family": "dense", "gradient_steps": 8, "examples_per_gradient": 4,
    "micro_batch_size": 1, "seed": 2718, "temperature": 0.02,
    "max_grad_norm": 1.0, "model_dtype": "float32", "forward_dtype": "bfloat16",
    "storage_dtype": "float32", "device": "cuda:0", "flash_attention": True,
    "model_mode": "train", "gradient_checkpointing": True,
    "parameter_partition": "hidden", "weights_advanced": False,
}
...
def _source_binding(locations, row):
    ...
    expected_path = Path(locations.experiment) / "outputs/dense-correctness-v3/dense" / run_id / "checkpoint-2345"
    ...
    require_same(row["native_checkpoint"], {
        "scope": "dense_primary_correctness_v3", "protocol_sha256": PRIMARY_SHA,
    ...
    return {
        "label": row["label"], "run_id": run_id, "checkpoint": str(checkpoint),
        "checkpoint_step": 2345, "native_checkpoint": copy.deepcopy(row["native_checkpoint"]),
...
def load_inputs(locations):
    ...
    for recorded, binding in verified["files"].items():
        verify_file(locations.locate(recorded), binding)
    if (data["parent_ledger_rows_checked"] != 500000
        or data["branch"]["rows"] != 50000 or data["calibration"]["rows"] != 32
    ...
    if file_identity(Path(locations.repository) / "configs/representation_probe.json")["sha256"] != "45e76c8daa5bda0213c346b39200a34ddc1e9295f37198a2bd238276ee6057e2":
        raise ValueError("Fixed calibration probe specification differs")
    branch = locations.locate("/root/embedding-optimizer-study/data/short-branch-50k-seed20260826/manifest.json").parent
    probe = locations.locate("/root/embedding-optimizer-study/data/probes/training-1024-seed1729/manifest.json").parent
    return {
        "scope": SCOPE, "scientific_admission": False, "execution_authorized": False,
        ...
        "sources": sources,
        "common_state_spec": {"path": str(Path(locations.repository) / "configs/common_state_probe.json"),
                              **file_identity(Path(locations.repository) / "configs/common_state_probe.json")},
        "probe_spec": {"path": str(Path(locations.repository) / "configs/representation_probe.json"),
                       **file_identity(Path(locations.repository) / "configs/representation_probe.json")},
    }


def calibration_request(inputs, state):
    if (inputs.get("scope") != SCOPE or inputs.get("scientific_admission") is not False
        or inputs.get("execution_authorized") is not False
        or inputs.get("accepted_audit_digests") != AUDITS or state not in STATES):
    ...
    return {
        ...
        "input_sha256": digest(inputs), "state": state,
        "source": copy.deepcopy(source), "probe": inputs["calibration"]["probe"],
...
def require_gradient_manifest(manifest, request, directory):
    ...
    expected_inputs = [r for r in request["source"]["native_checkpoint"]["files"]
    ...
    if (manifest.get("status") != "complete" or type(manifest.get("schema_version")) is not int
        ...
        or manifest["probe"]["manifest_sha256"] != "40953eb60bb5dbfa02d9abde5e634bb3221ee934d7ca654c5e5a7e961b39eed2"
    ...
    require_same(sorted(mapping, key=lambda r: r["model_name"]), sorted([
        {"model_name": r["name"], "checkpoint_name": r["name"].removeprefix("0.model."), "shape": r["shape"]}
        for r in wanted], key=lambda r: r["model_name"]))
    ...
    for step, shard in enumerate(shards):
        if (type(shard.get("step_index")) is not int or shard["step_index"] != step
            or shard["path"] != f"gradient-{step:04d}.safetensors"
            or shard["sample_ids"] != [r["sample_id"] for r in request["selection"][4 * step:4 * step + 4]]):
