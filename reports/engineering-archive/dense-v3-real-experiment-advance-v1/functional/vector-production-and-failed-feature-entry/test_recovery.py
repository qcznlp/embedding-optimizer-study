...
class Tests(unittest.TestCase):
    ...
    def test_gpu_lease_refusal_precedes_native_model_admission(self):
        calls = []
        auth = {'scope': 'synthetic', 'source_sha256': 'a',
                'functional_source_sha256': dispatch.FUNCTIONAL_SHA,
                'inputs_sha256': dispatch.INPUTS_SHA, 'settings': dispatch.SETTINGS,
                'execution_authorized': True, 'native_guards_modified': False,
                'committed_source_release': False, 'scientific_completion': False,
                'sources': {}, 'tests': {'path': 'synthetic-tests', 'bytes': 0, 'sha256': 'b'},
                'test_source': {'path': 'synthetic-source', 'bytes': 0, 'sha256': 'b'}}
        def lease(*_):
            calls.append('inherited_lease_check')
            raise ValueError('Synthetic lease refusal')
        entry = SimpleNamespace(SCOPE='synthetic', VALIDATION='synthetic-validation', PARENT='synthetic-parent',
            authenticate=lambda *_: self.fail('Native model admission preceded lease gate'),
            import_path=lambda name, path: (SimpleNamespace(require_inherited_leases=lease)
                if path == 'synthetic-validation' else SimpleNamespace(LEASE_ROOTS=[Path('/synthetic-a'), Path('/synthetic-b')])))
        geometry = SimpleNamespace(identity=lambda _: {'bytes': 0, 'sha256': 'b'})
        args = SimpleNamespace(source_sha='a', authorization_sha='b', gpu_token='0', lease_fd=[31, 32])
        with patch.object(dispatch, 'load_entry', return_value=entry), \
                patch.object(dispatch, 'source_guard', return_value=geometry), \
                patch.object(support, 'authority', return_value=(auth, {})), \
                patch.dict(os.environ, {'CUDA_VISIBLE_DEVICES': '0'}):
            with self.assertRaisesRegex(ValueError, 'lease refusal'):
                dispatch.authenticate(args, gpu_worker=True)
        self.assertEqual(calls, ['inherited_lease_check'])

    def test_changed_proposal_refused(self):
        expected = self.proposal(); changed = copy.deepcopy(expected)
        changed['state_order'] = changed['state_order'][1:]
        with patch.object(support, 'namespace'), patch.object(support, 'proposal_data', return_value=expected):
            with self.assertRaisesRegex(ValueError, 'Proposal'):
                support.check_proposal(ROOT, changed, dispatch.SETTINGS)
    ...
    def test_complete_actual_production_module_with_explicit_stubs(self):
        ...
        states = flow.observe_records(f.root / 'run', list(layout.CELLS),
            {j['plan']['state']['cell']: f.context.digest(j['plan']) for j in f.context.jobs},
            source_sha=f.args.source_sha, authorization_sha=f.args.authorization_sha,
            command_prefix=['/usr/bin/python', '-B', str(f.root / 'launch/recovery.py'),
                            '--source-sha', f.args.source_sha, '--authorization-sha', f.args.authorization_sha],
            coordinator_pid=1000100, output_root=f.entry.OUTPUT, expected_reuse_origin=f.origin,
            observe_exact=lambda _: self.fail('Unexpected real process read'))
    ...
    def test_wrong_coordinator_refused_before_any_process_read(self):
        root = self.root(); run = root / 'run'
        record = {'source_sha256': 'a', 'authorization_sha256': 'b',
                  'command': ['SYNTHETIC_WRONG_COMMAND'], 'pid': 100, 'start_ticks': 200,
                  'settings': dispatch.SETTINGS}
        write(run / 'coordinator.started.json', record)
        args = SimpleNamespace(source_sha='a', authorization_sha='b',
                               started_sha=support.identity(run / 'coordinator.started.json')['sha256'])
        with patch.object(dispatch, 'HERE', root), patch.object(dispatch, 'RUN', run), \
                patch.object(support, 'authority', return_value=({}, {})):
            with self.assertRaisesRegex(ValueError, 'coordinator'):
                observer.observe(args, exact=lambda _: self.fail('Unexpected process read'))
