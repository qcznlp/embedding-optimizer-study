...
class Tests(unittest.TestCase):
    def test_actual_native_parent_import_and_admission(self):
        from types import SimpleNamespace
        prior = m.original()
        _, context, _ = prior.authenticate(SimpleNamespace(
            source_sha=m.PINS[m.PRIOR / 'recovery.py'],
            authorization_sha=m.PINS[m.PRIOR / 'authorization.json']))
        self.assertEqual(len(context.jobs), 61)
...
if __name__ == '__main__':
    ...
    with Path(__file__).with_name('tests-second.json').open('x') as stream:
