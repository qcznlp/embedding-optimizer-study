...
DEPLOY = EXPERIMENT / 'launch/factorial-calibration-v2'
SOURCE_ROOT = STORY / 'reports/engineering-archive/dense-v3-factorial-worker-v1/actual/source-final'
OUTPUT = EXPERIMENT / 'analyses/dense-primary-v3-factorial-calibration-v2'
RUN = HERE / 'run'
...
LOADER_POLICY = {'primary_input_configuration_function': 'train._configure_dense_input_execution',
                 'dense_can_flatten_inputs': False,
                 'gradient_tensor_namespace': 'raw_safetensors_keys_after_verified_TensorStore_alias_mapping',
                 'native_tensor_and_backward_checks_unchanged': True}
...
@contextmanager
def configured_calibration_loader(calibration):
    """Apply the existing primary input policy before the native observed-loader check.

    Checkpoint exports do not serialize this runtime setting. The native component
    checks it but deliberately does not choose it. No tensor, loss, gradient,
    direction, kernel, or numerical guard is replaced here.
    """
    from embed_optim import probe_export, train
    original = calibration.gradient_probe._load_model
    original_mapping = calibration.gradient_probe._hidden_parameter_mapping
    need(original is probe_export._load_model, 'Unexpected pre-existing calibration loader override')
    def load(*args, **kwargs):
        model = original(*args, **kwargs)
        train._configure_dense_input_execution(model, SimpleNamespace(dense_can_flatten_inputs=False))
        return model
    def mapping(hidden, checkpoint):
        return raw_checkpoint_mapping(original_mapping(hidden, checkpoint), checkpoint)
    calibration.gradient_probe._load_model = load
    calibration.gradient_probe._hidden_parameter_mapping = mapping
    try:
        yield
    finally:
        calibration.gradient_probe._load_model = original
        calibration.gradient_probe._hidden_parameter_mapping = original_mapping


def raw_checkpoint_mapping(mapped, checkpoint):
    """Resolve TensorStore's component-0 aliases to the raw keys required by calibration.

    Only names change: original exporter mapping determines the parameters, and
    both raw names and shapes are checked against the same sealed source file.
    All native gradient/receipt checks remain mandatory, including exact 88-key coverage.
    """
    ...
    with safe_open(str(Path(checkpoint) / 'model.safetensors'), framework='pt', device='cpu') as store:
        raw_shapes = {name: list(store.get_slice(name).get_shape()) for name in store.keys()}
        wanted = {name for name, shape in raw_shapes.items()
                  if parameter_partition_name(name, len(shape)) == 'hidden'}
        need(len(mapped) == len(wanted) == 88, 'Raw source does not have the declared 88 hidden matrices')
        for model_name, alias, parameter in mapped:
            need(model_name.startswith('0.model.'), 'Unexpected model parameter namespace')
            raw = model_name.removeprefix('0.model.')
            need(raw in wanted and alias in (raw, '0.' + raw)
                 and list(parameter.shape) == raw_shapes[raw], 'Checkpoint alias/name/shape differs')
            result.append((model_name, raw, parameter))
        need({item[1] for item in result} == wanted and len({item[0] for item in result}) == 88,
             'Raw mapping omitted or duplicated a parameter')
    return result
...
def prepare(args):
    ...
    authority = {'scope': SCOPE, 'prepared_at_utc': geometry.now(), 'source_sha256': args.source_sha,
        ...
        'gpu_tokens': GPU, 'output_root': str(OUTPUT), 'loader_policy': LOADER_POLICY,
...
def authenticate(args, gpu=False):
    ...
    need(auth['scope'] == SCOPE and auth['source_sha256'] == args.source_sha
         ...
         and auth['loader_policy'] == LOADER_POLICY
...
def worker(args):
    ...
    with configured_calibration_loader(calibration):
    ...
    try:
        ...
        for state in STATES:
            ...
            with (RUN / f'{state}.verify.log').open('xb') as log:
                ...
                readers.append((state, child, None))
                handle = dispatch.process_identity(child.pid, argv)
                readers[-1] = (state, child, handle)
        ...
        for state, child, argv, handle in children:
            ...
            if not path.exists() and handle is not None:
                geometry.write_new(path, {**handle, 'state': state, 'exit_code': code,
                    'authorization_sha256': args.authorization_sha, 'exited_at_utc': geometry.now()})
        for state, child, handle in readers:
            code = child.wait()
            path = RUN / f'{state}.verify.exited.json'
            if not path.exists() and handle is not None:
                geometry.write_new(path, {**handle, 'exit_code': code,
                    'authorization_sha256': args.authorization_sha, 'exited_at_utc': geometry.now()})
