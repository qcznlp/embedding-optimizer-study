...
have been generated and numerically reproduced from copied source. Functional
analysis, crossed continuation and manuscript/source release remain incomplete.
The [owner-approved functional recovery is now genuinely running](reports/engineering-archive/dense-v3-functional-recovery-launch-v1/README.md):
at 2026-09-12 14:11 UTC, eleven of 61 vector states are accepted, including ten
new real checkpoint encodings. The source-bound coordinator/next worker are live
at that dated observation; no functional feature state is complete yet.
An [offline recovery-flow candidate](reports/engineering-archive/dense-v3-functional-flow-candidate-v1/README.md)
now checks the complete functional scheduling sequence with explicit stubs;
this is preparation, not resumed model analysis or experimental evidence.
The accepted pretrained vectors also pass [genuine checkpoint-backed readback after copying](reports/engineering-archive/dense-v3-pretrained-native-copy-v1/README.md),
without re-encoding; that reuse has now succeeded in the production recovery.
...
Full functional encoding/inference and a supported mechanistic explanation remain
incomplete; see the dated
[current evidence table](CURRENT_EXPERIMENT.md#current-evidence).
