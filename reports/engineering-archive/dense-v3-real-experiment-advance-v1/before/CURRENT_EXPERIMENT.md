...
**Owner explicitly approved proceeding on 2026-09-12:** “你有权做一切事情，目标是尽快完成任务”.
This answers the recovery question; do not keep waiting for that approval.
The tested new entry is installed and **genuinely running** at
`/root/embedding-optimizer-v3-experiment/launch/functional-dimensions-recovery-v1`.
Nineteen focused recovery tests pass, including the actual production function
bodies under explicit model/process/lease fixtures. Eleven original operational
bodies and both validated helpers remain unchanged. Native input admission passed
(session 86421 / terminal `84f65b`, exit 0). The real coordinator started at
**14:04:07 UTC**, session **27585**, PID **766302 / start 319358599**.
At the exact **14:11:14 UTC** observation it is live/S, with **11 / 61 accepted
vectors** (ten new exit-zero/native-verified checkpoints plus the reused pretrained
state), **0 / 61 features**, and the next exact worker live/R on token 0.
See the [launch, source and first real worker evidence](reports/engineering-archive/dense-v3-functional-recovery-launch-v1/README.md).
The current snapshot is `reports/engineering-archive/dense-v3-functional-recovery-launch-v1/actual/observation-fourth.json`.
These are dated observations, not a perpetual heartbeat.
Use only the new recovery source/authority and externally bound new coordinator
handle. Preserve the failed original attempt and accepted pretrained state.
Continue excluding `gpu.py`, broad process inspection, old controllers and any
external safety bypass. No new scientific result is claimed by these checks.

**Genuine pretrained-copy native readback complete through 2026-09-12 13:38 UTC.**
The [actual CPU compatibility check](reports/engineering-archive/dense-v3-pretrained-native-copy-v1/README.md)
reads the original accepted vectors and a byte-identical temporary copy with the
unchanged checkpoint-backed native reader. Both reads verify 134 tensor fingerprints;
all four arrays match bitwise. Only the returned manifest's absolute location
changes. Wrong external anchor and state plan are refused. All 66 bound inputs and
eleven pretrained checkpoint files remain unchanged. No encoding, feature calculation,
GPU lease, recovery authority or coordinator launch occurred. This completes the
real copied-pretrained compatibility item; do not repeat it as missing work.

**Full functional recovery-flow preparation checked through 2026-09-12 13:31 UTC.**
The [new isolated integration candidate](reports/engineering-archive/dense-v3-functional-flow-candidate-v1/README.md)
passes eighteen CPU tests of the complete coordinator/finalizer/feature sequence,
with sixty mocked new workers, one reused synthetic pretrained state, 122 feature
stub calls and all nested record chains. Eleven original operational bodies are
unchanged; only nested-parent preparation and pretrained routing are inserted in
the simulated coordinator. Sixty-six original bound files remain unchanged.
This is not a production launcher or native numerical acceptance. No authority
was created, original coordinator restarted, GPU leased or model encoded.
Actual functional progress remains **1 / 61 vector states, 0 / 61 feature states**.

**Both new prediction analyses backed up and recovered through 2026-09-12 13:12 UTC.**
The [immutable prediction snapshot](docs/predictor-analysis-restoration.md) now
preserves 47 files / 12,301,350 bytes at revision
`42689be00e1644aaf24721ecc239ec1c4d425a2a`. Full anonymous recovery finished
13:09:04 UTC; the copied isolated stdlib reader finished 13:09:25 UTC, reconstructing
errors from all 840 original and 5,040 exploratory predictions, 392 folds and 98
pooled/plotted comparisons. The two analysis roles and all numerical outputs are
unchanged. Fourteen older corrected subtrees and all root entries/attributes are
preserved. This completes data durability, not source publication, new inference,
functional recovery, crossed continuation or the manuscript.

**Comparator sensitivity checked through 2026-09-12 12:44 UTC.** The new
[explicitly post-result diagnostic](reports/dense-v3-predictor-sensitivity-v1/README.md)
retains all fourteen features under four recipe baselines and two
displacement-conditioned comparators: 84 comparisons / 336 folds / 5,040
independently verified predictions, with seven byte-identical copied-source/data
outputs. Adding only optimizer-specific raw LR to B0 lowers baseline RMSE from
2.905349 to 0.651744; adding displacement then worsens it to 0.795009. No feature
passes all four unconditioned baselines. This limits the preceding geometric
interpretation, not the unchanged endpoint results. Neither positive nor negative
post-result flags establish a functional or causal mechanism.

Actual complete weight-to-retrieval numerical prediction finished **2026-09-12
12:15 UTC**; independent full-matrix OLS, copied-source/data replay and complete
local archive verification finished by **12:24 UTC**. All original nine and
separate five exact features are retained. This is real fixed-grid prediction,
not formal whole-primary/source admission or functional/causal completion.
See the [actual analysis and its limitations](reports/dense-v3-weight-retrieval-v1/README.md).
...
and complete trajectory tables/figures recovered/reconstructed at **11:40 UTC**,
...
| Complete retrieval trajectories | **[Eight actual tables and all twelve curves](reports/dense-v3-complete-trajectories-v1/descriptive/summary.md)** generated and backed up; copied-source replay reproduces every CSV, all six endpoint contrasts unchanged | Curves are descriptive, not seed robustness, a functional mechanism or formal publication-consumer admission |
| Complete trajectory durability / raw-score index | **[26 / 26 files](docs/retrieval-trajectories-restoration.md)**, 1,093,911 bytes, anonymously recovered; isolated stdlib reader reconstructs stage statistics, areas, endpoint point effects and figure data | All 840 scores match a five-snapshot recovery index built from 2,888 previously recovered files / 37,364,806 bytes. Source/guide remain local; no second-host model experiment or new bootstrap |
...
| Actual weight-to-retrieval prediction | **[9 original + 5 exact features, all 60 states](reports/dense-v3-weight-retrieval-v1/README.md)**; 840 feature values and held-out predictions independently checked, 56 fold comparisons / 14 decisions, all 17 copied-source/data outputs byte-identical | Original 5/9 and exact 1/5 satisfy the locked predictive rule. Not significance, useful dimensions, mediation, richer-baseline robustness or formal source/publication admission |
| Post-result recipe/conditional sensitivity | **[84 comparisons / 336 folds / 5,040 predictions](reports/dense-v3-predictor-sensitivity-v1/README.md)** independently checked; all 14 original features, four recipe baselines, seven identical replay outputs | Predictor flags depend on comparator. Conditional entropy helps under B3 + displacement but not B0 + displacement. Exploratory, not a new confirmatory finding or a new selected baseline |
| Prediction-analysis durability | **[47 / 47 files](docs/predictor-analysis-restoration.md)**, 12,301,350 bytes; full anonymous download and copied isolated error reconstruction passed | Both new numerical analyses are now backed up. Source/guide remain local; no OLS refit, new model, functional recovery or paper completion |
...
| Functional dimension analysis | **[11 / 61 vector states accepted, 0 / 61 feature states](reports/engineering-archive/dense-v3-functional-recovery-launch-v1/README.md)** at 14:11:14 UTC; new approved coordinator live/S and next worker live/R | Ten genuine new exit-zero/native-verified checkpoints plus reused pretrained. Finish remaining encodings, whole-matrix readback and real features; preserve the failed original namespace |
...
| Full functional-flow preparation | **[18 isolated integration tests passed](reports/engineering-archive/dense-v3-functional-flow-candidate-v1/README.md)**, followed by 19 production recovery checks and actual native admission/launch | Preparation and recovery approval are complete. Do not repeat them as missing work or restart the live recovery; finish actual measurement |
| Genuine pretrained-copy compatibility | **[Two successful native reads](reports/engineering-archive/dense-v3-pretrained-native-copy-v1/README.md)** of original/copied genuine vectors; 134 checkpoint-backed tensor fingerprints each, four bitwise-equal arrays and two refusal controls | Reuse is now installed and has succeeded in the real recovery. It remains the same one pretrained state, not a new training run or new model encoding |
...
continuation remains zero of twelve. Full-trajectory tables/figures and all raw
outcomes now have immutable data-only backups below. Manuscript/source release
and historical safety boundaries remain unchanged.

The [complete trajectory snapshot](reports/engineering-archive/dense-v3-trajectory-artifact-backup-v1/README.md)
was uploaded at **11:39:09 UTC**, anonymously recovered at **11:40:10 UTC** and
independently reconstructed from its recovered bytes at **11:40:28 UTC**.
All **26 files / 1,093,911 bytes** match. The stdlib reader checks 840 scores,
run/optimizer-stage means and medians, all observed areas, both endpoint task-effect
tables, all six prior endpoint comparisons and every plotted checkpoint. Existing
interval values are matched, not newly bootstrapped. The data-only raw-score index
also reconstructs the full score grid from five previously anonymously recovered
snapshots: **2,888 files / 37,364,806 logical bytes**. Revision
`dbcd12376f483347cc570584abc588503c0a5fa0` preserves twenty other root entries,
thirteen prior corrected subtrees and root attributes. The source/guide remain
local WIP; no GPU work, functional recovery, new inference or paper finding occurred.
...
1. Complete primary retrieval, its eight-table replay, all raw-score backups and
   the complete trajectory data-only snapshot are done. The actual original
   nine-feature and separate five-feature exact weight-to-retrieval numerical
   analyses are also done, with all four dose folds, exact rules and every row
   retained. Do not repeat them as missing work or infer formal-consumer admission.
   Normalized cumulative displacement lowers pooled RMSE from 2.905349 to 1.003975
   points with 4/4 improved folds; original/exact stable ranks improve 0/4 folds;
   full-spectrum segment entropy reaches 2.347064 with 3/4 improvements.
   The separately labelled recipe/conditional sensitivity is now also complete;
   do not repeat its 84 comparisons. It shows comparator-sensitive predictive
   flags: B1 raw-rate baseline 0.651744, B1 + displacement 0.795009; full-spectrum
   entropy worsens B0 + displacement from 1.003975 to 1.260228 but improves B3 +
   displacement from 1.420297 to 1.347976. No feature passes all four recipe
   baselines. Preserve all locked and post-result outputs, without claiming
   norm-mediated, independent entropy, universally useless rank or causal effects.
   Both new numerical reports now have immutable data-only backup and independent
   recovered-error reconstruction; see the prediction restoration guide above.
   Do not repeat these completed uploads or analyses. Their source/guide remain
   local WIP, not a source release; no aggregate bundle containing Python was uploaded.
2. Resolve the scoped functional receipt-path recovery authority, preserving the
   ...
   The separate full-flow fixture integration and genuine origin-bound copied-
   pretrained native readback are both complete. Neither creates production
   authority. Reviewed recovery authentication/launcher and exact new worker/lease
   admission still remain before real dispatch.
