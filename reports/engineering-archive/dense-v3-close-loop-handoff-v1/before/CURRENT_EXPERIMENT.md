...
Dated snapshot: **2026-09-12 16:16 UTC**. This is not a perpetual heartbeat.
...
| Crossed continuations | **2/12 complete**; second pair at **245/391 each**, eight exact ranks live |
| New checkpoint verification/backup | First **10/10** native deep reads and immutable HF metadata/hash checks complete; backup watcher active |
...
The [latest training observation](reports/engineering-archive/dense-v3-functional-result-chain-v1/training-observation-1617.json)
...
The first two full branches provide actual production-topology/save/readback
...
Both training pools run disjoint balanced six-cell queues. Current second pair:
`factorial-v3-muon_state-muon-seed314159` (pool a) and
`factorial-v3-muon_state-adamw-seed314159` (pool b).
The first AdamW-source pair is complete. All eight current exact training ranks
...
The first two new runs (ten checkpoints) are remotely verified in
`qcz/embedding-optimizer-study-checkpoints`, revisions
`a219dcab85bda040e9d46f6f7ec18ee1b0acf598` and
`e9ab737562e50a6ec0ff838f22e97301fda93d63`. Their watcher waits for all twelve
native whole-run completions before backing up each. The watcher entry is
`/tmp/dense-v3-factorial-backup-launch.75Chtt86/backup.py`; do not restart it.
**Calibration and recovered functional payloads still need an off-host backup.**
...
2. Functional inference and all-four-feature post-result recipe-control sensitivity
   are complete and independently verified. The sensitivity numerically replays
   from copied source/data; finish original inference portability and off-host
   backup. Retain all unsupported and undefined findings.
