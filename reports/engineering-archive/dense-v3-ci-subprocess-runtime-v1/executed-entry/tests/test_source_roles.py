...
def test_real_roles_partition_every_test_without_overlap():
    ...
    interpreter = next(
        i
        for i, step in enumerate(steps)
        if step.get("name") == "Build the original-source CPU replay interpreter"
    )
    isolation = next(
        i
        for i, step in enumerate(steps)
        if step.get("name") == "Verify subprocess isolation under CPU emulation"
    )
    assert emulator < interpreter < isolation < probe
    assert "scripts/build_ci_python.py" in steps[interpreter]["run"]
    assert "scripts/check_cpu_replay_fds.py" in steps[isolation]["run"]
    for index in (isolation, probe, numerical):
        assert "$RUNNER_TEMP/cpu-python-runtime/install/bin/python3.12" in steps[index]["run"]
    builder = (ROOT / "scripts/build_ci_python.py").read_text()
    assert "56bfef1fdfc1221ce6720e43a661e3eb41785dd914ce99698d8c7896af4bdaa1" in builder
    assert 'ac_cv_func_close_range="no"' in builder
    isolation_source = (ROOT / "scripts/check_cpu_replay_fds.py").read_text()
    assert "close_fds=True" in isolation_source and "start_new_session=True" in isolation_source
    assert "pass_fds=(keep_fd,) if keep else ()" in isolation_source
    assert "close_fds=False" not in isolation_source
    assert (
