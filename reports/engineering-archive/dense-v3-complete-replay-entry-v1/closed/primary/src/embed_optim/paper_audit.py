...
ACTIVE_MANUSCRIPT_SOURCES = (
    ...
    Path("paper/generated/optimizer-primary.tex"),
    Path("paper/generated/dimension-utilization.tex"),
    Path("paper/generated/state-operator-factorial.tex"),
)
PENDING_MANUSCRIPT_SOURCES = tuple(
    path for path in ACTIVE_MANUSCRIPT_SOURCES if path != Path("paper/results.tex")
)
MANUSCRIPT_ENGINEERING_PATTERNS = {
    "packing": re.compile(r"\bpack(?:ed|ing)?\b", re.IGNORECASE),
    "padding": re.compile(r"\bpad(?:ded|ding)?\b", re.IGNORECASE),
    "candidate-breadth": re.compile(r"\bcandidate[- ]breadth\b", re.IGNORECASE),
    "execution-path": re.compile(r"\bexecution[- ]path\b", re.IGNORECASE),
    "failed-low-level-operation": re.compile(
        r"\b(?:addmm|kernel\s+(?:bug|crash|failure))\b", re.IGNORECASE
    ),
}
...
def _paper_main_topology_complete(root: Path) -> bool:
    ...
    weight_finding = main_text.find(r"\CorrectedWeightSpaceFinding")
    geometry_bridge = main_text.find(r"\CorrectedGeometryBridgeFinding")
    dimension_finding = main_text.find(r"\DimensionUtilizationFinding")
    dimension_figure = main_text.find(r"\DimensionUtilizationFigure")
    dimension_bridge = main_text.find(r"\DimensionRetrievalBridgeFinding")
    ...
    dimension_conclusion = main_text.find(r"\DimensionConclusionFinding")
    ...
    corrected_input = r"\input{generated/optimizer-primary}"
    corrected_input_position = main_text.find(corrected_input)
    dimension_input = r"\input{generated/dimension-utilization}"
    dimension_input_position = main_text.find(dimension_input)
    ...
    corrected_bridge_table = main_text.find(r"\CorrectedGeometryBridgeTable")
    dimension_table = main_text.find(r"\DimensionUtilizationAppendixTable")
    state_operator_table = main_text.find(r"\StateOperatorAppendixTable")
    ...
    return (
        ...
        < appendix
        < artifact
        < corrected_bridge_table
        < dimension_table
        < state_operator_table
...
def _final_document_language_problems(root: Path) -> list[str]:
    ...
    for relative in ACTIVE_MANUSCRIPT_SOURCES:
        path = root / relative
        if not path.is_file():
            continue
        try:
            ...
            continue
        problems.extend(
            f"{relative}: scientific-scope:{label}"
            for label, pattern in MANUSCRIPT_ENGINEERING_PATTERNS.items()
            if pattern.search(text)
        )
    return problems


def _active_manuscript_pending(root: Path) -> list[dict[str, Any]]:
    pending = []
    for relative in PENDING_MANUSCRIPT_SOURCES:
        path = root / relative
        if not path.is_file():
            pending.append({"path": str(relative), "status": "missing", "markers": None})
            continue
        try:
            markers = _strip_latex_comments(path.read_text(encoding="utf-8")).count(
                r"\ResultPending"
            )
        except (OSError, UnicodeDecodeError):
            pending.append({"path": str(relative), "status": "unreadable", "markers": None})
            continue
        if markers:
            pending.append({"path": str(relative), "status": "pending", "markers": markers})
    return pending
...
def _primary_publication_status(root: Path) -> dict[str, Any]:
    """A populated primary include is insufficient: reconstruct it from its sources."""
    from .corrected_publication import audit_report, parse_args

    summary = root / "reports/dense-no-packing-publication/summary_manifest.json"
    if not summary.is_file():
        return {"complete": False, "status": "prospective_pending", "summary_present": False}
    args = parse_args([])
    for name, value in vars(args).items():
        if isinstance(value, Path):
            setattr(args, name, root / value)
    try:
        manifest = audit_report(args)
    ...
    return {
        ...
        "validation_scope": "source_bound_exact_publication_reconstruction",
    }


def _dimension_publication_status(root: Path) -> dict[str, Any]:
    """Require closed, recomputable primary dimension findings for final release."""
    from .dimension_publication import audit_report
    from .dimension_release import PUBLICATION_DIR, publication_args

    summary = root / PUBLICATION_DIR / "summary_manifest.json"
    if not summary.is_file():
        return {"complete": False, "status": "prospective_pending", "summary_present": False}
    try:
        manifest = audit_report(publication_args(root), portable=True)
    except (OSError, KeyError, TypeError, ValueError) as error:
        return {
            "complete": False,
            "status": "invalid_or_stale",
    ...
    return {
        ...
        "summary_present": True,
        "coverage": manifest["coverage"],
        "validation_scope": "portable_publication_recomputation",
    }
...
def audit_paper(
    ...
    *,
    ...
    active_manuscript_pending = _active_manuscript_pending(root)
    ...
    dimension_publication = _dimension_publication_status(root)
    primary_publication = _primary_publication_status(root)
    complete = (
        not pending
        and not incomplete_evidence
        and paper_results_complete
        and not document_language_problems
        and not active_manuscript_pending
        and abstract_word_budget["complete"]
        and causal_chain_complete
        and state_operator_publication["complete"]
        and dimension_publication["complete"]
        and primary_publication["complete"]
    )
    ...
    if strict and not complete:
        raise ValueError(
            ...
            f"active_manuscript_pending={active_manuscript_pending}, "
            ...
            f"dimension_publication={dimension_publication['status']}, "
            f"primary_publication={primary_publication['status']}"
        )
    return result
