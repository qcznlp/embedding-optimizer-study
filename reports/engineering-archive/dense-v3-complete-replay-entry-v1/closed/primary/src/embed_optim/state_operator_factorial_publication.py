...
def _interpretation(rows: dict[str, dict[str, Any]]) -> str:
    ...
    if "state_operator_interaction" in positive:
        prefix = (
            "The relative Muon-versus-AdamW continuation effect is more favorable from the "
            "Muon-reached state at the tested horizon; this need not mean Muon helps "
            "from either state"
        )
        additions = []
        if "weight_state_effect" in positive:
            additions.append("a positive averaged source-state contrast")
        if "operator_effect" in positive:
            additions.append("a positive averaged continuation contrast")
        return (
            prefix
            + (" alongside " + " and ".join(additions) if additions else "")
            + ". This interaction does not establish full-trajectory co-adaptation."
        )
    if positive == {"weight_state_effect"}:
        return (
            "The averaged post-continuation source-state contrast favors the Muon-reached "
            "state. This does not establish its dominance over the other contrasts; "
            "inconclusive intervals are not equivalence to zero."
        )
    if positive == {"operator_effect"}:
        return (
            "The continuation contrast favors Muon after averaging the two fixed source states. "
            "It need not favor Muon within each state or dominate the other contrasts."
        )
    if positive == {"weight_state_effect", "operator_effect"}:
        return (
            "The averaged source-state and continuation contrasts both favor Muon at this "
            "horizon. Their positive signs do not imply an additive response; the "
            "interaction must be interpreted separately."
        )
    if positive:
        labels = ", ".join(sorted(name.replace("_", " ") for name in positive))
        return f"Positive evidence is confined to {labels}; the remaining contrasts are reported separately, without an equivalence claim."
    if negative:
        labels = ", ".join(sorted(name.replace("_", " ") for name in negative))
        return (
            f"No averaged contrast supports a Muon benefit; {labels} is supported in the "
            "opposite direction. These are endpoint contrasts, not a decomposition "
            "of the primary-training result."
        )
    return (
        "All three post-continuation contrasts are inconclusive under their marginal "
        "intervals. This is not evidence of equivalence or absence of an effect."
    )
...
def _render_latex(rows: dict[str, dict[str, Any]]) -> str:
    ...
    abstract = (
        "At the 50K continuation endpoint, state, operator, and interaction contrasts are "
        f"{_effect(weight)}, {_effect(operator)}, and {_effect(interaction)} nDCG@10. "
        "They do not decompose the primary-training contrast."
    )
    main = (
    ...
    conclusion = (
        "After optimizer-state reset and calibration-probe scale matching, the crossed "
        f"50K continuation finds state, operator, and interaction contrasts of {_effect(weight)}, "
    ...
    return "\n".join(
        (
            ...
            *effect_lines,
            ...
            "two-way seed/task cluster bootstrap. These are marginal 95\\% intervals, not "
...
def _expected_manifest(
    ...
    return {
        ...
        "claim_boundary": (
            "The crossed branch estimates post-continuation source-state, averaged operator, "
            "and interaction contrasts for one source-checkpoint pair and one 50K horizon. "
            "It does not decompose the primary 500K-training contrast, establish a measured "
            "geometry feature as a mediator, or turn inconclusive intervals into equivalence."
        ),
    }
