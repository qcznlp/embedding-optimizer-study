...
before = json.loads((MONITOR / 'evaluation-20260913T0352Z.json').read_text())
after = json.loads((MONITOR / 'evaluation-20260913T0358Z.json').read_text())
...
assert len(old) == 22 and len(current) == 24 and len(new) == 2
assert all(row[2:] == (391, 'FEVER') and row[1].endswith('seed161803') for row in new)
new_tasks = []
for pool, run, step, task in new:
    ...
    for item in verified['files']:
        ...
        assert path.name in ('FEVERDecontaminated.json', 'model_meta.json', 'run_settings.jsonl')
...
for pool, state in after['pools'].items():
    ...
    for worker in state['task_workers']:
        ...
        assert step == 391 and task == 'MSMARCO'
        path = ENTRY / 'run' / ('pool-' + pool) / 'jobs' / (run + '-391-MSMARCO.started.json')
...
observations = (
    'evaluation-20260913T0352Z.json', 'evaluation-20260913T0356Z.json', 'evaluation-20260913T0358Z.json',
    'summary-20260913T0353Z.json', 'document-20260913T0353Z.json', 'combined-20260913T0353Z.json',
    'resume-20260913T0353Z.json', 'outcomes-20260913T0353Z-cpu-only.json',
    'outcomes-observer-launch-20260913T0352Z.failed-tool.json', 'outcomes-observer-launch-20260913T0353Z.success-tool.json',
    'evaluation-session-events-20260913T0356Z.json',
)
...
record = {
    ...
    'observed_at_utc': after['observed_at_utc'], 'before_completed_tasks': 22, 'completed_tasks': 24,
...
with (OUT / 'readback.json').open('x') as handle:
    ...
    handle.write('\n')
print(json.dumps({'completed_tasks': 24, 'new_completed_tasks': new_tasks, 'new_starts': new_starts, 'copied_files': len(copies), 'readback': identity(OUT / 'readback.json'), 'scientific_inference_generated': False}))
