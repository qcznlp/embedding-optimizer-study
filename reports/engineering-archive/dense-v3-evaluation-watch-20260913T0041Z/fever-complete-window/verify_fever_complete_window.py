...
OUT = REPORT / 'fever-complete-window'
...
before = json.loads((MONITOR / 'evaluation-20260913T0406Z.json').read_text())
after = json.loads((MONITOR / 'evaluation-20260913T0411Z.json').read_text())
...
assert len(old) == 24 and len(current) == 26 and len(new) == 2
assert all(row[2:] == (391, 'FEVER') and row[1].endswith('seed161803') and row[1].startswith('factorial-v3-muon_state-') for row in new)
new_tasks = []
...
assert sum(w['job'][2] == 'MSMARCO' for w in active) == 8

observations = (
    'evaluation-20260913T0406Z.json', 'evaluation-20260913T0411Z.json',
    'summary-20260913T0411Z.json', 'document-20260913T0411Z.json', 'combined-20260913T0411Z.json',
    'resume-20260913T0411Z.json', 'outcomes-20260913T0411Z.json',
    'fever-final-session-events-20260913T0411Z.json', 'fever-final-observer-tools-20260913T0411Z.json',
)
...
record = {
    ...
    'observed_at_utc': after['observed_at_utc'], 'before_completed_tasks': 24, 'completed_tasks': 26,
...
with (OUT / 'readback.json').open('x') as handle:
    ...
    handle.write('\n')
print(json.dumps({'completed_tasks': 26, 'new_completed_tasks': new_tasks, 'new_starts': new_starts, 'copied_files': len(copies), 'readback': identity(OUT / 'readback.json'), 'scientific_inference_generated': False}))
