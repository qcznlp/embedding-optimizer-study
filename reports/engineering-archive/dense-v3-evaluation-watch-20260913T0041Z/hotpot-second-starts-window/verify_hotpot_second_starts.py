...
OUT = REPORT / 'hotpot-second-starts-window'
...
before = json.loads((MONITOR / 'evaluation-hotpot-first-starts.json').read_text())
after = json.loads((MONITOR / 'evaluation-hotpot-second-starts.json').read_text())
...
assert len(old) == 32 and len(current) == 34 and len(new) == 2
assert all(row[2:] == (391, 'MSMARCO') and row[1].endswith('seed271828') and row[1].startswith('factorial-v3-muon_state-') for row in new)
new_tasks = []
...
assert sum(w['job'][2] == 'MSMARCO' for w in active) == 4
assert sum(w['job'][2] == 'HotpotQA' for w in active) == 4

observations = (
    'evaluation-hotpot-first-starts.json', 'evaluation-hotpot-second-starts.json',
    'summary-hotpot-second-starts.json', 'document-hotpot-second-starts.json', 'combined-hotpot-second-starts.json',
    'resume-hotpot-second-starts.json', 'outcomes-hotpot-second-starts.json',
    'hotpot-second-starts-session-events.json', 'hotpot-second-starts-observer-tools.json',
)
...
record = {
    ...
    'observed_at_utc': after['observed_at_utc'], 'before_completed_tasks': 32, 'completed_tasks': 34,
...
with (OUT / 'readback.json').open('x') as handle:
    ...
    handle.write('\n')
print(json.dumps({'completed_tasks': 34, 'new_completed_tasks': new_tasks, 'new_starts': new_starts, 'copied_files': len(copies), 'readback': identity(OUT / 'readback.json'), 'scientific_inference_generated': False}))
