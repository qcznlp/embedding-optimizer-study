...
OUT = REPORT / 'msmarco-complete-window'
...
before = json.loads((MONITOR / 'evaluation-hotpot-second-starts.json').read_text())
after = json.loads((MONITOR / 'evaluation-msmarco-complete.json').read_text())
...
assert len(old) == 34 and len(current) == 40 and len(new) == 6
assert sum(row[3] == 'MSMARCO' for row in new) == 4
assert sum(row[3] == 'HotpotQA' for row in new) == 2
assert all(row[2] == 391 and ((row[3] == 'MSMARCO' and row[1].endswith('seed161803')) or (row[3] == 'HotpotQA' and row[1].endswith('seed314159') and row[1].startswith('factorial-v3-adamw_state-'))) for row in new)
new_tasks = []
for pool, run, step, task in new:
    ...
    for item in verified['files']:
        ...
        assert path.name in (task + 'Decontaminated.json', 'model_meta.json', 'run_settings.jsonl')
...
assert len(active) == 8 and len(new_starts) == 6
assert sum(w['job'][2] == 'MSMARCO' for w in active) == 0
assert sum(w['job'][2] == 'HotpotQA' for w in active) == 8

observations = (
    'evaluation-hotpot-second-starts.json', 'evaluation-msmarco-complete.json',
    'summary-msmarco-complete.json', 'document-msmarco-complete.json', 'combined-msmarco-complete.json',
    'resume-msmarco-complete.json', 'outcomes-msmarco-complete.json',
    'msmarco-complete-session-events.json', 'msmarco-complete-observer-tools.json',
    'msmarco-complete-current-metadata-check-tool.json',
)
...
record = {
    ...
    'observed_at_utc': after['observed_at_utc'], 'before_completed_tasks': 34, 'completed_tasks': 40,
...
with (OUT / 'readback.json').open('x') as handle:
    ...
    handle.write('\n')
print(json.dumps({'completed_tasks': 40, 'new_completed_tasks': new_tasks, 'new_starts': new_starts, 'copied_files': len(copies), 'readback': identity(OUT / 'readback.json'), 'scientific_inference_generated': False}))
