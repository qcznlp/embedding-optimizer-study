...
OUT = REPORT / 'msmarco-first-batch-window'
...
before = json.loads((MONITOR / 'evaluation-20260913T0417Z.json').read_text())
after = json.loads((MONITOR / 'evaluation-msmarco-first-batch.json').read_text())
...
assert len(old) == 26 and len(current) == 30 and len(new) == 4
assert all(row[2:] == (391, 'MSMARCO') and row[1].endswith('seed314159') for row in new)
new_tasks = []
...
assert len(active) == 8 and len(new_starts) == 4
assert sum(w['job'][2] == 'MSMARCO' for w in active) == 8

observations = (
    'evaluation-20260913T0417Z.json', 'evaluation-msmarco-first-batch.json',
    'summary-msmarco-first-batch.json', 'document-msmarco-first-batch.json', 'combined-msmarco-first-batch.json',
    'resume-msmarco-first-batch.json', 'outcomes-msmarco-first-batch.json',
    'msmarco-first-batch-session-events.json', 'msmarco-first-batch-observer-tools.json',
)
...
record = {
    ...
    'observed_at_utc': after['observed_at_utc'], 'before_completed_tasks': 26, 'completed_tasks': 30,
...
with (OUT / 'readback.json').open('x') as handle:
    ...
    handle.write('\n')
print(json.dumps({'completed_tasks': 30, 'new_completed_tasks': new_tasks, 'new_starts': new_starts, 'copied_files': len(copies), 'readback': identity(OUT / 'readback.json'), 'scientific_inference_generated': False}))
