"""Compile the complete active-v3 topology in an explicitly synthetic paper copy."""
...
from embed_optim.primary_v3_paper_layout import audit_paper_layout
...
def run(args):
    ...
    source_paths = (
        ...
        root / "tests/test_state_operator_factorial_completion.py",
        root / "src/embed_optim/paper_layout.py",
        root / "src/embed_optim/primary_v3_paper_layout.py",
    ...
    result = subprocess.run(
        ["make", "build/main.pdf", "PYTHON=" + sys.executable],
        cwd=paper,
        capture_output=True,
        text=True,
        timeout=55,
    ...
    for row in (*originals, *sources, *generation["sources"]):
        verify_file(row["path"], row)
    assert file_identity(args.generation)["sha256"] == args.generation_sha256
    require_same(handoff(), before)
    receipt = {
        "scope": "engineering_complete_synthetic_exact_publication_layout",
        "passed": True,
