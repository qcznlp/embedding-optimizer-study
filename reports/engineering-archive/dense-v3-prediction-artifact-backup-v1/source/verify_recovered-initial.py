...
def check_predictions(predictions, folds, summaries, states, exploratory):
    ...
    for r in folds:
        ...
        if not exploratory:
            need(Q(r['mse_reduction_exact']) == b-f and r['rmse_reduction'] == rms(b)-rms(f), 'Fold reduction differs')
    ...
    for r in summaries:
        ...
        need(Q(r['pooled_baseline_mse_exact']) == b and Q(r['pooled_feature_mse_exact']) == f
             ...
             and r['pooled_rmse_reduction'] == rms(b)-rms(f), 'Pooled arithmetic or decision differs')
        if not exploratory:
