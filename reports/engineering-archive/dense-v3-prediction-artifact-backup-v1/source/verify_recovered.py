...
def check_predictions(predictions, folds, summaries, states, exploratory):
    ...
    for r in summaries:
        ...
        result[key] = (rms(b), rms(f), improved, flag, rms_reduction(b, f))
...
def verify(root, manifest_sha):
    ...
    for b, values in (('locked', locked), ('sensitivity', sensitivity)):
        ...
        for p in plotted:
            ...
            seen.add(key); base, feature, improved, flag, reduction = values[key]
            ...
            if b == 'sensitivity':
                need(float(p['feature_minus_baseline_rmse_points']) == -100*reduction, 'Plot delta differs')
