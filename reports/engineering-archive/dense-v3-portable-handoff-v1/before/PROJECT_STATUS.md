...
## Scientific results complete; paper and source closeout — 2026-09-13

**07:55 UTC — stable complete-paper source and actual packaged rebuild.**
Read the [current-paper entry report](reports/engineering-archive/dense-v3-current-paper-entry-v1/README.md).
The twelve reviewed paper inputs now live at `paper/current`, authenticated
by snapshot 6ef28bcf1f2cd873bcb0a663c2baea57ec6dc98a4e441d0850608a61f46ae6b9.
The complete document checker is copied byte-identically into the package;
new `current_paper` and `make current` invoke it with explicit local inputs
and new output directories. Old `all`/`release` and their gate remain unchanged.

Initial 65 controls pass (69537/65b595). Actual Make target passes
(4778/50428f): 158 words, eight-page main, thirteen total pages, 25 embedded
non-Type-3 fonts. Expanded tests initially expose one omitted newly created
continuation-outcomes restoration guide (49464/583863, 85 pass/one fail).
After adding the guide, all 86 controls pass (30859/66d481); focused final
Ruff/format/diff checks pass (77d86f). The initial formatting failure remains
recorded; only new authored wrapper/tests were formatted.

Actual package check 81331/59d2ab exits zero: final sdist-to-wheel build exit
zero, original full distribution audit exit one with the same ten producer
paths, actual wheel-local document child exit zero. It checks all 28 imported
study modules are from the extracted wheel and the complete PDF text matches
the reviewed manuscript. Final wheel f37f33b7... has 426 members; 213 declared
data files, including the current manuscript and outcome-restoration guide.
This is same-host document relocation, not a physical new host or source
release. No scientific recomputation or external write; all jobs terminal.
**PROGRESS** in usable source/package delivery. Scientific-consumer integration,
remaining distribution findings, exact-resume divergence and publication remain.

**07:40 UTC — complete-result manuscript revised and fully visually reviewed.**
Read the [revision report and PDF](reports/paper-review/dense-v3-complete-manuscript-revision-v1/README.md).
Five deferred corrections, six closest references and a result-led reached-state
versus continuation-rule narrative are integrated. Twelve original document
inputs authenticated before copying; only main prose and bibliography change.
Original strict complete-document builder 28526/4ef483 exits zero: 158-word
abstract, main end page 8, thirteen total pages and 25 embedded non-Type-3
fonts. All thirteen actual pages were visually inspected; no missing/clipped
result or reference. Native generated evidence is unchanged. Capture ed7d38
preserves 57 paper/source/review files, 6,548,395 bytes, before ops updates.
This is a reviewed local candidate, not authoritative install/source release.

Both repaired recovery attempts are now terminal: 39663/8ceca3 and
75985/1672c5 exit one. All eight GPU ranks complete 78 updates and exit zero;
the fresh CPU readers fail the original exact model comparison. Read the
[endpoint diagnosis](reports/engineering-archive/dense-v3-resume-endpoint-diagnosis-v1/README.md).
Actual diagnostic 39062/b7f32d exits zero after complete native inventory
authentication: all 134 model tensors differ in each case, optimizer moment
leaves differ, scheduler/all-rank RNG/counters agree exactly. Maximum absolute
model differences are 0.000442043 and 0.00233306, descriptions not tolerances.
Cause remains unresolved; no backend is blamed without a matched boundary
check. Capture ed7d38 preserves 58 diagnosis/rank/reader/source files,
1,377,121 bytes. Both old and repaired failures remain failures.

**PROGRESS**: actual prose integration, strict fresh build, complete visual
review and exhaustive diagnosis. No new science or external write. No study
session remains live. Stable v3 source/consumer integration, distribution
admission, recovery divergence and final publication remain; goal is active.
All earlier dated live-job snapshots below are historical.

**07:17 UTC — restoration adapter and actual repaired GPU checks running.**
Read the [scoped recovery evidence](reports/engineering-archive/dense-v3-resume-device-recovery-v2/README.md).
The new helper validates complete named Adam state and moves only loaded scalar
counters, preserving FP32 bits and original moment/group objects. It does not
replace native loading or numerical kernels. 29 CPU helper controls (961c0f)
and 37 recovery/callback controls (61da0c) pass; actual preparation 1f599c passes.
Original HF download/native admission is reused without false new provenance.

Actual launches 429b45/8cc7fa yield sessions **39663 / 75985**. Observer
bdac2e confirms two live/S coordinators, eight direct live/R ranks and both
dual-lease admissions at 07:12:37. Actual eight-rank device-placement readback
0f678d passes at 07:13:34–37: each AdamW rank moves 134 counters, each Muon
rank 46 auxiliary counters, exact original bits retained. Both logs reach
at least step 318/391 (865c79), beyond the old first-update failure.
This is **PROGRESS** in a scoped implementation and actual GPU execution, not
endpoint equality, a new scientific run or release. The original two failed
attempts remain failed; native endpoint readers are pending. Archive 9dfb47
copies/checks 44 source/launch/placement files / 765,681 bytes. Original
science/numerical/PDF replay/backups remain complete. Final paper work continues.
At 07:17:43, exact observer eec176 confirms all eight ranks and both
coordinators live with no failure; rank-zero logs show 347/391 in each pool
(48f452). No endpoint reader has run and equivalence is still pending.

**06:56 UTC — full numerical/PDF replay and anonymous outcome backup complete.**
Backup 35024/d0cf11 exits zero at 06:48:52: all 593 files / 5,199,649 bytes
downloaded anonymously and verified. Revision
7766dd0ae83e36a4c16121703813f6a9eec63336; manifest
296c688d47617986d297044f901f0c0e1e41e552a1e06110cff8f6bf01770230.
The new [restoration guide](docs/continuation-outcomes-restoration.md) records
actual locations and per-file download/checksum instructions; its code parses,
but no duplicate download was performed merely to test documentation.

The original combined replay 56449/ef4678 exits one: primary numerical
reconstruction succeeds but records 48 refused opens for two nonexistent
editable-import path placeholders resolved from the outer working directory.
The unchanged zero-refusal gate rejects the original attempt. A new recorded
invocation of the same sealed code/manifest from its own component directory
succeeds, session 96872/acf598 exit zero at 06:53:05. Both original I/O guards
have zero refused opens/network attempts and no exception; all primary and
factorial numerics, paper source/extracted text and strict document checks
pass. No source, manifest, tolerance or guard changed, and old failure remains.
Completion SHA 6a0a8a3657d88a20529db8d45d32c31c5b74be783c0abb9d57994b0009d3c275.
Capture ec33e5 preserves 31 proof files / 1,444,738 bytes. All tracked sessions
are terminal. Both GPU resume failures remain unresolved, with the narrowly
identified state-device repair still unimplemented. Final prose/visual review
and source release remain. This is **PROGRESS**, not full goal completion.

**06:46 UTC — full evaluation/inference/native paper complete; resume bug isolated.**
Read the [native closeout](reports/engineering-archive/dense-v3-final-evaluation-closeout-v1/README.md).
All 168 original evaluation workers pass; both coordinators exit zero, d4b9d9
and 6a4b9d. Native summary 9793ed exits zero at 06:44:15 after both real
collectors and independent arithmetic/bootstrap verification: 168/4/126/3/60/840
table rows. State effect +0.3222 points, reset-operator -0.5264, interaction
inconclusive; three marginal intervals, not universal optimizer or mediation
claims. Actual document a07a56 exits zero at 06:44:48, 161-word abstract and
main endpoint page 8; full visual review and final prose remain pending.
Actual combined replay is live (bf35c3); HF outcome upload and remote hashes
pass, with anonymous full download still active (3eaa11). No source release.

Both separate GPU resume attempts fail, e5a992/083fed exit one; all eight
ranks exit one and no comparison reader runs. Every rank reports Adam step
state on CPU while the original fused kernel runs on its CUDA device. The
base loader keeps step location without fused/capturable group flags, whereas
the custom optimizer chooses fusion dynamically. This is a restore-path bug;
fresh-state scientific runs/evaluations remain complete. The original attempts,
logs and sources are preserved, not retried, replaced or tolerance-relaxed.
Only a separately recorded device-placement repair is planned, not implemented.
Metadata-only capture 825345 copies/checks 88 actual inference/document/failure
files, 3,242,117 bytes. This turn is **PROGRESS** in completed evaluation,
real statistical results/document generation and diagnosis of a real recovery
failure, with original combined/backup jobs still live. Full goal remains active.
