...
Source distribution is still not admitted. Read the latest actual audit, not an old build count.
Preserve all producer-path findings until genuinely resolved; never hide strings, remove negative
controls or weaken the scanner. Successful packaging or copied-input loading does not prove
every runtime entry works. Same-host relocation is not a physical second-host test.

## Next work

1. Integrate the already completed genuine scientific consumers and corrected numerical source
   into stable, portable source entry points. Source trees are still distinct; do not mistake
   the document entry for an install-and-train/reproduce-every-statistic release.
2. Resolve actual distribution/portability findings while retaining historical provenance and
   semantic negative controls. Keep current documentation separate from chronological archives.
