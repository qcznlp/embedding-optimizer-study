...
1. **Complete scientific source integration.** The verified numerical/runtime sources and
   scientific consumers still span distinct frozen assemblies. The stable current-paper
   entry solves document reproduction only, not training or all-statistic reproduction
   through a unified release. Integrate explicit location roles and genuine accepted parents.
2. **Distribution admission.** The original full audit still fails producer-path findings.
   Separate portable runtime roles/current documentation from retained historical provenance.
   Do not conceal strings, drop negative controls or weaken the scanner. The exact latest
   scope/result is in the [portable handoff report](reports/engineering-archive/dense-v3-portable-handoff-v1/README.md).
