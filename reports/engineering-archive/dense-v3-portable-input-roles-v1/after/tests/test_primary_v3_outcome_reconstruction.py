...
def test_cold_cli_uses_only_archived_sources(archive, contract, tmp_path):
    root, anchor = archive
    # Keep the actual historical producer forbidden, deriving its namespace from
    # this contract's already authenticated probe receipt rather than the host.
    probe_paths = [
        producer_path(row["path"]) for row in contract.dimensions.probe_acceptance["inputs"]
    ]
    manifests = [path for path in probe_paths if path.name == "manifest.json"]
    assert len(manifests) == 1
    suffix = Path("data/probes/decontaminated-beir-224-seed4242/manifest.json")
    assert manifests[0].parts[-len(suffix.parts) :] == suffix.parts
    producer_root = manifests[0].parents[len(suffix.parts) - 1]
    assert all(path.is_relative_to(producer_root / suffix.parent) for path in probe_paths)
    forbidden_roots = [str(ROOT), str(producer_root), str(root.parent / "producer")]
    code = """
...
# Exercise every original negative-control namespace before the actual replay.
for forbidden_root in forbidden:
    try:
        open(str(Path(forbidden_root)/'must-not-be-opened'), 'rb')
    except RuntimeError as error:
        assert str(error) == 'Original source/producer forbidden'
    else:
        raise AssertionError('Producer guard did not refuse before opening')
...
def test_semantic_refusals_after_all_envelopes_are_rehashed(archive, tmp_path, case):
    target = tmp_path / ("rehashed-" + case)
    shutil.copytree(archive[0], target)
    anchor = mutate(target, case)
    # The externally re-anchored transport and executing-source checks both pass.
    inputs = load((target, anchor))
    output = tmp_path / "refused"
    with pytest.raises((ValueError, FileNotFoundError)) as refusal:
        replay.reconstruct(inputs, output)
    assert "checksum" not in str(refusal.value).lower()
    assert "executing package source" not in str(refusal.value).lower()
    if case == "beir_missing_file":
        assert str(refusal.value) == "Require an ordinary source/payload file"
    assert not (output / "reconstruction.json").exists()
    if case == "outcome_table":
        assert "differs from fresh evidence/statistical recomputation" in str(refusal.value)
        assert (output / "outcomes/primary_summary.csv").is_file()
