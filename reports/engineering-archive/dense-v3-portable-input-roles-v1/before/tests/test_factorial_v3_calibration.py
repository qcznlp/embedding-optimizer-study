...
def test_declared_input_roles_never_fall_back_to_historical_host(tmp_path):
    roles = inputs_module.Locations(
        *(tmp_path / name for name in ("repo", "primary", "experiment", "data", "evidence"))
    )
    relocated = roles.locate(
        "/root/embedding-optimizer-story-refactor/" + inputs_module.EVIDENCE + "/data-first.json"
    )
    assert relocated == tmp_path / "evidence/data-first.json"
    for path in (
        "/unbound/file",
        "/root/embedding-optimizer-study/not-data/file",
        "/root/embedding-optimizer-study/data/../file",
    ):
        with pytest.raises(ValueError):
            roles.locate(path)
...
def test_symlink_role_refused(tmp_path):
    ...
    with pytest.raises(ValueError):
        roles.locate("/root/embedding-optimizer-story-refactor/configs/common_state_probe.json")
