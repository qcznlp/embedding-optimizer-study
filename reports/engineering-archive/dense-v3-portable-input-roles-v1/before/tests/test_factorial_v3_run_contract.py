...
def component(identity):
    return {
        ...
        "input_collator": copy.deepcopy(contract.COLLATOR_POLICY),
        "checkpoint_callback": copy.deepcopy(contract.CHECKPOINT_POLICY),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (("input_collator", "prompts", "query"), "document: "),
        (("checkpoint_callback", "steps"), [391]),
    ],
)
...
def test_canonical_handlers_bind_same_model_and_every_checkpoint(tmp_path):
    from sentence_transformers import SentenceTransformer
    from transformers import TrainerControl

    # A method-identity fixture only: no model load, forward or Trainer creation.
    model = object.__new__(SentenceTransformer)
    collator, callback = bound.input_handlers(model, tmp_path)
    bound.require_collator(collator, model)
    callback.on_train_begin(None, TrainerState(max_steps=391), TrainerControl())
    assert callback.targets == contract.STEPS
    for step in contract.STEPS:
        control = callback.on_step_end(None, TrainerState(global_step=step), TrainerControl())
        assert control.should_save


@pytest.mark.parametrize("change", ["different_model", "changed_prompt", "changed_preprocessor"])
def test_changed_live_prompt_handler_refused(tmp_path, change):
    from sentence_transformers import SentenceTransformer

    model = object.__new__(SentenceTransformer)
    collator, _ = bound.input_handlers(model, tmp_path)
    if change == "different_model":
        model = object.__new__(SentenceTransformer)
    elif change == "changed_prompt":
        collator.inner.prompts["query"] = ""
    elif change == "changed_preprocessor":
        collator.inner.preprocess_fn = lambda *args, **kwargs: None
    with pytest.raises(ValueError):
        bound.require_collator(collator, model)


@pytest.mark.parametrize("setting", ["data_collator", "callbacks"])
def test_custom_input_or_callback_cannot_enter_bound_constructor(setting):
    from types import SimpleNamespace

    kwargs = {setting: object() if setting == "data_collator" else [object()]}
    with pytest.raises(ValueError, match="owns its prompt"):
        bound.RunBoundFactorialTrainer(
            run_identity=metadata_identity(), args=SimpleNamespace(use_cpu=False), **kwargs
        )
