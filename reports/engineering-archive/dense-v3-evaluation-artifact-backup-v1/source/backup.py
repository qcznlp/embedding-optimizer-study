"""Additions-only backup of complete v3 endpoint and validation scores, never code."""
...
BEIR = REPOSITORY / "reports/engineering-archive/dense-v3-all-final-evaluations-v1"
VALIDATION = REPOSITORY / "reports/engineering-archive/dense-v3-validation-selection-readback-v1"
BEIR_READBACK_SHA = "b8c9aff0f580e3fee1d8f239b2943ef649ec154ae5767ee576ee47b38e027fd6"
VALIDATION_READBACK_SHA = "1bfad0688b108f1e0546af888b508b9a3d5de87350e46f925f57585e04b545c7"
SECRETS = re.compile(
...
def select_sources():
    ...
    beir = read(BEIR / "actual/all-twelve-final-readback.json", BEIR_READBACK_SHA)
    validation = read(VALIDATION / "actual/all-twelve-validation-readback.json", VALIDATION_READBACK_SHA)
    require(beir["protocol_sha256"] == PROTOCOL and len(beir["checkpoints"]) == 12
            and beir["scientific_completion"] is False and beir["full_primary_grid_complete"] is False,
            "Wrong complete endpoint evidence")
    require(validation["run_count"] == 12 and validation["scored_rows"] == 49152
            and validation["scientific_completion"] is False
            and validation["beir_scores_read_by_this_reader"] is False,
            "Wrong full validation evidence")
    selected = {}

    def add(prefix, root, item):
        source = Path(item["path"])
        name = f"{prefix}/{safe_name(source.relative_to(root).as_posix())}"
        binding = {key: item[key] for key in ("bytes", "sha256")}
        value = (source, binding)
        require(name not in selected or selected[name] == value, "Conflicting native input")
        selected[name] = value

    runs = set()
    for row in beir["checkpoints"]:
        require(row["step"] == 3907 and row["task_count"] == 14
                and row["run_id"] not in runs and len(row["original_workers"]) == 14,
                "Incomplete or duplicated final checkpoint")
        runs.add(row["run_id"])
        for item in [row["original_complete_receipt"], row["original_operational_receipt"],
                     *row["raw_score_and_metadata_snapshots"]]:
            add("native/beir-final", EXPERIMENT / "evaluations/dense-primary-v3", item)
    ...
    require(len(selected) == 552, "Require all 216 native BEIR files and 336 worker records")
    require(runs == {row["run_id"] for row in validation["native_selection_reread"]["run_metrics"]},
            "Validation and BEIR recipe populations differ")
    require(len(validation["raw_output_files"]) == 48 and len(validation["workers"]) == 12,
            "Incomplete original validation file population")
    for item in validation["raw_output_files"]:
        add("native/validation", EXPERIMENT / "evaluations/dense-primary-v3-validation", item)
    for worker in validation["workers"]:
        require(len(worker["files"]) == 4, "Incomplete validation worker proof")
        for item in worker["files"].values():
            add("provenance/validation-workers", EXPERIMENT / "launch/validation-handoff/run/jobs", item)
    require(len(selected) == 648, "Incomplete original measurement/provenance population")
    for name, key in (("all-twelve-validation-selection.json", "run/all-twelve-validation-selection.json"),
                      ("completed.json", "run/completed.json")):
        item = validation["original_bindings"][key]
        selected[f"native/validation-selection/{name}"] = (
            VALIDATION / "actual" / name, {k: item[k] for k in ("bytes", "sha256")})
    selected["provenance/validation-native-readback.json"] = (
        VALIDATION / "actual/all-twelve-validation-readback.json", {"sha256": VALIDATION_READBACK_SHA})
    for root, name, expected_sha in (
        (BEIR, "final-checkpoints", "ae86c2fa5f4e083b8ff5597f539f342fabd35e746bf7c739c574273c52820568"),
        (VALIDATION, "validation", "4bc2e0280665396489c53e22d012c3beb6feed47f59f8897a79339097de480d3"),
    ):
        manifest = read(root / "tables/manifest.json", expected_sha)
        require(len(manifest["outputs"]) == 3, "Unexpected generated table population")
        for filename, item in manifest["outputs"].items():
            selected[f"tables/{name}/{safe_name(filename)}"] = (
                root / "tables" / filename, {k: item[k] for k in ("bytes", "sha256")})
    ...
    require(len(selected) == 659, "Unexpected snapshot scope")
    return selected
        ...
        if path.suffix in (".json", ".jsonl"):
            ...
            if path.suffix == ".json":
                ...
                for line in raw.splitlines():
                    require(bool(line.strip()), "Blank JSONL record refused")
...
def prepare(work):
    ...
    for name, (source, expected) in sorted(sources.items()):
        require(
            source.suffix in {".json", ".jsonl", ".csv", ".md"},
    ...
    manifest = {
        "schema_version": 1,
        "scope": "complete-primary-v3-final-beir-and-validation-artifact-backup",
        "created_at_utc": stamp(),
        "primary_protocol_sha256": PROTOCOL,
        "scientific_completion": False,
        "source_code_included": False,
        "runs": 12,
        "checkpoint_stages": [3907],
        "beir_task_cells": 168,
        "validation_rows_per_run": 4096,
        "native_measurement_files": 264,
        "original_worker_records": 384,
        "payload_files": len(files),
        "payload_bytes": sum(f["bytes"] for f in files.values()),
        "files": files,
        "boundary": "Unchanged complete final BEIR and validation scores/metadata only; all 48 intermediate checkpoints remain required. No raw examples, code release, new model execution or functional/causal inference.",
    }
    write_new(destination / "artifact_manifest.json", manifest)
    manifest_id = file_identity(destination / "artifact_manifest.json")
    preflight = {
        "scope": "source-bound-additions-only-complete-evaluation-backup-preflight",
...
def compare_preserved(before, after, before_subtree, after_subtree):
    ...
    require(
        ...
        "Existing corrected subtrees changed",
    )
...
def require_regular_new_upload_modes(api, operations, work):
    # Read the backend's modes without uploading LFS content or making a commit.
    # Every new payload is small text; refuse unexpected LFS/ignore/overwrite modes
    # before the one-shot write instead of allowing new root attributes implicitly.
    import inspect
    from huggingface_hub import _commit_api, hf_api

    require(file_identity(Path(inspect.getsourcefile(_commit_api)))["sha256"] ==
            "250ed0e5a5a39383974cab5baae08a964e60f3f40c394c0d13a52e119e1f5f39",
            "Upload-mode client source changed")
    require(file_identity(Path(inspect.getsourcefile(hf_api)))["sha256"] ==
            "659636025aa3a7efefa69ca7f16741d8d6cc12f9301beac8c69f9b3d51f81cd4",
            "HF API client source changed")
    _commit_api._fetch_upload_modes(
        additions=operations, repo_type="dataset", repo_id=REPO,
        headers=api._build_hf_headers(token=True), revision=PARENT,
        endpoint="https://huggingface.co", create_pr=False,
    )
    observed = {
        operation.path_in_repo: {"mode": operation._upload_mode,
                                 "ignored": operation._should_ignore,
                                 "remote_oid": operation._remote_oid}
        for operation in operations
    }
    write_new(work / "upload-mode-preflight.json", {
        "observed_at_utc": stamp(), "repo_id": REPO, "revision": PARENT,
        "files": observed, "lfs_payloads_uploaded": False, "commit_created": False,
    })
    require(len(observed) == 660 and all(
        row == {"mode": "regular", "ignored": False, "remote_oid": None}
        for row in observed.values()), "Unexpected upload mode: no commit attempted")
...
def upload(work):
    ...
    require(ADDITION not in before_subtree, "Existing evaluation namespace; inspect first")
    operations = [
        CommitOperationAdd(
    ...
    require_regular_new_upload_modes(api, operations, work)
    write_new(
    ...
    commit = api.create_commit(
        ...
        token=True,
        num_threads=2,
        commit_message="Back up all corrected v3 final BEIR and validation scores (no source release)",
    )
    ...
    result = {
        **uploaded,
        ...
        "corrected_root_tree_changed_only_by_new_evaluation_subtree": True,
...
def download_verify(work):
    ...
    result = {
        "scope": "actual-complete-anonymous-evaluation-artifact-download-verification",
...
def main():
    ...
    require(
        ...
        and root.name.startswith("dense-v3-evaluation-artifact-backup.")
        and root.is_dir()
