...
scan = json.loads((work / 'publication-credential-audit.json').read_text())
...
assert len(verified) == len(scan['findings'])
result = {'complete': True, 'scan_sha256': hashlib.sha256((work / 'publication-credential-audit.json').read_bytes()).hexdigest(),
          'original_scan_modified': False, 'scanner_relaxed': False, 'unresolved_findings': 0, 'classifications': verified}
with (work / 'publication-credential-classification.json').open('x') as stream:
