"""One-shot, additions-only preservation of an accepted pretrained vector state."""
import argparse
import hashlib
...
ORIGINAL = ROOT / 'reports/engineering-archive/dense-v3-first-stage-evaluations-v1/functional-first-state'
...
PARENT = 'fdad53c239d9ed59141fcafbd92a0438bb1a657f'
NAMESPACE = 'corrected-dense-correctness-v3'
ADDITION = NAMESPACE + '/pretrained-functional-vectors'
...
def transport():
    ...
    spec = importlib.util.spec_from_file_location('pretrained_backup_transport', TRANSPORT)
...
def sources(t):
    reader.bound(ORIGINAL.parent / 'verification.json', {
        'sha256': '1053bf60fd193cfe4170887961c7abdc1420111615ee65572b16ab1fa86726e1'})
    ...
    for name, (size, sha) in reader.ORIGINALS.items():
        ...
        if name.startswith('vectors/'):
            original = EXP / 'analyses/dense-primary-v3-functional-dimensions/vectors/states/pretrained' / Path(name).name
            t.compare_file(original, value)
        elif name.startswith('native/'):
            original = EXP / 'launch/functional-dimensions/run/jobs' / Path(name).name
            t.compare_file(original, value)
        out[name] = (p, value)
...
def prepare(t, work):
    ...
    manifest = {'scope': 'accepted_pretrained_functional_vectors_data_only',
                'primary_protocol_sha256': reader.PROTOCOL, 'accepted_vector_states': 1,
                'expected_campaign_states': 61, 'feature_states': 0,
    ...
    result = {'prepared_at_utc': t.stamp(), 'repo_id': REPO, 'parent_revision': PARENT,
              ...
              'files': 10, 'bytes': sum(v['bytes'] for _, v in selected.values()) + ident['bytes'],
...
def check_modes(modes, prefix, names):
    ...
    for name in names:
        expected = {'mode': 'lfs' if name == 'vectors/vectors.npz' else 'regular',
...
def check_preservation(before, after, old_sub, new_sub):
    ...
    require(len(old_sub) == 9 and ADDITION not in old_sub
...
def upload(t, work):
    ...
    require('*.npz filter=lfs diff=lfs merge=lfs -text' in attrs.read_text().splitlines(),
            'Existing NPZ LFS rule missing')
    before, old_sub = t.root_inventory(api, PARENT), t.root_inventory(api, PARENT, NAMESPACE)
    require(len(old_sub) == 9 and ADDITION not in old_sub, 'Unexpected existing namespace')
    ...
    commit = api.create_commit(REPO, repo_type='dataset', parent_commit=PARENT, operations=ops,
        token=True, num_threads=2, commit_message='Preserve accepted pretrained DenseOn functional vectors (data only)')
    ...
    result = {'uploaded_at_utc': t.stamp(), 'repo_id': REPO, 'revision': commit.oid,
              'prefix': pre['prefix'], 'manifest': pre['manifest'], 'files': 10,
    ...
    audit = {**result, 'verified_at_utc': t.stamp(), 'durability_verified': True,
             ...
             'old_corrected_subtrees_unchanged': 9, 'root_attributes_unchanged': True,
...
def recover(t, work):
    ...
    result = {'completed_at_utc': t.stamp(), 'revision': audit['revision'], 'prefix': pre['prefix'],
              'downloaded_root': str(root), 'files': 10, 'bytes': pre['bytes'],
...
if __name__ == '__main__':
    ...
    require(args.workdir == Path(__file__).parent and args.workdir.parent == Path('/tmp')
            and args.workdir.name.startswith('dense-v3-pretrained-vector-backup.')
