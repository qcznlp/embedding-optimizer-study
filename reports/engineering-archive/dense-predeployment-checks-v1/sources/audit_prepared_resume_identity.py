...
def main():
    ...
    expected_identities, prototype_cases = {}, []
    prototype_checks = 0
    source_identity = {
    ...
    for run_id in RUNS:
        ...
        expected_identities[run_id] = expected

        original_identity = proposed(config, original_data)
        proposal.require_identity(expected, original_identity)
        prototype_cases.append(
            {
                "run_id": run_id,
                "case": "original",
                "should_equal": True,
                "requested_identity": original_identity,
            }
        )
        ...
        relocated_identity = proposed(moved, relocated)
        proposal.require_identity(expected, relocated_identity)
        prototype_cases.append(
            {
                "run_id": run_id,
                "case": "identical_relocation",
                "should_equal": True,
                "requested_identity": relocated_identity,
            }
        )
        prototype_checks += 2
        variants = [
    ...
    report = {
        ...
        "diagnostic_data": [
            {"root": str(path), "identity": data(path)}
            for path in (original_data, relocated, changed)
        ],
