...
controlled same-group replay comparisons**; its unpatched `run_training` entrypoint completes
**three baseline and three continuation calls**. This is a new detached source checkout, not a
deployment or a retrieval finding. The [older independent-process control](reports/engineering-archive/dense-canonical-replay-v1/README.md)
does not silently certify these changed source bytes. All earlier failures remain intact.

Keep the test results separate: the candidate's full suite has **923 cases / 11 unwaived old
source-contract failures**; the isolated paper/audit checkout passes **all 1,382 cases**.
...
finish the remaining predeployment checks and resolve deployment/publication gates. Do not resume
