...
Last updated: **2026-09-06 09:40 UTC**; the machine-readable snapshot is refreshed at this handoff.
...
The [preceding control](reports/engineering-archive/dense-canonical-replay-v1/README.md) separately
passed a new-process-group replay; that older receipt does not certify the newly changed source.
All are engineering evidence, not retrieval results or validation of the old matrix.
...
| Candidate checkpoint replay | 24/24 exact same-group checks with the explicit fixed-layout diagnostic control | Independent-process acceptance of this new source or default DDP bitwise equivalence |
...
| Isolated paper/audit suite | 1,382 cases, zero failures/errors/skips | Correctness of the different candidate source tree |
...
923-case suite with the isolated checkout's 1,382-case suite.
...
1. Review the [sealed candidate and transition proposal](reports/engineering-archive/dense-correction-candidate-v1/replication-transition-proposal.md).
   The bounded integration work is complete. Do not redo the old origin diagnosis. A separate-process
   replay of the new source and the full recipe/data/source resume gate remain explicit predeployment
   checks; do not silently adopt the diagnostic communication strategy as a speed improvement.
...
[the pre-update archive](reports/engineering-archive/dense-correction-candidate-v1/before/PROJECT_STATUS.md).
