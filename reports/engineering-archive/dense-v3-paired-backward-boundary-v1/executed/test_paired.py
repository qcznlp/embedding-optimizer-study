...
def fixture():
    ...
    trainer = SimpleNamespace(model_wrapped=model, model_accepts_loss_kwargs=True,
        compute_loss_func=None, args=SimpleNamespace(device=torch.device('cpu')),
...
def test_original_installed_training_step_scales_none_count_for_both_flags(accepts_loss_kwargs):
    ...
    assert hashlib.sha256(Path(inspect.getsourcefile(Trainer)).read_bytes()).hexdigest() == '060eda6fcd587e79caeca7c3f246ea7bab1af410479c9776584470b8f4ac48a8'
    model = torch.nn.Linear(1,1,bias=False)
    with torch.no_grad():
        model.weight.fill_(2.)
    owner = SimpleNamespace(
        _prepare_context_parallel_inputs=lambda m,i:(nullcontext,i),
        _prepare_inputs=lambda i:i, compute_loss_context_manager=nullcontext,
        compute_loss=lambda m,i,**kw:m.weight.square().sum(),
        optimizer=None, state=SimpleNamespace(global_step=313),
        args=SimpleNamespace(torch_empty_cache_steps=None,optim='adamw_torch',n_gpu=1),
        model_accepts_loss_kwargs=accepts_loss_kwargs, compute_loss_func=None,
        current_gradient_accumulation_steps=4, accelerator=Accelerator(cpu=True,gradient_accumulation_steps=1))
    assert Trainer._get_num_items_in_batch(owner,[{'query_input_ids':torch.ones(1)}],torch.device('cpu')) is None
    result = Trainer.training_step(owner,model,{},num_items_in_batch=None)
    assert result.item() == 1. and model.weight.grad.item() == 1.
