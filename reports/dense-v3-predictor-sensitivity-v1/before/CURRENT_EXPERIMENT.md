...
1. Complete primary retrieval, its eight-table replay, all raw-score backups and
   ...
   This does not show entropy adds information conditional on displacement, or
   exclude nonlinear rate/optimizer-stage effects omitted by the additive baseline.
   Any further sensitivity must be explicitly post-result and keep these locked
   results unchanged. The new report/source are local, not yet remotely backed up
   or published. Do not upload aggregate bundles containing embedded Python.
