...
def main():
    ...
    for j,group in enumerate(GROUPS):
        for i,feature in enumerate(features):
            ...
            matrix[i,j]=delta
            annotations[i,j]=('0.000' if delta==0 else f'{delta:+.3f}')+('*' if r['diagnostic_flag'] else '')
    ...
    fig.subplots_adjust(left=.40,right=.89,top=.80,bottom=.135)
    cax=fig.add_axes([.912,.24,.013,.43]);fig.colorbar(im,cax=cax,label='Added feature − comparator RMSE (points)')
