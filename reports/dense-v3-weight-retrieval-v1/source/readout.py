...
def pure_kernels(repository):
    ...
    def load(filename, names, deps):
        ...
        module.__dict__.update(dict(np=np, math=math, csv=csv, json=json, copy=copy,
             ...
             Decimal=Decimal, localcontext=localcontext, Q=Fraction, Any=object, Path=Path, **deps))
...
def inspect_inputs(args, kernels):
    ...
    for snapshot in index["snapshots"]:
        ...
        for row in snapshot["checkpoints"]:
            ...
            values = [task_map[(*key, task)] for task in index["tasks"]]
            exact_mean = sum(map(Fraction.from_float, values), Fraction()) / 14
            need(row["macro_ndcg_at_10"] == float(exact_mean), "Indexed rational macro differs")
            # Preserve the accepted original summarizer's fmean outcome bytes.
            # Rounded sum then division and rounded rational mean can differ by one ULP.
            need(score_map[(row["run_id"], stage)]["mean_ndcg_at_10"] == statistics.fmean(values),
                 "Original fmean macro outcome differs")
            ...
            for stage, ckpt in enumerate(plan["checkpoint_files"], 1):
                ...
                if kind == "approximate":
                    joined.append(dict(run_id=run_id, stage=stage, step=step,
                         ...
                         mean_ndcg_at_10=score_map[(run_id, stage)]["mean_ndcg_at_10"],
                         indexed_rational_macro_ndcg_at_10=indexed["macro_ndcg_at_10"]))
