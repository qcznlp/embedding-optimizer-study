...
1. Complete primary retrieval, its eight-table replay, all raw-score backups and
   the complete trajectory data-only snapshot are done. Do not restart workers,
   repeat completed backups or upload aggregate bundles containing embedded Python.
   With the full sixty-state outcomes now available, inspect the native/source
   integration needed to execute the already locked original nine-feature and
   separate five-feature exact weight-to-retrieval prediction analyses. Preserve
   all four dose folds, every feature/undefined row, exact numerical rules and
   existing admission boundaries; no dummy upstream acceptance, new post-hoc
   feature selection or formal-consumer success may be inferred from data readout.
