...
| Representation bridge | Both representation-tier `summary_manifest.json` files and `reports/representation-space/representation-dynamics.manifest.json` | Each tier contains exactly two pretrained plus 120 checkpoint reports with the frozen probe identity, sample groups, representation roles, and ranking-reference contract; the plotted dynamics rehash both strict summaries. |
