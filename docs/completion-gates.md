...
| NAACL manuscript | `paper/main.tex`, `paper/results.tex`, `paper/README.md`, `configs/paper_claim_protocol.json`, and `reports/paper-results.manifest.json` | `embed-optim-render-paper-results` generates the five headline macros from every frozen evidence tier; `make -C paper` builds with the pinned official ACL style; and `embed-optim-audit-paper --strict` proves that the claim protocol, source tables, generated TeX bytes, and every headline still match with no pending macro. |
