...
| Training artifacts | Run directories and `logs/formal-checkpoint-audit.json` | 24/24 terminal runs and 120/120 non-empty, step-consistent model/optimizer/scheduler/Trainer/RNG checkpoints pass deep validation. |
| Training dynamics | Canonical Trainer histories and W&B canonical runs | Every run has 391 finite loss records plus one explicit terminal/system row, unique increasing steps through 3,907, audited system metrics, and exactly one matching content-addressed W&B history tagged `canonical-current`; `embed-optim-sync-wandb` reads back and rehashes all 9,408 remote rows and audits current-run uniqueness, while retained historical revisions are permitted but must not carry the current tag. |
...
| Mean-versus-tail stability | `configs/tail_stability_analysis.json` and `reports/tail-stability/summary_manifest.json` | The explicitly post-hoc discovery tier rehashes all 20 anchor sample tables and reports 60 operator rows, four AdamW contrasts, 40 symmetric cross-tail rows, and four cross-tail summaries. Its separately prospective tier passes only after all 90 three-seed short-branch checkpoints show an auditable final-stage decision under the frozen joint loss-p95/unseen-margin-p05 rule; fixed-state evidence alone cannot satisfy this gate. |
| Spectrum/basis explanatory intervention | `configs/spectral_transplant_intervention.json` and `reports/spectral-transplant/summary_manifest.json` | All 20 frozen anchors contain the baseline, both native directions, four Adam-basis spectrum-interpolation points, the reverse spectrum/basis transplant, and three disjoint spectral-band transplants; exactly 49,280 paired sample records plus the 2×2, path, and band summaries pass source/output hash audit. This post-hoc gate may explain the immediate effect but cannot independently establish the long-horizon mechanism. |
...
| Cross-space join | `reports/mechanism-bridge/summary_manifest.json` | Exactly 120 checkpoint rows and 96 within-run transitions join the strict training-loss, weight, representation, and 1,680-unit BEIR sources; the seven frozen geometry associations remain descriptive and the two loss-to-retrieval associations are explicitly labeled post-hoc. |
