...
| NAACL manuscript | `paper/main.tex`, `paper/results.tex`, `paper/README.md`, and `configs/paper_claim_protocol.json` | `make -C paper` builds with the pinned official ACL style and `embed-optim-audit-paper --strict` proves that the claim protocol still binds every frozen analysis source, every headline has complete authoritative evidence, and no pending result macro remains. |
