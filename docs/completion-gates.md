...
| NAACL manuscript | `paper/main.tex`, `paper/results.tex`, five files under `paper/generated/`, `paper/README.md`, `configs/paper_claim_protocol.json`, and `reports/paper-results.manifest.json` | `embed-optim-render-paper-results` generates the five headline macros and five evidence tables from every frozen tier; `make -C paper` builds with the pinned official ACL style; and `embed-optim-audit-paper --strict` proves that the claim protocol, source tables, generated TeX hashes, headlines, and result tables still match with no pending marker. |
