...
def contracts():
    ...
    from embed_optim.primary_v3_publication_archive import PROTOCOL
    ...
    return PublicationContract.load(ROOT / PROTOCOL, primary)


def test_actual_exact_archive_authoring_refuses_missing_primary_before_any_output(tmp_path):
    from embed_optim import primary_v3_exact_reconstruction_archive as archive

    args = SimpleNamespace(experiment_root=tmp_path / "missing", output=tmp_path / "forbidden")
    with pytest.raises(ValueError, match="ordinary retained run"):
        archive.build(contracts(), args)
    assert not args.output.exists()


@pytest.mark.parametrize(
    "marker", ["upstream_primary_admission_simulated", "upstream_admission_simulated"]
)
def test_new_exact_authoring_cannot_accept_simulated_upstream(marker, tmp_path, monkeypatch):
    from embed_optim import primary_v3_exact_reconstruction_archive as archive

    contract = contracts()
    monkeypatch.setattr(archive.parent, "prepare", lambda *a: ({}, {}))
    monkeypatch.setattr(archive.exact, "gather", lambda *a: ({}, {"nested": {marker: True}}, {}))
    with pytest.raises(ValueError, match="Synthetic admission cannot supply primary publication"):
        archive.prepare(contract, SimpleNamespace())
    assert not list(tmp_path.iterdir())


def test_actual_source_only_exact_contract_closure_loads_after_relocation(tmp_path):
    from embed_optim import primary_v3_exact_reconstruction_archive as archive
    ...
    contract = contracts()
    exact_contract = archive.exact.ExactBridgeContract.load(
        ROOT / archive.PROTOCOL, contract.primary
    )
    selected = source_selection(contract.inference, {})
    archive.parent.add_publication_sources(contract, selected)
    archive.add_sources(exact_contract, selected)
    output = tmp_path / "relocated"
    record = files.write(output, selected, {"scope": "engineering_source_only_fixture"})
    files.inspect(output, record["manifest_sha256"])
    primary = PrimaryV3Contract.load(
        output / "source/configs/dense_primary_v3_protocol.json",
        output / "source",
        output / "training-source",
    )
    actual = archive.exact.ExactBridgeContract.load(output / "source" / archive.PROTOCOL, primary)
    assert actual.sha256 == exact_contract.sha256
    assert len(actual.primary.inputs["runs"]) == 12


@pytest.mark.parametrize("kind", ["zero", "norm"])
def test_coherently_recomputed_but_wrong_state_measurement_is_refused(tmp_path, kind):
    raw, contract, old = fixture(tmp_path)
    directory = tmp_path / "exact"
    manifest = read_json(directory / "manifest.json")
    entry = manifest["outputs"][1]
    record_path, arrays_path = (directory / entry[key]["path"] for key in ("records", "arrays"))
    value, arrays = read_json(record_path), load_file(arrays_path)
    for row in value["records"]:
        prefix = row["displacement_kind"] + "|" + row["tensor"] + "|"
        if kind == "zero":
            arrays[prefix + "singular"].zero_()
            for side in ("left", "right"):
                arrays.pop(prefix + side)
        else:
            arrays[prefix + "singular"] *= 2
        row.update(
            spectrum_metrics(
                arrays[prefix + "singular"].numpy(),
                row["shape"],
                SETTINGS,
                None if kind == "zero" else row["relative_reconstruction_residual"],
            )
        )
    record_path.write_text(json.dumps(value))
    save_file(
    ...
    for key in ("records", "arrays"):
        entry[key].update(file_identity(directory / entry[key]["path"]))
    (directory / "manifest.json").write_text(json.dumps(manifest))
    with pytest.raises(ValueError, match="same displacement norm"):
        inspect(tmp_path, raw, contract, old)
