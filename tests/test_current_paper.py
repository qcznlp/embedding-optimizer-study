...
def test_default_make_target_uses_complete_paper_and_preserves_legacy_source():
    makefile = (ROOT / "paper/Makefile").read_text()
    assert ".DEFAULT_GOAL := current" in makefile
    assert "\nall: current\n" in makefile
    ...
    assert "embed_optim.paper_reproduction" in release
    assert "--numerical-bundle" in release and "--paper-dir" in release
    assert "--output" in release and "test -n" in release
    assert hashlib.sha256((ROOT / "paper/legacy.Makefile").read_bytes()).hexdigest() == (
        "2218e172b16bf812c47495174715aaf816dc8e35f6ae0b7d5158a1f3e2de44c2"
    )
    assert "MAKE='$(MAKE) -f legacy.Makefile' release" in makefile


def test_formatter_exception_is_only_for_the_authenticated_original_checker():
    config = tomllib.loads((ROOT / "pyproject.toml").read_text())["tool"]["ruff"]
    assert config["format"]["exclude"] == ["src/embed_optim/complete_paper_document.py"]
    assert config["lint"]["per-file-ignores"] == {
        "src/embed_optim/complete_paper_document.py": ["I001"]
    }
    assert (
        hashlib.sha256(Path(document.__file__).read_bytes()).hexdigest() == entry.COMPONENT_SHA256
    )
