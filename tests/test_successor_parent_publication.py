...
def test_repaired_parent_preserves_all_47_commands_and_arguments_exactly():
    ...
    # Compare in the recorded coordinate system using the original four-slot
    # path projection. No command, flag, interpreter or scientific input changes.
    recorded_paper = next(item for item in old["steps"] if item["name"] == "paper-release")
    recorded_root = Path(recorded_paper["command"][2]).parent
    steps = project_steps(steps, ROOT, recorded_root)
