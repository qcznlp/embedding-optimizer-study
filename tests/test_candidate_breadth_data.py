...
def test_nested_candidates_skip_noneligible_positive_rows_before_identity_check() -> None:
    row = {
        "positive_id": 90_000,
        "negative_ids": list(range(1, 8)),
        "negative_pool_indices": list(range(7)),
    }
    with pytest.raises(InsufficientEligibleCandidates, match="fewer than ten"):
        nested_candidate_ids(
            [80_000, *range(1, 2049)],
            [1.0, *([1.0] * 2048)],
            row,
...
def test_nested_candidates_reject_eligible_row_with_different_positive() -> None:
    row = {
        "positive_id": 90_000,
        "negative_ids": list(range(1, 8)),
        "negative_pool_indices": list(range(7)),
    }
    with pytest.raises(ValueError, match="positive does not match"):
        nested_candidate_ids(
            [80_000, *range(1, 2049)],
            [1.0, *([0.9] * 2048)],
            row,
