...
def test_release_steps_bind_candidate_outputs_and_dense_release_gates(tmp_path: Path) -> None:
    ...
    assert steps[10].command == steps[9].command + ("--audit-only",)
    assert steps[17].command == steps[10].command
    assert steps[11].name == "paper-results"
    assert steps[11].command != steps[9].command
    ...
    for step in (*steps[5:9], steps[11], steps[12], steps[18]):
