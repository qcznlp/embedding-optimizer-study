...
def test_width_seven_baseline_check_is_sample_exact(tmp_path) -> None:
    ...
    assert result["worst_sample_id"] == 10
    assert result["worst_metric"] == METRICS[0]

    records[0]["positive_margin"] += 1e-3
    failed = _baseline_check(records, baseline, tolerance=1e-5)
    assert failed["within_tolerance"] is False
    assert failed["maximum_absolute_error"] == pytest.approx(1.0001e-3)
    assert failed["worst_sample_id"] == 10
    assert failed["worst_metric"] == "positive_margin"
    assert failed["metric_maximum_absolute_errors"]["positive_margin"] == pytest.approx(1.0001e-3)
