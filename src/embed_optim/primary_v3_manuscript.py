...
RESULT_MACROS = {
    RESULT_FILES[0]: (
        ...
        "ExactGeometrySensitivityFinding",
        "ExactGeometrySensitivityTable",
    ),
    RESULT_FILES[1]: (
...
def render_constants(primary):
    ...
    for row in rows:
        ...
        require_same(
            expected["data"]["selected_columns"],
            ["query", "positive", *(f"negative_{i}" for i in range(7)), "length"],
        )
    ...
    for recipe in recipes:
        if recipe["max_length"] != 8192:
            raise ValueError("Primary context constant differs")
...
def inspect_sources(repository, expected_includes, constants):
    ...
    main = texts["main.tex"]
    if definitions(main) or re.search(r"\\let(?![A-Za-z@])", main):
        raise ValueError("Main text may not redefine generated result macros")
    ...
    for name, text in texts.items():
        ...
        if name != "main.tex" and INCLUDEGRAPHICS.search(text):
            raise ValueError("Generated evidence may not introduce an external figure")
    for name, required in RESULT_MACROS.items():
        macros = definitions(texts[name])
        for macro in required:
            values = macros.get(macro, [])
            wrapped = macro in {"CorrectedGeometryBridgeFinding", "CorrectedGeometryBridgeTable"}
            if (
                len(values) != (2 if wrapped else 1)
                or any(value["arity"] != 0 or not value["body"].strip() for value in values)
                or values[0]["kind"] != r"\newcommand"
                or (wrapped and values[1]["kind"] != r"\renewcommand")
            ):
                raise ValueError("Incomplete or redefined active result macro: " + macro)
