...
def nested_candidate_ids(
    ...
    *,
    ...
    # A query can have one mined-score row per positive.  The frozen validation
    # builder skips non-eligible rows before accepting the first row with a
    # ten-negative pool, so identity must be checked only after that same
    # eligibility decision.  An eligible row with a different positive still
    # indicates source drift and remains a hard failure.
