...
def _baseline_check(
    ...
    *,
    ...
    """Measure, but do not suppress, a failure of the frozen bridge check.

    The prospective protocol makes width-7 reproduction part of the support
    rule and explicitly says that a failure falsifies the missing-candidate
    explanation.  A mismatch is therefore a scientific result that must reach
    the summary, rather than a reason to discard all wider-width measurements.
    Structural/source mismatches remain hard errors.
    """
    ...
    maximum_error = 0.0
    metric_errors = {metric: 0.0 for metric in METRICS}
    worst_sample_id: int | None = None
    worst_metric: str | None = None
    for sample_id, row in observed.items():
        for metric in METRICS:
            error = abs(float(row[metric]) - float(expected[sample_id][metric]))
            metric_errors[metric] = max(metric_errors[metric], error)
            if error > maximum_error:
                maximum_error = error
                worst_sample_id = sample_id
                worst_metric = metric
    return {
        ...
        "metric_maximum_absolute_errors": metric_errors,
